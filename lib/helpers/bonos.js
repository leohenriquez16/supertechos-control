...
export const KPIS_SUPERVISOR = [
  ...
  { key: 'avance', label: 'Obras avanzando', pesoDefault: 20 },
...
export const KPIS_GERENTE = [
  ...
  { key: 'levantamientos', label: 'Levantamientos a tiempo', pesoDefault: 10 },
...
export function calcularBonoSupervisor(persona, ctx) {
  ...
  return combinar(aplicarOverrides([kJornadas, kReportes, kAvance, kProd, kCalidad], config?.kpiOverrides));
}
...
export function calcularBonoGerente(persona, ctx) {
  ...
  const kLev = { key: 'levantamientos', label: 'Levantamientos a tiempo', peso: 10, score: pct(aTiempo, svs.length),
