...
export function calcularCuadre({ movimientos, fechaInicio, fechaFin }) {
  ...
  // Separamos gastos con/sin factura — son distintos para auditoría.
  const gastosConFactura = gastosAprob.filter(m => !m.datosIA?.sin_factura);
  const gastosSinFactura = gastosAprob.filter(m => !!m.datosIA?.sin_factura);

  const totalConFactura = gastosConFactura.reduce((s, m) => s + Number(m.monto || 0), 0);
  const totalSinFactura = gastosSinFactura.reduce((s, m) => s + Number(m.monto || 0), 0);

  // Agrupación por categoría (todos los aprobados, con o sin factura)
  const porCategoria = {};
