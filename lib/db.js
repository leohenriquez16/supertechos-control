...
const cajaChicaRowToObj = (m) => ({
  ...
  // Nota: foto_data NO se carga aquí para no inflar listados.
  // Usar obtenerFotoFactura(id) cuando la quieras mostrar.
  tieneFoto: !!m.foto_data || !!m.foto_url,
...
const SELECT_CAJA_SIN_FOTO = 'id, persona_id, proyecto_id, fecha, tipo, monto, signo_ajuste, proveedor, rnc, concepto, datos_ia, status, motivo_rechazo, aprobado_por_id, aprobado_at, creado_por_id, created_at';

// Listar movimientos con filtros opcionales. NO trae foto_data (pesa).
export async function listarMovimientosCajaChica({
  ...
  let q = supabase.from('caja_chica_movimientos').select(SELECT_CAJA_SIN_FOTO).order('fecha', { ascending: false });
  if (personaId) q = q.eq('persona_id', personaId);
  if (proyectoId) q = q.eq('proyecto_id', proyectoId);
...
// Crea un movimiento. Para gasto_factura, foto_data debe venir como dataURL base64.
export async function crearMovimientoCajaChica(mov) {
  const row = {
    id: mov.id || ('cc_' + Date.now() + Math.random().toString(36).slice(2, 6)),
  ...
  const { data, error } = await supabase.from('caja_chica_movimientos').insert(row).select(SELECT_CAJA_SIN_FOTO).single();
  if (error) throw error;
...
// Trae solo foto_data de un movimiento (lazy, evita inflar listados).
export async function obtenerFotoFacturaCajaChica(id) {
  const { data, error } = await supabase.from('caja_chica_movimientos').select('foto_data').eq('id', id).single();
  if (error) throw error;
  return data?.foto_data || null;
}
