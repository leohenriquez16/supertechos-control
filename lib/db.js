...
export async function loadAllData() {
  try {
    const [personalRes, sistemasRes, proyectosRes, reportesRes, enviosRes, configRes] = await Promise.all([
      ...
      supabase.from('personal').select('id, nombre, pin, roles, maestro_id, telefono, direccion, cedula_numero, fecha_ingreso, recomendado_por, email, foto_2x2, cedula_frente, cedula_reverso, reporte_audio_habilitado, modo_pago, tarifa_m2, tarifa_dia, caja_chica_habilitada, limite_caja_chica').order('nombre'),
