...
export async function loadAllData() {
  try {
    ...
    const personal = (personalRes.data || []).map(mapPersonaRow);
    ...
    } : { costos_indirectos_pct: 15, margen_objetivo_pct: 30, mostrarMiProduccionNomina: false };
...
export async function listarGarantias({ incluirArchivadas = false } = {}) {
  ...
  const { data, error } = await q;
  if (error) throw error;
  return (data || []).map(garantiaRowToObj);
}
...
export async function crearGarantia(g, { cadaMeses = null } = {}) {
  ...
  // Generar calendario de mantenimientos.
  if (cadaMeses) {
    const ms = calcularMantenimientos(garantia, cadaMeses).map(m => ({
      id: 'man_' + Date.now() + Math.random().toString(36).slice(2, 6),
      garantia_id: garantia.id, cliente_id: garantia.clienteId, ubicacion_id: garantia.ubicacionId,
      tipo: 'inspeccion', fecha_programada: m.fecha, estado: m.estado,
    }));
...
export async function crearGarantiaDesdeProyecto(proyecto, sistema) {
  ...
  const duracion = sistema?.garantia_meses || sistema?.garantiaMeses || null;
  ...
  return crearGarantia({
    ...
    sistemaId: proyecto.sistema, sistemaNombre: sistema?.nombre || null,
    fechaInicio, duracionMeses: duracion, m2, monto: proyecto.valorCotizacion ?? null,
    origen: 'erp',
  }, { cadaMeses: sistema?.mantenimiento_cada_meses || sistema?.mantenimientoCadaMeses || null });
}
