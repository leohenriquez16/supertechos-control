...
export async function crearProyecto(proy) {
  ...
  // v8.10.22: si no se mandó fechaAprobacion explícita y el proyecto se crea como 'aprobado',
  // usar hoy como fallback. Para otros estados (cubicando, etc.), null.
  const estadoInicial = proy.estado || 'aprobado';
  const fechaAprobacion = proy.fechaAprobacion ||
    (estadoInicial === 'aprobado' ? new Date().toISOString().split('T')[0] : null);
