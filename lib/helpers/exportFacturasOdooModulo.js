...
// v8.27.51: resolver concepto → NOMBRE EXACTO de producto de Odoo. El match del
// import es case-sensitive y algunos productos tienen sufijo ("Materiales Varios
// (copia)"), así que: 1) exacto sin distinguir mayúsculas; 2) si no, único que
// contiene el texto. Devuelve el nombre EXACTO de Odoo o null.
export function construirResolverProducto(productos = []) {
  const exact = {}; // nombre en minúsculas -> nombre exacto
  const all = [];
  for (const p of productos) {
    const n = String(p.name || '').trim();
    if (!n) continue;
    const low = n.toLowerCase();
    if (exact[low] == null) exact[low] = n;
    all.push({ low, name: n });
  }
  return (value) => {
    ...
    const hits = all.filter((p) => p.low.includes(low));
...
function toCSV(facturas, catPorId = {}, maps = {}) {
  ...
  const headers = [
    ...
    // product_id (nombre EXACTO de Odoo) — así Odoo autocompleta cuenta e ITBIS del producto.
  ...
  const rows = facturas.map((f) => {
    ...
    // Producto: si hay mapeo por categoría (nombre EXACTO puesto por el usuario), se escribe
    // TAL CUAL —incluye productos company-specific que el resolver quizá no ve—. Si no hay
    // mapeo, se resuelve el concepto al nombre exacto de Odoo (arregla mayúsculas/sufijos);
    // vacío si no resuelve (no bloquea) y se reporta.
    const mapeadoProducto = (cat?.odooProducto || '').trim();
    let producto = '';
    if (mapeadoProducto) {
      producto = mapeadoProducto;
    } else {
      const pista = (f.concepto || '').trim();
      if (pista) {
        const exacto = resolverProducto ? resolverProducto(pista) : null;
        if (exacto) producto = exacto;
        else if (sin) sin.producto.add(pista);
      }
    }
