...
// v8.54.2 (C2): últimas comunicaciones por entidad de un tipo (para la Torre: "sin responder"
// y "última comunicación"). Devuelve, por entity_id, la comunicación más reciente.
export async function ultimasComunicaciones(entityType) {
  if (!entityType) return {};
  const { data } = await supabase
    .from('chatter_mensajes')
    .select('entity_id, direccion, canal, created_at')
    .eq('entity_type', entityType).eq('tipo', 'comunicacion')
    .order('created_at', { ascending: false })
    .limit(2000);
  const out = {};
  (data || []).forEach((r) => { if (!out[r.entity_id]) out[r.entity_id] = { direccion: r.direccion, canal: r.canal, createdAt: r.created_at }; });
  return out;
}
