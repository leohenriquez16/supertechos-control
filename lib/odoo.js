...
// v8.41.0: buscar SUPLIDORES en Odoo por nombre o RNC — para vincular la entidad
// suplidor del ERP con su res.partner (espejo: el nombre se alinea al de Odoo).
export async function buscarSuplidoresOdoo(q) {
  const url = process.env.ODOO_URL, db = process.env.ODOO_DB;
  const username = process.env.ODOO_USERNAME, apiKey = process.env.ODOO_API_KEY;
  if (!url || !db || !username || !apiKey) throw new Error('Faltan variables de entorno de Odoo');
  const uid = await odooAuthenticate(url, db, username, apiKey);
  const digits = String(q || '').replace(/\D/g, '');
  const dom = digits.length >= 9
    ? [['vat', 'ilike', digits]]
    : [['supplier_rank', '>', 0], ['name', 'ilike', String(q || '').trim()]];
  const ps = await odooCall(url, db, uid, apiKey, 'res.partner', 'search_read',
    [dom], { fields: ['id', 'name', 'vat', 'supplier_rank'], limit: 8 });
  return (ps || []).map(p => ({ id: p.id, name: p.name || '', vat: p.vat || '' }));
}

// v8.46.0: COSTOS REALES por cuenta analítica (rentabilidad por obra).
// Lee account.analytic.line de la(s) cuenta(s): las compras/facturas de proveedor
// imputadas a la obra entran con amount NEGATIVO; ingresos con positivo.
// Devuelve total de costo (positivo) + desglose por línea para el detalle.
export async function costosAnaliticaOdoo(analiticaIds) {
  const url = process.env.ODOO_URL;
  const db = process.env.ODOO_DB;
  const username = process.env.ODOO_USERNAME;
  const apiKey = process.env.ODOO_API_KEY;
  if (!url || !db || !username || !apiKey) throw new Error('Faltan variables de entorno de Odoo');
  const ids = (Array.isArray(analiticaIds) ? analiticaIds : [analiticaIds])
    .map(n => Number(n)).filter(n => n > 0);
  if (!ids.length) return { totalCosto: 0, totalIngreso: 0, lineas: [] };
  const uid = await odooAuthenticate(url, db, username, apiKey);
  const rows = await odooCall(url, db, uid, apiKey, 'account.analytic.line', 'search_read',
    [[['account_id', 'in', ids]]],
    { fields: ['id', 'date', 'name', 'amount', 'product_id', 'partner_id', 'account_id'], limit: 5000, order: 'date asc' });
  let totalCosto = 0, totalIngreso = 0;
  const lineas = (rows || []).map(l => {
    const amount = Number(l.amount) || 0;
    if (amount < 0) totalCosto += -amount; else totalIngreso += amount;
    return {
      id: l.id, fecha: l.date || '', concepto: l.name || '',
      monto: amount,
      producto: Array.isArray(l.product_id) ? l.product_id[1] : null,
      proveedor: Array.isArray(l.partner_id) ? l.partner_id[1] : null,
      cuenta: Array.isArray(l.account_id) ? l.account_id[1] : null,
    };
  });
  return { totalCosto: Math.round(totalCosto * 100) / 100, totalIngreso: Math.round(totalIngreso * 100) / 100, lineas };
}

// v8.47.0: SALIDAS DE ALMACÉN por referencia de cotización (rentabilidad por obra).
// Los despachos a obra se registran en Odoo como delivery orders (stock.picking
// outgoing, done) con la referencia en `origin` (a veces con formato libre:
// "ST 5716" vs "ST-C5716"). Buscamos por el número de la referencia y agregamos
// las cantidades hechas por producto.
// v8.49.5: salidas de almacén PENDIENTES en Odoo — las entregas que generan las ventas
// confirmadas (stock.picking outgoing, sin hacer). El módulo Almacén las importa como
// despachos para alistar con firma. READ-ONLY: no se escribe nada en Odoo.
export async function salidasPendientesOdoo() {
  const url = process.env.ODOO_URL;
  const db = process.env.ODOO_DB;
  const username = process.env.ODOO_USERNAME;
  const apiKey = process.env.ODOO_API_KEY;
  if (!url || !db || !username || !apiKey) throw new Error('Faltan variables de entorno de Odoo');
  const uid = await odooAuthenticate(url, db, username, apiKey);
  // Solo desde HOY en adelante (pedido de Leo 2026-08-28): Odoo arrastra ~76 entregas
  // sin validar desde 2021-22 y no queremos halar nada viejo — limpieza aparte.
  const desde = '2026-08-28';
  const picks = await odooCall(url, db, uid, apiKey, 'stock.picking', 'search_read',
    [[['picking_type_id.code', '=', 'outgoing'], ['state', 'in', ['confirmed', 'assigned', 'waiting']], ['scheduled_date', '>=', desde]]],
    { fields: ['id', 'name', 'origin', 'scheduled_date', 'partner_id', 'state', 'company_id'], limit: 100, order: 'scheduled_date desc' });
  if (!picks || !picks.length) return { pendientes: [] };
  const ids = picks.map(p => p.id);
  const moves = await odooCall(url, db, uid, apiKey, 'stock.move', 'search_read',
    [[['picking_id', 'in', ids], ['state', 'not in', ['done', 'cancel']]]],
    { fields: ['picking_id', 'product_id', 'product_uom_qty', 'product_uom'], limit: 2000 });
  const porPick = {};
  (moves || []).forEach(m => {
    const pid = Array.isArray(m.picking_id) ? m.picking_id[0] : m.picking_id;
    (porPick[pid] = porPick[pid] || []).push({
      descripcion: Array.isArray(m.product_id) ? m.product_id[1] : String(m.product_id || ''),
      cantidad: Number(m.product_uom_qty) || null,
      unidad: Array.isArray(m.product_uom) ? m.product_uom[1] : '',
    });
  });
  return {
    pendientes: picks.map(p => ({
      pickingId: p.id, name: p.name, origin: p.origin || '',
      cliente: Array.isArray(p.partner_id) ? p.partner_id[1] : '',
      fecha: (p.scheduled_date || '').slice(0, 10), estado: p.state,
      empresa: Array.isArray(p.company_id) ? p.company_id[1] : '',
      items: porPick[p.id] || [],
    })),
  };
}

export async function salidasAlmacenOdoo(referencias) {
  const url = process.env.ODOO_URL;
  const db = process.env.ODOO_DB;
  const username = process.env.ODOO_USERNAME;
  const apiKey = process.env.ODOO_API_KEY;
  if (!url || !db || !username || !apiKey) throw new Error('Faltan variables de entorno de Odoo');
  const refs = (Array.isArray(referencias) ? referencias : [referencias]).filter(Boolean);
  // número de la ref: "ST-C5716" -> "5716" (los pickings usan "ST 5716", "ST-C5716", etc.)
  const numeros = [...new Set(refs.map(r => String(r).replace(/\D+/g, '')).filter(n => n.length >= 3))];
  if (!numeros.length) return { pickings: [], productos: [] };
  const uid = await odooAuthenticate(url, db, username, apiKey);
  const dominio = ['&', ['state', '=', 'done'], ['picking_type_id.code', '=', 'outgoing']];
  if (numeros.length === 1) dominio.push(['origin', 'ilike', numeros[0]]);
  else {
    for (let i = 0; i < numeros.length - 1; i++) dominio.push('|');
    numeros.forEach(n => dominio.push(['origin', 'ilike', n]));
  }
  const picks = await odooCall(url, db, uid, apiKey, 'stock.picking', 'search_read', [dominio],
    { fields: ['id', 'name', 'origin', 'date_done', 'partner_id'], limit: 200, order: 'date_done asc' });
  if (!picks || !picks.length) return { pickings: [], productos: [] };
  const ids = picks.map(p => p.id);
  const moves = await odooCall(url, db, uid, apiKey, 'stock.move', 'search_read',
    [[['picking_id', 'in', ids], ['state', '=', 'done']]],
    { fields: ['picking_id', 'product_id', 'product_uom_qty', 'quantity'], limit: 2000 });
  const porProducto = {};
  (moves || []).forEach(m => {
    const nombre = Array.isArray(m.product_id) ? m.product_id[1] : String(m.product_id || '');
    const qty = Number(m.quantity != null ? m.quantity : m.product_uom_qty) || 0;
    if (!porProducto[nombre]) porProducto[nombre] = { producto: nombre, cantidad: 0 };
    porProducto[nombre].cantidad += qty;
  });
  return {
    pickings: picks.map(p => ({ id: p.id, name: p.name, origin: p.origin || '', fecha: p.date_done || '' })),
