...
export function mapPersonaRow(p) {
  return {
    ...
    tipoDocumento: p.tipo_documento || 'cedula', // v8.27.25: 'cedula' | 'pasaporte'
    ...
    cartaBancaria: p.carta_bancaria || '',
    buenaConducta: p.buena_conducta || '',
    ...
    facturasHabilitada: !!p.facturas_habilitada, // v8.27.36: módulo Facturas (Lily)
    levantamientoHabilitado: !!p.levantamiento_habilitado,
    ...
    gerenteNombre: p.gerente_nombre || '',
    odooEmployeeId: p.odoo_employee_id || null,
    whatsapp: p.whatsapp || '',
    ...
    inspeccionesHabilitada: !!p.inspecciones_habilitada, // v8.35.3: solo Jhonathan
    activo: p.activo !== false, // v8.49.0: default activo si la columna aún no existe o es null
  };
}

// v8.49.0: completitud de la ficha de RR.HH. de una persona. Devuelve el % lleno
// y qué campos obligatorios faltan, para el medidor y el filtro "fichas incompletas".
// foto2x2 se excluye a propósito (se carga en 2.º plano y daría falsos negativos).
export function completitudPersona(p) {
  const req = [
    ['Cédula', p?.cedulaNumero],
    ['Teléfono', p?.telefono],
    ['Dirección', p?.direccion],
    ['Fecha de nacimiento', p?.fechaNacimiento],
    ['Contacto de emergencia', p?.contactoEmergenciaNombre && p?.contactoEmergenciaTelefono],
    ['Tipo de sangre', p?.tipoSangre],
    ['Fecha de ingreso', p?.fechaIngreso],
    ['Puesto', p?.puesto],
    ['Departamento', p?.departamento],
    ['Cuenta de banco', p?.bancoNumeroCuenta],
  ];
  const lleno = (v) => !!v && (typeof v !== 'string' || v.trim() !== '');
  const faltan = req.filter(([, v]) => !lleno(v)).map(([k]) => k);
  const total = req.length;
  const llenos = total - faltan.length;
  return { pct: Math.round((llenos / total) * 100), faltan, total, llenos };
}
