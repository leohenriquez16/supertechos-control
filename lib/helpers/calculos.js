...
// v8.17.79: valor "derivado" del proyecto = Σ m²×precio (principal + suplementos)
// de cada área. Es el valor teórico del sistema antes de reconciliar con lo cotizado.
export const getValorDerivadoProyecto = (proyecto, sistema, sistemas) => {
  return (proyecto.areas || []).reduce((acc, area) => {
    const sistemaIdArea = area.sistemaId || proyecto.sistema;
    const sistemaArea = (sistemas && sistemas[sistemaIdArea]) || sistema;
    if (!sistemaArea) return acc;
    return acc + area.m2 * getPrecioTotalM2Area(area, sistemaArea, sistemas);
  }, 0);
};

// Factor de escala al valor cotizado real (Odoo / PDF). La producción "cruda"
// (m²×precio teórico × peso) se multiplica por este factor para valorarla al precio
// que realmente firmó el cliente. Devuelve 1 si el proyecto no tiene valorCotizacion
// cargado. Es la MISMA lógica que usa calcAvanceProyecto para el detalle del proyecto.
export const getFactorCotizacion = (proyecto, sistema, sistemas) => {
  const cotizada = proyecto.valorCotizacion != null && proyecto.valorCotizacion !== '' && !isNaN(Number(proyecto.valorCotizacion));
  if (!cotizada) return 1;
  const valorDerivado = getValorDerivadoProyecto(proyecto, sistema, sistemas);
  return valorDerivado > 0 ? Number(proyecto.valorCotizacion) / valorDerivado : 1;
};
...
export const calcAvanceArea = (proyecto, areaId, reportes, sistema) => {
  ...
  const reportesArea = reportes.filter(r => r.proyectoId === proyecto.id && r.areaId === areaId && !r.reparacion); // v8.50.0: retoques no suman avance
  const m2PorTarea = {};
...
export const calcAvanceProyecto = (proyecto, reportes, sistema, sistemas) => {
  ...
  // Si está, también escalamos el producido proporcionalmente (factor) para que
  ...
  const factor = getFactorCotizacion(proyecto, sistema, sistemas);
...
export const produccionPorDia = (reportes, proyectos, sistemas) => {
  const porDia = {};
  reportes.forEach(r => {
    if (r.reparacion) return; // v8.50.0: retoques no son producción nueva
...
export const calcEstadoPagoProyecto = ({
  ...
  const reportesProy = (reportes || []).filter(r => r.proyectoId === proyecto.id && !r.reparacion); // v8.50.0: retoques no devengan MDO
