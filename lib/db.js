...
import { limpiarNombreProyecto } from './helpers/proyectos'; // v8.51.2: guardrail nombre de obra
...
// v8.50.4 (ticket Miguel "se borraron los avances"): Supabase corta los SELECT en 1,000
// filas por defecto. Al pasar de 1,000 reportes (hoy ~1,860), las obras viejas dejaban de
// recibir sus reportes y el avance salía en 0% — la data SIEMPRE estuvo completa en la DB.
// Se traen TODOS paginando en bloques de 1,000.
// v8.50.5: helper GENÉRICO contra el tope de 1,000 filas de Supabase. Recibe una
// función que construye la query (se reconstruye por página porque los builders no
// son reutilizables) y devuelve TODAS las filas paginando en bloques de 1,000.
async function fetchSinTope(buildQuery) {
  const PAGE = 1000;
  let todas = [];
  for (let desde = 0; ; desde += PAGE) {
    const { data, error } = await buildQuery().range(desde, desde + PAGE - 1);
...
async function fetchTodosLosReportes() {
  ...
  for (let desde = 0; ; desde += PAGE) {
    const { data, error } = await supabase.from('reportes').select('*')
      .order('fecha', { ascending: false }).range(desde, desde + PAGE - 1);
    if (error) return { data: todos, error: todos.length ? null : error };
...
export async function loadAllData() {
  try {
    const [personalRes, sistemasRes, proyectosRes, reportesRes, enviosRes, configRes, categoriasCajaRes] = await Promise.all([
      ...
      supabase.from('personal').select('id, nombre, pin, roles, maestro_id, telefono, direccion, cedula_numero, tipo_documento, fecha_ingreso, recomendado_por, email, reporte_audio_habilitado, modo_pago, tarifa_m2, tarifa_dia, caja_chica_habilitada, facturas_habilitada, levantamiento_habilitado, limite_caja_chica, max_transaccion_caja_chica, pin_temporal, onboarding_completado, invitado_at, invitado_por_id, fecha_nacimiento, whatsapp, tipo_sangre, contacto_emergencia_nombre, contacto_emergencia_telefono, contacto_emergencia_relacion, banco, banco_tipo_cuenta, banco_numero_cuenta, banco_titular_nombre, banco_titular_cedula, dieta_habilitada, hospedaje_habilitado, inspecciones_habilitada, activo').order('nombre'),
      supabase.from('sistemas').select('*'),
      fetchSinTope(() => supabase.from('proyectos').select('*').order('created_at', { ascending: false })).then(d => ({ data: d })), // v8.50.5: sin tope 1000
      fetchTodosLosReportes(), // v8.50.4: SIN el tope de 1000 filas de Supabase (ver fetchTodosLosReportes)
      fetchSinTope(() => supabase.from('envios').select('*').order('fecha', { ascending: false })).then(d => ({ data: d })), // v8.50.5: sin tope 1000
    ...
    const sistemas = {};
    // v8.27.79: merge de garantía/mantenimiento (columnas propias) al objeto del sistema —
    // el Reporte Final de Obra las necesita en el cliente.
    (sistemasRes.data || []).forEach(s => {
      sistemas[s.id] = { ...s.data, garantia_meses: s.garantia_meses ?? s.data?.garantia_meses ?? null, mantenimiento_cada_meses: s.mantenimiento_cada_meses ?? s.data?.mantenimiento_cada_meses ?? null };
    });
    ...
    const proyectos = (proyectosRes.data || []).filter(p => !p.archivado).map(p => ({
      ...
      // v8.32.0: vínculo con la cuenta analítica de Odoo + sub-cotizaciones detectadas
      analiticaOdooId: p.analitica_odoo_id || null,
      analiticaOdooNombre: p.analitica_odoo_nombre || null,
      subCotizaciones: p.sub_cotizaciones || [],
      ...
      brigadas: p.brigadas || [], // v8.27.22: cuadrillas del proyecto [{id,nombre,maestroId,ayudantesIds}]
      ...
      // v8.9.13: pausas
      pausas: p.pausas || [],
      // v8.27.73: proyectos de cotización en USD (el valor en áreas ya está en RD$)
      monedaOrigen: p.moneda_origen || null,
      tasaUsd: p.tasa_usd != null ? Number(p.tasa_usd) : null,
      createdAt: p.created_at, // v8.27.73: para "proyectos nuevos por mes" en Producción
    }));
    ...
    const reportes = (reportesRes.data || []).map(r => ({
      ...
      // v8.30.4: revisión del supervisor
      revisadoPorId: r.revisado_por_id || null, revisadoPorNombre: r.revisado_por_nombre || null,
      revisadoAt: r.revisado_at || null, observacionSupervisor: r.observacion_supervisor || null,
      // v8.9.11: audio + IA
      audioUrl: r.audio_url || null,
      transcripcion: r.transcripcion || null,
      datosIA: r.datos_ia || {},
      retroactivo: !!r.retroactivo, // v8.27.73: reporte atrasado / carga inicial (excluible del histórico limpio)
      excluirNomina: !!r.excluir_nomina, // v8.27.77: duplicado de trabajo ya pagado → fuera del cálculo de nómina
      excluirNominaMotivo: r.excluir_nomina_motivo || null, // v8.50.3: constancia de por qué se excluyó
      reparacion: !!(r.datos_ia && r.datos_ia.reparacion), // v8.50.0: retoque en área ya trabajada — no suma avance/producción/nómina
    ...
    const categoriasCajaChica = (categoriasCajaRes?.data || []).map(c => ({
      ...
      // v8.27.48: mapeo a Odoo (para el CSV de facturas)
      odooProducto: c.odoo_producto || '', odooCuenta: c.odoo_cuenta || '', odooImpuesto: c.odoo_impuesto || '',
    }));
    ...
    const [
      ...
      _safe(listarVehiculos(), [], 'Vehículos'),
    ]);
...
export async function guardarPerfil(personaId, campos) {
  ...
  if (campos.facturasHabilitada !== undefined) updates.facturas_habilitada = !!campos.facturasHabilitada; // v8.27.36
  // v8.49.0: datos RR.HH. completos editables por el admin (antes solo en onboarding)
  if (campos.fechaNacimiento !== undefined) updates.fecha_nacimiento = campos.fechaNacimiento || null;
  if (campos.whatsapp !== undefined) updates.whatsapp = campos.whatsapp ? normalizarTelefono(campos.whatsapp) : null;
  if (campos.tipoSangre !== undefined) updates.tipo_sangre = campos.tipoSangre || null;
  if (campos.contactoEmergenciaNombre !== undefined) updates.contacto_emergencia_nombre = campos.contactoEmergenciaNombre || null;
  if (campos.contactoEmergenciaTelefono !== undefined) updates.contacto_emergencia_telefono = campos.contactoEmergenciaTelefono ? normalizarTelefono(campos.contactoEmergenciaTelefono) : null;
  if (campos.contactoEmergenciaRelacion !== undefined) updates.contacto_emergencia_relacion = campos.contactoEmergenciaRelacion || null;
  if (campos.puesto !== undefined) updates.puesto = campos.puesto || null;
  if (campos.departamento !== undefined) updates.departamento = campos.departamento || null;
  if (campos.banco !== undefined) updates.banco = campos.banco || null;
  if (campos.bancoTipoCuenta !== undefined) updates.banco_tipo_cuenta = campos.bancoTipoCuenta || null;
  if (campos.bancoNumeroCuenta !== undefined) updates.banco_numero_cuenta = campos.bancoNumeroCuenta || null;
  if (campos.bancoTitularNombre !== undefined) updates.banco_titular_nombre = campos.bancoTitularNombre || null;
  if (campos.bancoTitularCedula !== undefined) updates.banco_titular_cedula = campos.bancoTitularCedula || null;
...
export async function crearProyecto(proy) {
  ...
  const { error } = await supabase.from('proyectos').insert({
    id: proy.id, nombre: limpiarNombreProyecto(proy.nombre, proy.cliente), cliente: proy.cliente,
    referencia_odoo: proy.referenciaOdoo, referencia_proyecto: proy.referenciaProyecto ? limpiarNombreProyecto(proy.referenciaProyecto, proy.cliente) : proy.referenciaProyecto,
    ...
    // v8.27.73: cotizaciones en USD — el valor ya viene convertido a RD$; se guarda la
    // moneda de origen y la tasa usada para trazabilidad.
    moneda_origen: proy.monedaOrigen || null,
    tasa_usd: proy.tasaUsd != null ? Number(proy.tasaUsd) : null,
  ...
  // v8.31.1: proyecto nuevo en estado aprobado → chequeo de completitud + aviso
  // inmediato a Miguel/Erisdania si falta info (auto-llena ubicación del levantamiento).
  if ((proy.estado || 'aprobado') === 'aprobado') {
    notificarProyectoIncompleto(proy.id);
  }
}
...
export async function actualizarProyecto(proy) {
  ...
  const updates = {
    nombre: limpiarNombreProyecto(proy.nombre, proy.cliente), cliente: proy.cliente,
    referencia_odoo: proy.referenciaOdoo, referencia_proyecto: proy.referenciaProyecto ? limpiarNombreProyecto(proy.referenciaProyecto, proy.cliente) : proy.referenciaProyecto,
    sistema_id: proy.sistema, supervisor_id: supervisorId, maestro_id: maestroId,
    ayudantes_ids: ayudantesIds,
    fecha_inicio: proy.fecha_inicio, fecha_entrega: proy.fecha_entrega,
...
export async function crearReporte(r) {
  // v8.27.73: higiene del histórico de producción.
  // - Fecha FUTURA (typo) → se rechaza con mensaje claro.
  // - Reporte con >7 días de atraso → se marca retroactivo automático (cuenta para nómina,
  //   pero el dashboard de producción lo puede excluir del histórico limpio).
  const hoyLocal = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Santo_Domingo' }).format(new Date());
  if (r.fecha && r.fecha > hoyLocal) {
    throw new Error(`La fecha del reporte (${r.fecha}) está en el futuro. Verifica la fecha.`);
  }
  const retro = !!(r.fecha && (new Date(hoyLocal) - new Date(r.fecha)) / 86400000 > 7);
  const { error } = await supabase.from('reportes').insert({
    id: r.id, proyecto_id: r.proyectoId, area_id: r.areaId, tarea_id: r.tareaId,
    fecha: r.fecha,
    retroactivo: r.retroactivo != null ? !!r.retroactivo : retro,
    ...
    transcripcion: r.transcripcion || null,
    // v8.50.0 (ticket Miguel M.): reparación/retoque — se marca en datos_ia y queda
    // excluido de nómina desde el origen (el avance/producción lo filtran por el flag).
    datos_ia: { ...(r.datosIA || {}), ...(r.reparacion ? { reparacion: true } : {}) },
    ...(r.reparacion ? { excluir_nomina: true } : {}),
  });
  if (error) throw error;
...
// v8.27.78: guardar el contacto de REPORTES del cliente (correo/teléfono a quien se le
// envían los reportes de la obra) sin pasar por el update completo del proyecto.
export async function guardarContactoReportesProyecto(proyectoId, { email, telefono, nombre } = {}) {
  const u = { updated_at: new Date().toISOString() };
  if (email !== undefined) u.contacto_cliente_email = email || '';
  if (telefono !== undefined) u.contacto_cliente_telefono = telefono || '';
  if (nombre !== undefined) u.contacto_cliente_nombre = nombre || '';
  const { error } = await supabase.from('proyectos').update(u).eq('id', proyectoId);
  if (error) throw error;
}

// v8.42.3: medidor de consumo de IA — lecturas para el panel (owner).
export async function listarUsoIA({ desde } = {}) {
  let q = supabase.from('ai_usage')
    .select('created_at, funcion, usuario_nombre, modelo, input_tokens, output_tokens')
    .order('created_at', { ascending: false }).limit(5000);
  if (desde) q = q.gte('created_at', desde);
  const { data, error } = await q;
  if (error) { console.warn('listarUsoIA:', error.message); return []; }
  return data || [];
}

// v8.27.77 (ticket Miguel "m² duplicados entre quincenas"): marcar/desmarcar un reporte
// como EXCLUIDO de la nómina (trabajo ya pagado en un corte anterior que se re-reportó).
// v8.50.3: con CONSTANCIA — motivo, quién y cuándo (caso Despertar: pagado por adelantado).
export async function marcarExclusionNominaReporte(reporteId, excluir, { motivo, porId } = {}) {
  const { error } = await supabase.from('reportes')
    .update({
      excluir_nomina: !!excluir,
      excluir_nomina_motivo: excluir ? (motivo || null) : null,
      excluir_nomina_por_id: excluir ? (porId || null) : null,
      excluir_nomina_at: excluir ? new Date().toISOString() : null,
    }).eq('id', reporteId);
  if (error) throw error;
}
...
// ============================================================
export async function enviarCorreoReporte(destinatarios, asunto, html, opciones = {}) {
  try {
    const { cc } = opciones; // v8.27.40: soporte de copia (CC)
    await fetch('/api/enviar-reporte', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ destinatarios, asunto, html, ...(cc ? { cc } : {}) }),
    });
...
export async function reemplazarPersonal(nuevoPersonal) {
  ...
  const rows = nuevoPersonal.map(p => ({
    ...
    facturas_habilitada: !!p.facturasHabilitada, // v8.27.36
    ...
    ...(p.inspeccionesHabilitada !== undefined ? { inspecciones_habilitada: !!p.inspeccionesHabilitada } : {}),
    ...
    ...(p.activo !== undefined ? { activo: !!p.activo } : {}), // v8.49.0
  }));
...
export async function listarJornadasProyecto(proyectoId) {
  ...
  return (data || []).map(j => ({
    ...
    brigadasPresentes: j.brigadas_presentes || [], // v8.27.22
...
export async function listarJornadasEnRango(desde, hasta) {
  ...
  return (data || []).map(j => ({
    ...
    diaDoble: j.dia_doble || false,
    condicionDia: j.condicion_dia || 'normal',
    // v8.28.2: quién abrió/cerró — para el KPI "jornadas propias" del bono
    iniciadaPorId: j.iniciada_por_id || null,
    finalizadaPorId: j.finalizada_por_id || null,
  }));
}
...
export async function obtenerJornadaHoy(proyectoId, fecha) {
  ...
  return {
    ...
    brigadasPresentes: data.brigadas_presentes || [], // v8.27.22
...
// v8.27.65: trae la jornada ABIERTA (sin hora_fin) más reciente del proyecto, sin importar
// la fecha. Sirve para que un turno que cruza medianoche siga siendo finalizable a mano
// (antes la vista se ataba a fecha===hoy y "perdía" la jornada al cambiar el día).
export async function obtenerJornadaAbiertaProyecto(proyectoId) {
  const { data, error } = await supabase
    .from('jornadas')
    .select('*')
    .eq('proyecto_id', proyectoId)
  ...
  return {
    ...
    inicioLat: data.inicio_lat, inicioLng: data.inicio_lng, inicioPrecisionM: data.inicio_precision_m, inicioDistanciaObraM: data.inicio_distancia_obra_m,
    personasPresentesIds: data.personas_presentes_ids || [],
    brigadasPresentes: data.brigadas_presentes || [],
    horaFin: data.hora_fin, finalizadaPorId: data.finalizada_por_id, finalizadaPorNombre: data.finalizada_por_nombre,
    finLat: data.fin_lat, finLng: data.fin_lng, finPrecisionM: data.fin_precision_m, finDistanciaObraM: data.fin_distancia_obra_m,
    nota: data.nota,
    diaDoble: data.dia_doble || false,
...
export async function iniciarJornada(j) {
  const row = {
    ...
    brigadas_presentes: j.brigadasPresentes || [], // v8.27.22
    nota: j.nota || null,
  };
...
export async function actualizarPersonasJornada(jornadaId, personasIds, brigadasPresentes = null) {
  const patch = {
    personas_presentes_ids: personasIds,
    updated_at: new Date().toISOString(),
  };
  if (brigadasPresentes != null) patch.brigadas_presentes = brigadasPresentes; // v8.27.22
  const { error } = await supabase.from('jornadas').update(patch).eq('id', jornadaId);
  if (error) throw error;
}
...
export async function cambiarEstadoProyecto(proyectoId, estadoNuevo, usuario, nota, datosExtra = {}) {
  ...
  // v8.31.1: proyecto que llega a APROBADO → debe quedar COMPLETO el mismo día.
  // Auto-llena ubicación desde el levantamiento y, si aún falta info, notifica
  // de inmediato a Miguel y Erisdania (fire and forget).
  if (estadoNuevo === 'aprobado') {
    notificarProyectoIncompleto(proyectoId);
  }
}
...
export async function listarTareas({ completadas = false, asignadaA = null } = {}) {
  ...
  return (data || []).map(t => ({
    ...
    proyectoInternoId: t.proyecto_interno_id || null, // v8.34.0: espacios administrativos
    asignadaAId: t.asignada_a_id, asignadaANombre: t.asignada_a_nombre,
    supervisorId: t.supervisor_id || null, supervisorNombre: t.supervisor_nombre || null, // v8.33.0
    prioridad: t.prioridad || 'normal', comentarios: t.comentarios || [], // v8.33.2
    subtareas: t.subtareas || [], likes: t.likes || [], // v8.33.4
    horaLimite: t.hora_limite || null, fechaPlanificada: t.fecha_planificada || null, // v8.37.0
    historial: t.historial || [], // v8.37.0
...
export async function crearTarea(t) {
  const ctx = getAuditContext();
  // v8.37.0: historial de la tarea — quién la creó y a quién quedó asignada.
  const ahoraIso = new Date().toISOString();
  const quien = ctx.usuarioNombre || t.supervisorNombre || null;
  const historial = [{ tipo: 'creada', por: quien, at: ahoraIso }];
  if (t.asignadaANombre) historial.push({ tipo: 'asignada', a: t.asignadaANombre, por: quien, at: ahoraIso });
  const { error } = await supabase.from('tareas').insert({
    id: t.id, proyecto_id: t.proyectoId, tipo: t.tipo, titulo: t.titulo, descripcion: t.descripcion || null,
    proyecto_interno_id: t.proyectoInternoId || null, // v8.34.0

    asignada_a_id: t.asignadaAId || null, asignada_a_nombre: t.asignadaANombre || null,
    supervisor_id: t.supervisorId || null, supervisor_nombre: t.supervisorNombre || null, // v8.33.0
    prioridad: t.prioridad || 'normal', // v8.33.2
    fecha_limite: t.fechaLimite || null,
    hora_limite: t.horaLimite || null, fecha_planificada: t.fechaPlanificada || null, // v8.37.0
    historial,
  });
  if (error) throw error;
  registrarAudit({
...
// ============================================================
// ESPACIOS ADMINISTRATIVOS + TAREAS RECURRENTES (v8.34.0)
// Espacios = proyectos internos (cierre fiscal, traspaso, implementación…):
// agrupan tareas SIN tocar producción, bonos ni analíticas de obras.
// Recurrentes = obligaciones con fecha (impuestos, pagos, cierres) que se
// convierten solas en tareas N días antes de la fecha.
// ============================================================
const mapProyectoInterno = (p) => ({
  id: p.id, nombre: p.nombre, area: p.area, descripcion: p.descripcion,
  responsableId: p.responsable_id, responsableNombre: p.responsable_nombre,
  fechaMeta: p.fecha_meta, estado: p.estado || 'activo',
  creadoPorNombre: p.creado_por_nombre, createdAt: p.created_at,
});

export async function listarProyectosInternos({ incluirCerrados = false } = {}) {
  let q = supabase.from('proyectos_internos').select('*').order('created_at', { ascending: false });
  if (!incluirCerrados) q = q.eq('estado', 'activo');
  const { data, error } = await q;
  if (error) throw error;
  return (data || []).map(mapProyectoInterno);
}

export async function crearProyectoInterno(p) {
  const { error } = await supabase.from('proyectos_internos').insert({
    id: p.id, nombre: p.nombre, area: p.area || null, descripcion: p.descripcion || null,
    responsable_id: p.responsableId || null, responsable_nombre: p.responsableNombre || null,
    fecha_meta: p.fechaMeta || null,
    creado_por_id: p.creadoPorId || null, creado_por_nombre: p.creadoPorNombre || null,
  });
  if (error) throw error;
}

export async function actualizarProyectoInterno(id, cambios) {
  const u = {};
  if (cambios.nombre !== undefined) u.nombre = cambios.nombre;
  if (cambios.area !== undefined) u.area = cambios.area || null;
  if (cambios.descripcion !== undefined) u.descripcion = cambios.descripcion || null;
  if (cambios.responsableId !== undefined) { u.responsable_id = cambios.responsableId || null; u.responsable_nombre = cambios.responsableNombre || null; }
  if (cambios.fechaMeta !== undefined) u.fecha_meta = cambios.fechaMeta || null;
  if (cambios.estado !== undefined) u.estado = cambios.estado;
  const { error } = await supabase.from('proyectos_internos').update(u).eq('id', id);
  if (error) throw error;
}

export async function listarTareasRecurrentes() {
  const { mapRecurrente } = await import('./helpers/recurrentes');
  const { data, error } = await supabase.from('tareas_recurrentes').select('*').order('created_at', { ascending: false });
  if (error) throw error;
  return (data || []).map(mapRecurrente);
}

export async function crearTareaRecurrente(r) {
  const { error } = await supabase.from('tareas_recurrentes').insert({
    id: r.id, titulo: r.titulo, descripcion: r.descripcion || null, area: r.area || null,
    proyecto_interno_id: r.proyectoInternoId || null,
    responsable_id: r.responsableId || null, responsable_nombre: r.responsableNombre || null,
    supervisor_id: r.supervisorId || null, supervisor_nombre: r.supervisorNombre || null,
    prioridad: r.prioridad || 'normal', frecuencia: r.frecuencia,
    dia_mes: r.diaMes ?? null, dia_mes_2: r.diaMes2 ?? null, dia_semana: r.diaSemana ?? null,
    dias_aviso: r.diasAviso ?? 3,
    creado_por_id: r.creadoPorId || null, creado_por_nombre: r.creadoPorNombre || null,
  });
  if (error) throw error;
}

export async function actualizarTareaRecurrente(id, cambios) {
  const u = {};
  if (cambios.activo !== undefined) u.activo = cambios.activo;
  if (cambios.titulo !== undefined) u.titulo = cambios.titulo;
  if (cambios.responsableId !== undefined) { u.responsable_id = cambios.responsableId || null; u.responsable_nombre = cambios.responsableNombre || null; }
  if (cambios.diasAviso !== undefined) u.dias_aviso = cambios.diasAviso;
  const { error } = await supabase.from('tareas_recurrentes').update(u).eq('id', id);
  if (error) throw error;
}
...
// Genera las tareas que tocan hoy (también corre en el cron diario).
export async function generarTareasRecurrentes() {
  const { generarTareasRecurrentesCon } = await import('./helpers/recurrentes');
  const hoy = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Santo_Domingo' }).format(new Date());
  return generarTareasRecurrentesCon(supabase, hoy);
}
...
export async function listarTodosDetalles({ proyectoId = null, personaId = null } = {}) {
  // v8.50.5: sin tope de 1,000 (la nómina histórica crece cada quincena)
  const data = await fetchSinTope(() => {
    let q = supabase.from('detalle_nomina').select('*').order('created_at', { ascending: false });
    if (proyectoId) q = q.eq('proyecto_id', proyectoId);
    if (personaId) q = q.eq('persona_id', personaId);
    return q;
  });
...
export async function listarCostosDia(proyectoId) {
  ...
  if (error) throw error;
  return (data || []).map(c => ({
    id: c.id, proyectoId: c.proyecto_id, personaId: c.persona_id,
    costoDia: c.costo_dia != null ? Number(c.costo_dia) : null, // v8.50.3: puede nacer sin tarifa (solo modo)
...
// v8.45.0: actualizar SOLO las coordenadas de una locación (sin tocar el resto).
export async function actualizarCoordsUbicacionCliente(id, lat, lng) {
  const { error } = await supabase.from('cliente_ubicaciones').update({ latitud: Number(lat), longitud: Number(lng) }).eq('id', id);
  if (error) throw error;
}
...
function reclamacionRowToObj(r) {
  return {
    ...
    contactoId: r.contacto_id || null, // v8.53.4: contacto del cliente (tabla contactos)
    ...
    // v8.53.1 (Fase 3B): hito informe de solución entregado al cliente
    informeEntregadoAt: r.informe_entregado_at || null,
    informeUrl: r.informe_url || null,
    informeNombre: r.informe_nombre || null,
    ...
    fotos: Array.isArray(r.fotos) ? r.fotos : [], // v8.49.11: fotos del trabajo (ticket Edwin)
...
// v8.51.6: métricas ligeras para el KPI del Dashboard — levantamientos hechos
// (survey_status completed/validated) y reclamaciones atendidas (estado ≠ abierta),
// con la fecha del último cambio. Solo los últimos 40 días para computar hoy/semana/mes.
export async function metricasActividad() {
  const desde = new Date(Date.now() - 40 * 86400000).toISOString();
  let sites = [], recl = [];
  try {
    const { data } = await supabase.schema('surveys').from('sites')
      .select('survey_status, updated_at').gte('updated_at', desde);
    sites = data || [];
  } catch (e) { console.warn('metricasActividad sites:', e?.message); }
  try {
    const { data } = await supabase.from('reclamaciones')
      .select('estado, updated_at, fecha_resuelta').gte('updated_at', desde);
    recl = data || [];
  } catch (e) { console.warn('metricasActividad recl:', e?.message); }
  return { sites, reclamaciones: recl };
}
export async function crearReclamacion(r) {
  ...
  const { data, error } = await supabase.from('reclamaciones').insert({
    ...
    contacto_id: r.contactoId || null, // v8.53.4
...
// v8.49.11 (ticket Edwin): fotos del trabajo realizado en la reclamación.
export async function subirFotoReclamacion(blob, recId) {
  const path = `reclamaciones/${recId}/${Date.now()}.jpg`;
  const { error } = await supabase.storage.from('documentos-fotos').upload(path, blob, { contentType: 'image/jpeg', upsert: false });
  if (error) throw error;
  const { data } = supabase.storage.from('documentos-fotos').getPublicUrl(path);
  return data.publicUrl;
}
export async function guardarFotosReclamacion(id, fotos) {
  const { error } = await supabase.from('reclamaciones').update({ fotos }).eq('id', id);
  if (error) throw error;
}
...
export async function actualizarReclamacion(id, campos) {
  ...
  // v8.50.0 (ticket Edwin): la etapa Odoo también se actualiza — antes cambiar el estado
  // local NO movía la tarjeta del Kanban porque estadoOdoo() prioriza odoo_stage.
  if (campos.odooStage !== undefined) u.odoo_stage = campos.odooStage || null;
  ...
  if (campos.contactoId !== undefined) u.contacto_id = campos.contactoId || null; // v8.53.4
  // v8.53.1 (Fase 3B): hito informe de solución entregado al cliente (cierra el ciclo).
  if (campos.informeEntregadoAt !== undefined) u.informe_entregado_at = campos.informeEntregadoAt || null;
  if (campos.informeUrl !== undefined) u.informe_url = campos.informeUrl || null;
  if (campos.informeNombre !== undefined) u.informe_nombre = campos.informeNombre || null;
  const { error } = await supabase.from('reclamaciones').update(u).eq('id', id);
  if (error) throw error;
}

// v8.53.1 (Fase 3B): sube el PDF del informe de solución al Storage y devuelve su URL pública.
export async function subirInformeReclamacion(fileOrBlob, recId, nombre = null) {
  const ext = (nombre && nombre.includes('.')) ? nombre.split('.').pop().toLowerCase() : 'pdf';
  const path = `reclamaciones/${recId}/informe-${Date.now()}.${ext}`;
  const contentType = ext === 'pdf' ? 'application/pdf' : (fileOrBlob?.type || 'application/octet-stream');
  const { error } = await supabase.storage.from('documentos-fotos').upload(path, fileOrBlob, { contentType, upsert: false });
  if (error) throw error;
  const { data } = supabase.storage.from('documentos-fotos').getPublicUrl(path);
  return data.publicUrl;
}
...
// ============================================================
// v8.54.1 (C3): RETROALIMENTACIÓN DEL CLIENTE (CSAT) por ticket.
// ============================================================
function tokenCalificacion() {
  return (Date.now().toString(36) + Math.random().toString(36).slice(2, 10) + Math.random().toString(36).slice(2, 6)).replace(/[^a-z0-9]/g, '');
}
function calificacionRowToObj(r) {
  return {
    id: r.id, entityType: r.entity_type, entityId: r.entity_id, token: r.token,
    clienteNombre: r.cliente_nombre || '', contexto: r.contexto || '',
    calificacion: r.calificacion != null ? Number(r.calificacion) : null,
    comentario: r.comentario || '', respondidoAt: r.respondido_at || null,
    creadoPorNombre: r.creado_por_nombre || '', createdAt: r.created_at,
  };
}
// Crea (o reutiliza si ya hay una sin responder) la solicitud de calificación de un ticket.
export async function crearSolicitudCalificacion({ entityType, entityId, clienteNombre = '', contexto = '', creadoPor = null }) {
  // Reutiliza una pendiente para no acumular links del mismo ticket.
  const { data: prev } = await supabase.from('retroalimentacion_cliente')
    .select('*').eq('entity_type', entityType).eq('entity_id', String(entityId)).is('respondido_at', null)
    .order('created_at', { ascending: false }).limit(1);
  if (prev && prev[0]) return calificacionRowToObj(prev[0]);
  const row = {
    id: 'csat_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6),
    entity_type: entityType, entity_id: String(entityId), token: tokenCalificacion(),
    cliente_nombre: clienteNombre || null, contexto: contexto || null,
    creado_por_id: creadoPor?.id || null, creado_por_nombre: creadoPor?.nombre || null,
  };
  const { data, error } = await supabase.from('retroalimentacion_cliente').insert(row).select('*').single();
  if (error) throw error;
  return calificacionRowToObj(data);
}
export async function listarCalificacionesEntidad(entityType, entityId) {
  const { data } = await supabase.from('retroalimentacion_cliente')
    .select('*').eq('entity_type', entityType).eq('entity_id', String(entityId))
    .order('created_at', { ascending: false });
  return (data || []).map(calificacionRowToObj);
}
// Todas las calificaciones (para el CSAT de la Torre). Solo las respondidas importan al promedio.
export async function listarCalificaciones({ entityType = null } = {}) {
  let q = supabase.from('retroalimentacion_cliente').select('*').order('created_at', { ascending: false });
  if (entityType) q = q.eq('entity_type', entityType);
  const data = await fetchSinTope(() => q);
  return (data || []).map(calificacionRowToObj);
}
...
export async function listarMovimientosCajaChica({
  ...
  rnc = null,
  // v8.27.26: ordenar por 'fecha' (default, para reportes/cuadre/export por período) o
  // por 'created_at' (fecha de SUBIDA) en las vistas de navegación — así una factura
  // con la fecha mal leída por la IA (año viejo) no se hunde y aparece arriba al subirla.
  ordenarPor = 'fecha',
} = {}) {
  const colOrden = ordenarPor === 'created_at' ? 'created_at' : 'fecha';
  // v8.27.82: Supabase/PostgREST corta en 1,000 filas por defecto. La "Gestión
  // global" ya pasó de 1,000 movimientos, así que el histórico (KPIs, por proyecto,
  // etc.) salía CORTADO a los 1,000 más recientes. Paginamos en lotes de 1,000 para
  // traer TODO. Las llamadas con filtro (persona/proyecto) casi siempre caben en un
  // solo lote, así que no cambia su costo.
  const PAGE = 1000;
  const todos = [];
  for (let desde = 0; ; desde += PAGE) {
    let q = supabase.from('caja_chica_movimientos').select(SELECT_CAJA).order(colOrden, { ascending: false });
    if (personaId) q = q.eq('persona_id', personaId);
    if (proyectoId) q = q.eq('proyecto_id', proyectoId);
    ...
    const lote = data || [];
    todos.push(...lote);
    if (lote.length < PAGE) break; // último lote
  }
  return todos.map(cajaChicaRowToObj);
}
...
// ============================================================
// v8.27.36: FACTURAS ODOO — captura de facturas de gasto para exportar a Odoo.
// Lily sube foto → IA extrae → revisa → guarda. Admin exporta a Odoo (CSV + fotos).
// ============================================================
const BUCKET_FACTURAS = 'facturas-odoo';

export async function subirFotoFacturaOdoo({ blobOrDataUrl, id, fecha }) {
  let blob = blobOrDataUrl;
  if (typeof blobOrDataUrl === 'string') {
    blob = dataUrlToBlob(blobOrDataUrl);
    if (!blob) throw new Error('dataURL inválido');
  ...
  const [anio, mes] = (fecha || new Date().toISOString().split('T')[0]).split('-');
  const ext = extFromMime(blob.type);
  const path = `${anio}/${mes}/${id}.${ext}`;
  const { error } = await supabase.storage.from(BUCKET_FACTURAS).upload(path, blob, {
...
export async function obtenerUrlFirmadaFacturaOdoo(path, expiraSegundos = 3600) {
  if (!path) return null;
  const { data, error } = await supabase.storage.from(BUCKET_FACTURAS).createSignedUrl(path, expiraSegundos);
  if (error) throw error;
  return data?.signedUrl || null;
}

function mapFacturaOdooRow(r) {
  return {
    id: r.id, empresa: r.empresa, proveedor: r.proveedor, rnc: r.rnc,
    tipoNcf: r.tipo_ncf, ncf: r.ncf, fecha: r.fecha, concepto: r.concepto,
    monto: r.monto != null ? Number(r.monto) : 0,
    itbis: r.itbis != null ? Number(r.itbis) : null,
    itbisModo: r.itbis_modo || 'aparte',
    reembolsable: !!r.reembolsable,
    subtotal: r.subtotal != null ? Number(r.subtotal) : null,
    cuentaGasto: r.cuenta_gasto || null,
    cuentaAnalitica: r.cuenta_analitica || '', // v8.27.40
    moneda: r.moneda || 'DOP', // v8.27.42
    fotoPath: r.foto_path, datosIA: r.datos_ia || null,
    estado: r.estado || 'borrador', exportadaAt: r.exportada_at,
    odooMoveId: r.odoo_move_id || null, verificadaOdooAt: r.verificada_odoo_at || null, // v8.27.61
    fotoOdooAt: r.foto_odoo_at || null, // v8.27.62: foto ya adjuntada en Odoo
    creadoPorId: r.creado_por_id, creadoPorNombre: r.creado_por_nombre,
    createdAt: r.created_at,
  };
}

export async function listarFacturasOdoo({ estado = null } = {}) {
  let q = supabase.from('facturas_odoo').select('*').order('created_at', { ascending: false });
  if (estado) q = q.eq('estado', estado);
  const { data, error } = await q;
  if (error) { console.warn('listarFacturasOdoo:', error.message); return []; }
  return (data || []).map(mapFacturaOdooRow);
}

export async function crearFacturaOdoo(f) {
  const id = f.id || ('fac_' + Date.now() + Math.random().toString(36).slice(2, 7));
  let fotoPath = f.fotoPath || null;
  if (f.fotoDataUrl) fotoPath = await subirFotoFacturaOdoo({ blobOrDataUrl: f.fotoDataUrl, id, fecha: f.fecha });
  const row = {
    id, empresa: f.empresa || null, proveedor: f.proveedor || null, rnc: f.rnc || null,
    tipo_ncf: f.tipoNcf || null, ncf: f.ncf || null, fecha: f.fecha || null,
    concepto: f.concepto || null, monto: Number(f.monto) || 0,
    itbis: f.itbis != null && f.itbis !== '' ? Number(f.itbis) : null,
    itbis_modo: f.itbisModo || 'aparte',
    reembolsable: !!f.reembolsable,
    subtotal: f.subtotal != null && f.subtotal !== '' ? Number(f.subtotal) : null,
    cuenta_gasto: f.cuentaGasto || null,
    cuenta_analitica: f.cuentaAnalitica || null, // v8.27.40: número azul = proyecto para Odoo
    moneda: f.moneda || 'DOP', // v8.27.42: DOP | USD | EUR (Odoo la necesita)
    foto_path: fotoPath, datos_ia: f.datosIA || null,
    estado: f.estado || 'borrador',
    creado_por_id: f.creadoPorId || null, creado_por_nombre: f.creadoPorNombre || null,
  };
  const { data, error } = await supabase.from('facturas_odoo').insert(row).select('*').single();
  if (error) throw error;
  return mapFacturaOdooRow(data);
}

export async function actualizarFacturaOdoo(id, campos) {
  const u = { updated_at: new Date().toISOString() };
  const map = {
    empresa: 'empresa', proveedor: 'proveedor', rnc: 'rnc', tipoNcf: 'tipo_ncf',
    ncf: 'ncf', fecha: 'fecha', concepto: 'concepto', monto: 'monto', itbis: 'itbis',
    itbisModo: 'itbis_modo', reembolsable: 'reembolsable', subtotal: 'subtotal', cuentaGasto: 'cuenta_gasto',
    cuentaAnalitica: 'cuenta_analitica', // v8.27.40
    moneda: 'moneda', // v8.27.42
    estado: 'estado', datosIA: 'datos_ia',
    odooMoveId: 'odoo_move_id', verificadaOdooAt: 'verificada_odoo_at', // v8.27.61
    fotoOdooAt: 'foto_odoo_at', // v8.27.62
  };
  for (const [k, col] of Object.entries(map)) if (campos[k] !== undefined) u[col] = campos[k];
  if (campos.fotoDataUrl) u.foto_path = await subirFotoFacturaOdoo({ blobOrDataUrl: campos.fotoDataUrl, id, fecha: campos.fecha });
  const { data, error } = await supabase.from('facturas_odoo').update(u).eq('id', id).select('*').single();
  if (error) throw error;
  return mapFacturaOdooRow(data);
}

export async function eliminarFacturaOdoo(id) {
  const { data } = await supabase.from('facturas_odoo').select('foto_path').eq('id', id).single();
  if (data?.foto_path) await supabase.storage.from(BUCKET_FACTURAS).remove([data.foto_path]).catch(() => {});
  const { error } = await supabase.from('facturas_odoo').delete().eq('id', id);
  if (error) throw error;
}

export async function marcarFacturasOdooExportadas(ids) {
  if (!ids || !ids.length) return;
  const { error } = await supabase.from('facturas_odoo')
    .update({ estado: 'exportada', exportada_at: new Date().toISOString() }).in('id', ids);
  if (error) throw error;
}

// ============================================================
// v8.27.38: VEHÍCULOS (flota) — datos + docs (matrícula, seguro).
// ============================================================
const BUCKET_VEHICULOS = 'vehiculos-docs';

const vehiculoRowToObj = (r) => ({
  id: r.id, marca: r.marca, modelo: r.modelo,
  anio: r.anio != null ? Number(r.anio) : null,
  color: r.color, chasis: r.chasis, placa: r.placa,
  matriculaPath: r.matricula_path, seguroPath: r.seguro_path,
  seguroAseguradora: r.seguro_aseguradora, seguroVence: r.seguro_vence,
  empresa: r.empresa, notas: r.notas, activo: r.activo !== false,
  // v8.33.0: ficha completa + responsable
  responsableId: r.responsable_id || null, tipo: r.tipo || '',
  capacidadCargaKg: r.capacidad_carga_kg != null ? Number(r.capacidad_carga_kg) : null,
  combustible: r.combustible || '', odometroKm: r.odometro_km != null ? Number(r.odometro_km) : null,
  odometroFecha: r.odometro_fecha || null, matriculaVence: r.matricula_vence || null,
  revisionVence: r.revision_vence || null, tagPeaje: r.tag_peaje || '',
  estadoOperativo: r.estado_operativo || 'activo',
  esParticular: r.es_particular === true, // v8.50.1: carro personal de un empleado
  proximoMantKm: r.proximo_mant_km != null ? Number(r.proximo_mant_km) : null,
  proximoMantFecha: r.proximo_mant_fecha || null,
  gpsUrl: r.gps_url || '', // v8.27.85: enlace de rastreo (Pressto)
  gpsDeviceId: r.gps_device_id ?? null, // v8.43.0: unidad GPS (posición en vivo)
  garantiaVence: r.garantia_vence || null, garantiaKm: r.garantia_km ?? null, // v8.44.0
  garantiaCasa: r.garantia_casa || '', garantiaNotas: r.garantia_notas || '',
  createdAt: r.created_at, updatedAt: r.updated_at,
});

export async function listarVehiculos({ soloActivos = false } = {}) {
  try {
    let q = supabase.from('vehiculos').select('*').order('marca');
    if (soloActivos) q = q.eq('activo', true);
    const { data, error } = await q;
    if (error) throw error;
    return (data || []).map(vehiculoRowToObj);
  } catch (e) { console.warn('listarVehiculos fallback:', e?.message); return []; }
}

// Sube un documento (matrícula/seguro) del vehículo. file = File/Blob (PDF o imagen).
export async function subirDocVehiculo({ file, vehiculoId, tipo }) {
  if (!(file instanceof Blob)) throw new Error('file debe ser File o Blob');
  const ext = extFromMime(file.type) || 'bin';
  const path = `${vehiculoId}/${tipo}_${Date.now()}.${ext}`;
  const { error } = await supabase.storage.from(BUCKET_VEHICULOS).upload(path, file, {
...
export async function obtenerUrlDocVehiculo(path, expiraSegundos = 3600) {
  if (!path) return null;
  const { data, error } = await supabase.storage.from(BUCKET_VEHICULOS).createSignedUrl(path, expiraSegundos);
  if (error) throw error;
  return data?.signedUrl || null;
}

export async function crearVehiculo(v) {
  const id = v.id || ('veh_' + Date.now() + Math.random().toString(36).slice(2, 6));
  const row = {
    id, marca: v.marca || null, modelo: v.modelo || null,
    anio: v.anio ? Number(v.anio) : null, color: v.color || null,
    chasis: v.chasis || null, placa: v.placa || null,
    matricula_path: v.matriculaPath || null, seguro_path: v.seguroPath || null,
    seguro_aseguradora: v.seguroAseguradora || null, seguro_vence: v.seguroVence || null,
    empresa: v.empresa || null, notas: v.notas || null, activo: v.activo !== false,
    // v8.33.0
    responsable_id: v.responsableId || null, tipo: v.tipo || null,
    capacidad_carga_kg: v.capacidadCargaKg ? Number(v.capacidadCargaKg) : null,
    combustible: v.combustible || null,
    odometro_km: v.odometroKm ? Number(v.odometroKm) : null,
    odometro_fecha: v.odometroFecha || null,
    matricula_vence: v.matriculaVence || null, revision_vence: v.revisionVence || null,
    tag_peaje: v.tagPeaje || null, estado_operativo: v.estadoOperativo || 'activo',
    proximo_mant_km: v.proximoMantKm ? Number(v.proximoMantKm) : null,
    proximo_mant_fecha: v.proximoMantFecha || null,
    gps_url: v.gpsUrl || null,
    gps_device_id: v.gpsDeviceId ? Number(v.gpsDeviceId) : null, // v8.43.0
    garantia_vence: v.garantiaVence || null, garantia_km: v.garantiaKm ? Number(v.garantiaKm) : null, // v8.44.0
    garantia_casa: v.garantiaCasa || null, garantia_notas: v.garantiaNotas || null,
    es_particular: v.esParticular === true, // v8.50.1
  };
  const { data, error } = await supabase.from('vehiculos').insert(row).select('*').single();
  if (error) throw error;
  return vehiculoRowToObj(data);
}

export async function actualizarVehiculo(id, campos) {
  const u = { updated_at: new Date().toISOString() };
  const map = {
    marca: 'marca', modelo: 'modelo', anio: 'anio', color: 'color', chasis: 'chasis',
    placa: 'placa', matriculaPath: 'matricula_path', seguroPath: 'seguro_path',
    seguroAseguradora: 'seguro_aseguradora', seguroVence: 'seguro_vence',
    empresa: 'empresa', notas: 'notas', activo: 'activo',
    // v8.33.0
    responsableId: 'responsable_id', tipo: 'tipo', capacidadCargaKg: 'capacidad_carga_kg',
    combustible: 'combustible', odometroKm: 'odometro_km', odometroFecha: 'odometro_fecha',
    matriculaVence: 'matricula_vence', revisionVence: 'revision_vence', tagPeaje: 'tag_peaje',
    estadoOperativo: 'estado_operativo', proximoMantKm: 'proximo_mant_km', proximoMantFecha: 'proximo_mant_fecha',
    esParticular: 'es_particular', // v8.50.1
    gpsUrl: 'gps_url', gpsDeviceId: 'gps_device_id', // v8.43.0
    garantiaVence: 'garantia_vence', garantiaKm: 'garantia_km', garantiaCasa: 'garantia_casa', garantiaNotas: 'garantia_notas', // v8.44.0
  };
  for (const [k, col] of Object.entries(map)) {
    if (campos[k] === undefined) continue;
    if (k === 'anio') u[col] = campos[k] ? Number(campos[k]) : null;
    else if (k === 'seguroVence') u[col] = campos[k] || null;
    else if (k === 'activo') u[col] = !!campos[k];
    else u[col] = campos[k] || null;
  }
  const { data, error } = await supabase.from('vehiculos').update(u).eq('id', id).select('*').single();
  if (error) throw error;
  return vehiculoRowToObj(data);
}

export async function eliminarVehiculo(id) {
  const { data } = await supabase.from('vehiculos').select('matricula_path, seguro_path').eq('id', id).single();
  const paths = [data?.matricula_path, data?.seguro_path].filter(Boolean);
  if (paths.length) await supabase.storage.from(BUCKET_VEHICULOS).remove(paths).catch(() => {});
  const { error } = await supabase.from('vehiculos').delete().eq('id', id);
  if (error) throw error;
}

// ============================================================
// v8.35.2: Licencia de conducir del chofer (persona). Vive en la persona
// (personal.licencia_*) y se muestra desde la ficha del vehículo asignado.
// ============================================================
export async function subirDocLicencia({ file, personaId }) {
  if (!(file instanceof Blob)) throw new Error('file debe ser File o Blob');
  const ext = extFromMime(file.type) || 'bin';
  const path = `licencias/${personaId}/lic_${Date.now()}.${ext}`;
  const { error } = await supabase.storage.from('personal-fotos').upload(path, file, {
    contentType: file.type || 'application/octet-stream', upsert: true,
  });
  if (error) throw error;
  return path;
...
export async function obtenerUrlDocLicencia(path, expiraSegundos = 3600) {
  if (!path) return null;
  const { data, error } = await supabase.storage.from('personal-fotos').createSignedUrl(path, expiraSegundos);
  if (error) throw error;
  return data?.signedUrl || null;
}

export async function guardarLicenciaPersona(personaId, { licenciaPath, licenciaCategoria, licenciaVence } = {}) {
  if (!personaId) return null;
  const u = {};
  if (licenciaPath !== undefined) u.licencia_path = licenciaPath || null;
  if (licenciaCategoria !== undefined) u.licencia_categoria = licenciaCategoria || null;
  if (licenciaVence !== undefined) u.licencia_vence = licenciaVence || null;
  if (Object.keys(u).length === 0) return null;
  const { data, error } = await supabase.from('personal').update(u).eq('id', personaId)
    .select('id, licencia_path, licencia_categoria, licencia_vence').single();
  if (error) throw error;
  return { id: data.id, licenciaPath: data.licencia_path || '', licenciaCategoria: data.licencia_categoria || '', licenciaVence: data.licencia_vence || null };
}

// Trae la licencia de varias personas (para mostrar en las tarjetas de vehículos).
export async function obtenerLicenciasDePersonas(ids = []) {
  const limpio = [...new Set((ids || []).filter(Boolean))];
  if (limpio.length === 0) return {};
  const { data, error } = await supabase.from('personal')
    .select('id, nombre, licencia_path, licencia_categoria, licencia_vence').in('id', limpio);
  if (error) { console.warn('obtenerLicenciasDePersonas:', error.message); return {}; }
  const m = {};
  for (const r of (data || [])) m[r.id] = {
    nombre: r.nombre, licenciaPath: r.licencia_path || '',
    licenciaCategoria: r.licencia_categoria || '', licenciaVence: r.licencia_vence || null,
  };
  return m;
}
...
// ============================================================
export const ANGULOS_INSPECCION = [
  { k: 'alante', label: 'Alante (frente)' },
  { k: 'lados', label: 'Lados' },
  { k: 'atras', label: 'Atrás' },
  { k: 'bonete', label: 'Bonete abierto' },
  { k: 'odometro', label: 'Odómetro' },
  { k: 'interior', label: 'Interior' },
];

export async function crearInspeccion({ vehiculoId, fecha, realizadaPorId, realizadaPorNombre, tipo = 'oficina', autorizadaPorId = null, autorizadaPorNombre = null, odometroKm = null, estadoGeneral = null, notas = null, checklist = null }) {
  const id = 'insp_' + Date.now() + Math.random().toString(36).slice(2, 6);
  const row = {
    id, vehiculo_id: vehiculoId, fecha: fecha || new Date().toISOString().slice(0, 10),
    realizada_por_id: realizadaPorId || null, realizada_por_nombre: realizadaPorNombre || null,
    tipo, autorizada_por_id: autorizadaPorId, autorizada_por_nombre: autorizadaPorNombre,
    odometro_km: odometroKm != null && odometroKm !== '' ? Number(odometroKm) : null,
    estado_general: estadoGeneral || null, notas: notas || null,
    ...(checklist ? { checklist } : {}), // v8.51.0: niveles y estado (prevención)
  };
  const { data, error } = await supabase.from('vehiculo_inspecciones').insert(row).select('*').single();
  if (error) throw error;
  return data;
}

export async function subirFotoInspeccion({ file, inspeccionId, angulo }) {
  if (!(file instanceof Blob)) throw new Error('file debe ser File o Blob');
  const ext = extFromMime(file.type) || 'jpg';
  const path = `inspecciones/${inspeccionId}/${angulo}_${Date.now()}.${ext}`;
  const { error } = await supabase.storage.from(BUCKET_VEHICULOS).upload(path, file, {
    contentType: file.type || 'image/jpeg', upsert: true,
  });
  if (error) throw error;
  const { error: dbErr } = await supabase.from('vehiculo_inspeccion_fotos').insert({
    id: 'if_' + Date.now() + Math.random().toString(36).slice(2, 6),
    inspeccion_id: inspeccionId, angulo, storage_path: path,
  });
  if (dbErr) throw dbErr;
  return path;
}

export async function obtenerUrlFotoInspeccion(path, expiraSegundos = 3600) {
  if (!path) return null;
  const { data, error } = await supabase.storage.from(BUCKET_VEHICULOS).createSignedUrl(path, expiraSegundos);
  if (error) throw error;
  return data?.signedUrl || null;
...
export async function listarInspecciones(vehiculoId) {
  const { data, error } = await supabase.from('vehiculo_inspecciones')
    .select('*').eq('vehiculo_id', vehiculoId).order('fecha', { ascending: false }).order('created_at', { ascending: false });
  if (error) throw error;
  const insps = data || [];
  if (insps.length === 0) return [];
  const { data: fotos } = await supabase.from('vehiculo_inspeccion_fotos').select('*').in('inspeccion_id', insps.map(i => i.id));
  const porInsp = {};
  for (const f of (fotos || [])) (porInsp[f.inspeccion_id] = porInsp[f.inspeccion_id] || []).push({ angulo: f.angulo, path: f.storage_path });
  return insps.map(i => ({
    id: i.id, vehiculoId: i.vehiculo_id, fecha: i.fecha,
    realizadaPorNombre: i.realizada_por_nombre, tipo: i.tipo, autorizadaPorNombre: i.autorizada_por_nombre,
    odometroKm: i.odometro_km != null ? Number(i.odometro_km) : null,
    estadoGeneral: i.estado_general, notas: i.notas, createdAt: i.created_at,
    fotos: porInsp[i.id] || [],
  }));
}

export async function autorizarInspeccionRemota({ vehiculoId, autorizadoAId, autorizadoANombre, autorizadaPorId, autorizadaPorNombre, vence = null }) {
  const id = 'ia_' + Date.now() + Math.random().toString(36).slice(2, 6);
  await supabase.from('vehiculo_inspeccion_autorizaciones').update({ activa: false }).eq('vehiculo_id', vehiculoId).eq('activa', true);
  const { data, error } = await supabase.from('vehiculo_inspeccion_autorizaciones').insert({
    id, vehiculo_id: vehiculoId, autorizado_a_id: autorizadoAId || null, autorizado_a_nombre: autorizadoANombre || null,
    autorizada_por_id: autorizadaPorId || null, autorizada_por_nombre: autorizadaPorNombre || null, vence,
  }).select('*').single();
  if (error) throw error;
  return data;
}

export async function listarAutorizacionesInspeccion({ vehiculoId = null } = {}) {
  let q = supabase.from('vehiculo_inspeccion_autorizaciones').select('*').eq('activa', true).is('usada_inspeccion_id', null);
  if (vehiculoId) q = q.eq('vehiculo_id', vehiculoId);
  const { data, error } = await q;
  if (error) { console.warn('listarAutorizacionesInspeccion:', error.message); return []; }
  return data || [];
}

export async function marcarAutorizacionUsada(autorizacionId, inspeccionId) {
  await supabase.from('vehiculo_inspeccion_autorizaciones').update({ usada_inspeccion_id: inspeccionId, activa: false }).eq('id', autorizacionId);
}

export async function revocarAutorizacionInspeccion(id) {
  await supabase.from('vehiculo_inspeccion_autorizaciones').update({ activa: false }).eq('id', id);
}
...
export async function crearCategoriaCajaChica({ nombre, color, icono, descripcion, orden, aplicaA = null, odooProducto = null, odooCuenta = null, odooImpuesto = null }) {
  ...
  const row = {
    ...
    odoo_producto: odooProducto || null, odoo_cuenta: odooCuenta || null, odoo_impuesto: odooImpuesto || null, // v8.27.48
  };
...
export async function actualizarCategoriaCajaChica(id, campos) {
  ...
  if (campos.odooProducto !== undefined) updates.odoo_producto = campos.odooProducto || null; // v8.27.48
  if (campos.odooCuenta !== undefined) updates.odoo_cuenta = campos.odooCuenta || null;
  if (campos.odooImpuesto !== undefined) updates.odoo_impuesto = campos.odooImpuesto || null;
  if (Object.keys(updates).length === 0) return;
...
export async function completarOnboarding(personaId, datos) {
  ...
  if (datos.tipoDocumento != null) updates.tipo_documento = datos.tipoDocumento; // v8.27.25
...
// ============================================================
// CONTABILIDAD — Flujo de Caja (v8.28.0): compromisos fijos + celdas manuales
// ============================================================

const compromisoRowToObj = (r) => ({
  id: r.id,
  empresa: r.empresa,
  concepto: r.concepto,
...
export async function listarCompromisosFijos() {
  const { data, error } = await supabase.from('cont_compromisos_fijos').select('*')
    .order('frecuencia', { ascending: false }).order('dia_mes', { ascending: true, nullsFirst: true });
  if (error) throw error;
  return (data || []).map(compromisoRowToObj);
}

export async function guardarCompromisoFijo(c) {
  if (!c?.empresa || !c?.concepto) throw new Error('guardarCompromisoFijo: faltan empresa o concepto');
  const row = {
    empresa: c.empresa,
    concepto: c.concepto,
    ...
    dia_mes: c.frecuencia === 'semanal' ? null : (c.diaMes != null ? Number(c.diaMes) : null),
    activo: c.activo !== false,
    notas: c.notas || null,
  };
  const q = c.id
    ? supabase.from('cont_compromisos_fijos').update(row).eq('id', c.id)
    : supabase.from('cont_compromisos_fijos').insert(row);
  const { data, error } = await q.select('*').single();
  if (error) throw error;
  return compromisoRowToObj(data);
}

export async function eliminarCompromisoFijo(id) {
  const { error } = await supabase.from('cont_compromisos_fijos').delete().eq('id', id);
  if (error) throw error;
}

const flujoManualRowToObj = (r) => ({
  id: r.id,
  empresa: r.empresa,
  tipo: r.tipo,               // saldo_inicial | cobro | abono_vencido
  semana: r.semana || null,   // lunes de la semana (yyyy-mm-dd); null en saldo_inicial
  monto: Number(r.monto || 0),
...
export async function listarFlujoManual() {
  const { data, error } = await supabase.from('cont_flujo_manual').select('*');
  if (error) throw error;
  return (data || []).map(flujoManualRowToObj);
}

// Upsert manual (select→update/insert) para no depender de ON CONFLICT sobre
// el índice con COALESCE(semana, ...).
export async function guardarFlujoManual({ empresa, tipo, semana = null, monto = 0, montoUsd = 0, nota = null }) {
  if (!empresa || !tipo) throw new Error('guardarFlujoManual: faltan empresa o tipo');
  let q = supabase.from('cont_flujo_manual').select('id').eq('empresa', empresa).eq('tipo', tipo);
  q = semana ? q.eq('semana', semana) : q.is('semana', null);
  const { data: prev, error: e1 } = await q.limit(1);
  if (e1) throw e1;
  const row = { monto: Number(monto || 0), monto_usd: Number(montoUsd || 0), nota, actualizado_en: new Date().toISOString() };
  if (prev?.length) {
    const { data, error } = await supabase.from('cont_flujo_manual').update(row).eq('id', prev[0].id).select('*').single();
    if (error) throw error;
    return flujoManualRowToObj(data);
  }
  const { data, error } = await supabase.from('cont_flujo_manual')
    .insert({ empresa, tipo, semana, ...row }).select('*').single();
  if (error) throw error;
  return flujoManualRowToObj(data);
}

const flujoEntradaRowToObj = (r) => ({
  id: r.id,
  empresa: r.empresa,
  concepto: r.concepto,
  categoria: r.categoria || 'otro',   // avance | cubicacion | cobro | otro
  monto: Number(r.monto || 0),
  moneda: r.moneda || 'DOP',
  fechaEsperada: r.fecha_esperada,
  cobrado: r.cobrado === true,
  notas: r.notas || null,
});

export async function listarFlujoEntradas() {
  const { data, error } = await supabase.from('cont_flujo_entradas').select('*')
    .order('fecha_esperada', { ascending: true });
  if (error) throw error;
  return (data || []).map(flujoEntradaRowToObj);
}

export async function guardarFlujoEntrada(e) {
  if (!e?.empresa || !e?.concepto || !e?.fechaEsperada) throw new Error('guardarFlujoEntrada: faltan empresa, concepto o fecha esperada');
  const row = {
    empresa: e.empresa,
    concepto: e.concepto,
    categoria: ['avance', 'cubicacion', 'cobro'].includes(e.categoria) ? e.categoria : 'otro',
    monto: Number(e.monto || 0),
    moneda: e.moneda === 'USD' ? 'USD' : 'DOP',
    fecha_esperada: e.fechaEsperada,
    cobrado: e.cobrado === true,
    notas: e.notas || null,
  };
  const q = e.id
    ? supabase.from('cont_flujo_entradas').update(row).eq('id', e.id)
    : supabase.from('cont_flujo_entradas').insert(row);
  const { data, error } = await q.select('*').single();
  if (error) throw error;
  return flujoEntradaRowToObj(data);
}

export async function eliminarFlujoEntrada(id) {
  const { error } = await supabase.from('cont_flujo_entradas').delete().eq('id', id);
  if (error) throw error;
}
...
// ============================================================
// v8.28.2: Bonos trimestrales por KPIs ("Mi bono")
// ============================================================
function bonoConfigRowToObj(r) {
  return {
    id: r.id, personaId: r.persona_id, rolBono: r.rol_bono || 'supervisor',
    montoObjetivoRd: Number(r.monto_objetivo_rd) || 0,
    metaProduccionRd: Number(r.meta_produccion_rd) || 0,
    kpiOverrides: r.kpi_overrides || {},
    activo: r.activo !== false, notas: r.notas || '',
  };
}
export async function listarBonosConfig() {
  const { data, error } = await supabase.from('bonos_config').select('*');
  if (error) throw error;
  return (data || []).map(bonoConfigRowToObj);
}
export async function guardarBonoConfig(cfg) {
  const row = {
    persona_id: cfg.personaId,
    rol_bono: cfg.rolBono || 'supervisor',
    monto_objetivo_rd: Number(cfg.montoObjetivoRd) || 0,
    meta_produccion_rd: Number(cfg.metaProduccionRd) || 0,
    kpi_overrides: cfg.kpiOverrides || {},
    activo: cfg.activo !== false,
    notas: cfg.notas || null,
    updated_at: new Date().toISOString(),
  };
  const { error } = await supabase.from('bonos_config').upsert(row, { onConflict: 'persona_id' });
  if (error) throw error;
}

// ============================================================
// v8.29.0: Logística — requisiciones de materiales + viajes/rutas
// ============================================================
function requisicionRowToObj(r) {
  return {
    id: r.id, proyectoId: r.proyecto_id,
    solicitadoPorId: r.solicitado_por_id, solicitadoPorNombre: r.solicitado_por_nombre,
    estado: r.estado, urgente: !!r.urgente, notas: r.notas || '',
    esCompra: !!r.es_compra, cotizacionUrl: r.cotizacion_url || null, // v8.39.0
    ocOdooId: r.oc_odoo_id || null, ocOdooName: r.oc_odoo_name || null,
    // v8.48.0: alistos — despacho/venta + modo de entrega + sign-off de retiro
    tipo: r.tipo || 'obra', modoEntrega: r.modo_entrega || 'envio',
    clienteNombre: r.cliente_nombre || '', referencia: r.referencia || '',
    retiroPorNombre: r.retiro_por_nombre || '', retiroCedula: r.retiro_cedula || '',
    retiroFirmaUrl: r.retiro_firma_url || null, retiroFotoUrl: r.retiro_foto_url || null,
    retiradaAt: r.retirada_at || null,
    listaAt: r.lista_at, entregadaAt: r.entregada_at, createdAt: r.created_at,
    // v8.49.4: doble confirmación de recepción (obra u oficina, corroborada con la firma del chofer)
    recepcionConfirmadaAt: r.recepcion_confirmada_at || null,
    recepcionConfirmadaPorNombre: r.recepcion_confirmada_por_nombre || null,
    recepcionOrigen: r.recepcion_origen || null,
    odooPickingId: r.odoo_picking_id || null, odooPickingName: r.odoo_picking_name || null, // v8.49.5
    items: (r.requisicion_items || []).map(i => ({
      id: i.id, descripcion: i.descripcion, cantidad: i.cantidad, unidad: i.unidad || '', despachado: !!i.despachado,
      estadoCompra: i.estado_compra || null, // v8.39.0
    })).sort((a, b) => (a.id > b.id ? 1 : -1)),
  };
}
export async function listarRequisiciones({ proyectoId = null, estados = null } = {}) {
  // v8.50.5: sin tope de 1,000 (crece a diario con pedidos y despachos)
  const data = await fetchSinTope(() => {
    let q = supabase.from('requisiciones').select('*, requisicion_items(*)').order('created_at', { ascending: false });
    if (proyectoId) q = q.eq('proyecto_id', proyectoId);
    if (estados && estados.length) q = q.in('estado', estados);
    return q;
  });
  return (data || []).map(requisicionRowToObj);
}
export async function crearRequisicion({ proyectoId = null, solicitadoPorId, solicitadoPorNombre, urgente, notas, items, esCompra = false, cotizacionUrl = null, tipo = 'obra', modoEntrega = 'envio', clienteNombre = null, referencia = null, odooPickingId = null, odooPickingName = null, estadoCompraInicial = null }) {
  const id = 'req_' + Date.now() + Math.random().toString(36).slice(2, 6);
  const { error } = await supabase.from('requisiciones').insert({
    id, proyecto_id: proyectoId || null, solicitado_por_id: solicitadoPorId || null,
    solicitado_por_nombre: solicitadoPorNombre || null, urgente: !!urgente, notas: notas || null,
    es_compra: !!esCompra, cotizacion_url: cotizacionUrl || null, // v8.39.0
    // v8.48.0: alistos
    tipo: tipo || 'obra', modo_entrega: modoEntrega || 'envio',
    cliente_nombre: clienteNombre || null, referencia: referencia || null,
    odoo_picking_id: odooPickingId || null, odoo_picking_name: odooPickingName || null, // v8.49.5
  });
  if (error) throw error;
  const rows = (items || []).filter(i => (i.descripcion || '').trim()).map((i, n) => ({
    id: id + '_i' + String(n).padStart(2, '0'),
    requisicion_id: id, descripcion: i.descripcion.trim(),
    cantidad: i.cantidad === '' || i.cantidad == null ? null : Number(i.cantidad),
    unidad: i.unidad || null,
    estado_compra: estadoCompraInicial || null, // v8.50.0: solicitud de compra nace 'solicitado'
  }));
  if (rows.length) {
    const { error: e2 } = await supabase.from('requisicion_items').insert(rows);
    if (e2) throw e2;
  }
  return id;
}
// v8.49.4: confirmar la RECEPCIÓN de una requisición entregada (maestro en obra u oficina),
// siempre corroborada contra la prueba del chofer (foto + firma).
export async function confirmarRecepcionRequisicion(id, { porId = null, porNombre = null, origen = 'obra' } = {}) {
  const { error } = await supabase.from('requisiciones').update({
    recepcion_confirmada_at: new Date().toISOString(),
    recepcion_confirmada_por_id: porId, recepcion_confirmada_por_nombre: porNombre,
    recepcion_origen: origen,
  }).eq('id', id);
  if (error) throw error;
}
// v8.49.4: prueba de entrega del chofer para una requisición (foto + firma + quién recibió).
export async function obtenerPruebaEntrega(requisicionId) {
  const { data, error } = await supabase.from('viaje_paradas')
    .select('entrega_foto_url, entrega_firma_url, recibido_por_nombre, completada_at, viajes(chofer_nombre)')
    .eq('requisicion_id', requisicionId).eq('estado', 'completada')
    .order('completada_at', { ascending: false }).limit(1);
  if (error) throw error;
  const p = data?.[0];
  if (!p) return null;
  return {
    fotoUrl: p.entrega_foto_url || null, firmaUrl: p.entrega_firma_url || null,
    recibidoPorNombre: p.recibido_por_nombre || null, completadaAt: p.completada_at || null,
    choferNombre: p.viajes?.chofer_nombre || null,
  };
}
export async function actualizarRequisicion(id, campos) {
  const u = { updated_at: new Date().toISOString() };
  if (campos.estado !== undefined) {
    u.estado = campos.estado;
    if (campos.estado === 'lista') u.lista_at = new Date().toISOString();
    if (campos.estado === 'entregada') u.entregada_at = new Date().toISOString();
  }
  if (campos.notas !== undefined) u.notas = campos.notas || null;
  if (campos.urgente !== undefined) u.urgente = !!campos.urgente;
  if (campos.cotizacionUrl !== undefined) u.cotizacion_url = campos.cotizacionUrl || null; // v8.39.0
  if (campos.esCompra !== undefined) u.es_compra = !!campos.esCompra;
  // v8.48.0: alistos
  if (campos.modoEntrega !== undefined) u.modo_entrega = campos.modoEntrega || 'envio';
  if (campos.clienteNombre !== undefined) u.cliente_nombre = campos.clienteNombre || null;
  if (campos.referencia !== undefined) u.referencia = campos.referencia || null;
  const { error } = await supabase.from('requisiciones').update(u).eq('id', id);
  if (error) throw error;
}

// v8.48.0: sign-off de un RETIRO — el cliente/tercero pasó a buscar el material.
// Registra quién retiró (nombre + cédula), la firma y una foto opcional, y marca la
// requisición como ENTREGADA. No pasa por Rutas.
export async function subirRetiroAlisto(blob, reqId, tipo) {
  const path = `retiros/${reqId}-${tipo}-${Date.now()}.${tipo === 'firma' ? 'png' : 'jpg'}`;
  const { error } = await supabase.storage.from('documentos-fotos').upload(path, blob, {
    contentType: tipo === 'firma' ? 'image/png' : 'image/jpeg', upsert: false,
  });
  if (error) throw error;
  const { data } = supabase.storage.from('documentos-fotos').getPublicUrl(path);
  return data.publicUrl;
}
export async function registrarRetiroAlisto(id, { porNombre, cedula, firmaUrl = null, fotoUrl = null }) {
  const { error } = await supabase.from('requisiciones').update({
    estado: 'entregada', entregada_at: new Date().toISOString(), retirada_at: new Date().toISOString(),
    retiro_por_nombre: porNombre || null, retiro_cedula: cedula || null,
    retiro_firma_url: firmaUrl || null, retiro_foto_url: fotoUrl || null,
    updated_at: new Date().toISOString(),
  }).eq('id', id);
  if (error) throw error;
}
// v8.49.11 (ticket Erisdania): modificar la solicitud después de enviada — agregar o
// quitar renglones (solo admin/almacén, en pendiente/preparando).
export async function agregarItemRequisicion(requisicionId, { descripcion, cantidad = null, unidad = null }) {
  const { error } = await supabase.from('requisicion_items').insert({
    id: requisicionId + '_x' + Date.now().toString(36),
    requisicion_id: requisicionId, descripcion: (descripcion || '').trim(),
    cantidad: cantidad === '' || cantidad == null ? null : Number(cantidad), unidad: unidad || null,
  });
  if (error) throw error;
}
export async function eliminarItemRequisicion(itemId) {
  const { error } = await supabase.from('requisicion_items').delete().eq('id', itemId);
  if (error) throw error;
}
export async function marcarItemRequisicion(itemId, despachado) {
  const { error } = await supabase.from('requisicion_items').update({ despachado: !!despachado }).eq('id', itemId);
  if (error) throw error;
}
// v8.39.0: flujo de compras por renglón — sin stock → solicitado a compras →
// cotizado → esperando aprobación → comprado. null = en stock / no aplica.
export async function marcarEstadoCompraItem(itemId, estadoCompra) {
  const { error } = await supabase.from('requisicion_items').update({ estado_compra: estadoCompra || null }).eq('id', itemId);
  if (error) throw error;
}
// v8.39.0: la cotización adjunta de una requisición-compra (foto o PDF).
export async function subirCotizacionRequisicion(file, reqId) {
  const ext = (file.name || '').split('.').pop().toLowerCase() || (file.type === 'application/pdf' ? 'pdf' : 'jpg');
  const path = `cotizaciones/${reqId}-${Date.now()}.${ext}`;
  const { error } = await supabase.storage.from('documentos-fotos').upload(path, file, {
...
// ============================================================
// DILIGENCIAS + LUGARES FRECUENTES (v8.40.0)
// Diligencia = algo pendiente de mover (hoy: retiro de material SOBRANTE en obra,
// reportado por el maestro con foto y listado). Vive "sin planificar" en Rutas
// como recordatorio hasta que se monta en un viaje.
// ============================================================
const mapDiligencia = (d) => ({
  id: d.id, tipo: d.tipo, proyectoId: d.proyecto_id, descripcion: d.descripcion || '',
  fotoUrl: d.foto_url || null, estado: d.estado, viajeId: d.viaje_id || null,
  creadoPorNombre: d.creado_por_nombre || '', createdAt: d.created_at,
  asignadaAt: d.asignada_at, completadaAt: d.completada_at,
});

export async function listarDiligencias({ estados = null, proyectoId = null } = {}) {
  let q = supabase.from('diligencias').select('*').order('created_at', { ascending: true });
  if (estados && estados.length) q = q.in('estado', estados);
  if (proyectoId) q = q.eq('proyecto_id', proyectoId);
  const { data, error } = await q;
  if (error) throw error;
  return (data || []).map(mapDiligencia);
}

export async function crearDiligencia({ tipo = 'retiro_sobrante', proyectoId, descripcion, fotoUrl = null, creadoPorId, creadoPorNombre }) {
  const id = 'dil_' + Date.now() + Math.random().toString(36).slice(2, 6);
  const { error } = await supabase.from('diligencias').insert({
    id, tipo, proyecto_id: proyectoId || null, descripcion: descripcion || null,
    foto_url: fotoUrl || null, creado_por_id: creadoPorId || null, creado_por_nombre: creadoPorNombre || null,
  });
  if (error) throw error;
  return id;
}

export async function actualizarDiligencia(id, campos) {
  const u = {};
  if (campos.estado !== undefined) {
    u.estado = campos.estado;
    if (campos.estado === 'asignada') u.asignada_at = new Date().toISOString();
    if (campos.estado === 'completada') u.completada_at = new Date().toISOString();
    if (campos.estado === 'sin_planificar') { u.viaje_id = null; u.asignada_at = null; }
  }
  if (campos.viajeId !== undefined) u.viaje_id = campos.viajeId || null;
  const { error } = await supabase.from('diligencias').update(u).eq('id', id);
  if (error) throw error;
}

export async function actualizarFotoDiligencia(id, fotoUrl) {
  const { error } = await supabase.from('diligencias').update({ foto_url: fotoUrl || null }).eq('id', id);
  if (error) throw error;
}

// v8.42.0: prueba de entrega — foto del material y firma del que recibe (PNG).
export async function subirEntregaParada(blob, paradaId, tipo) {
  const path = `entregas/${paradaId}-${tipo}-${Date.now()}.${tipo === 'firma' ? 'png' : 'jpg'}`;
  const { error } = await supabase.storage.from('documentos-fotos').upload(path, blob, {
    contentType: tipo === 'firma' ? 'image/png' : 'image/jpeg', upsert: false,
  });
  if (error) throw error;
...
// v8.42.0: documento de la parada (OC/cotización/factura — imagen o PDF).
export async function subirDocParada(file, viajeId) {
  const ext = (file.name || '').split('.').pop().toLowerCase() || (file.type === 'application/pdf' ? 'pdf' : 'jpg');
  const path = `paradas/${viajeId}-${Date.now()}.${ext}`;
  const { error } = await supabase.storage.from('documentos-fotos').upload(path, file, {
    contentType: file.type || 'application/octet-stream', upsert: false,
  });
  if (error) throw error;
...
// Foto del sobrante — SIEMPRE comprimida antes (comprimirImagenABlob en el caller).
export async function subirFotoSobrante(blob, diligenciaId) {
  const path = `sobrantes/${diligenciaId}-${Date.now()}.jpg`;
  const { error } = await supabase.storage.from('documentos-fotos').upload(path, blob, {
...
// ---- SUPLIDORES con locaciones (v8.41.0, espejo de Odoo) ----
// v8.49.13: estaciones de combustible habilitadas para la tarjeta flotilla.
export async function listarEstacionesCombustible() {
  const { data, error } = await supabase.from('estaciones_combustible').select('*').eq('activo', true).order('nombre');
  if (error) { console.warn('estaciones:', error.message); return []; }
  return (data || []).map(e => ({ id: e.id, nombre: e.nombre, direccion: e.direccion || '', provincia: e.provincia || '', lat: e.lat, lng: e.lng }));
}
export async function listarSuplidores() {
  const { data, error } = await supabase.from('suplidores').select('*, suplidor_locaciones(*)').eq('activo', true).order('nombre');
  if (error) throw error;
  return (data || []).map(s => ({
    id: s.id, nombre: s.nombre, rnc: s.rnc || '', odooPartnerId: s.odoo_partner_id || null, email: s.email || '', // v8.49.12
    locaciones: (s.suplidor_locaciones || []).filter(l => l.activo !== false)
      .map(l => ({ id: l.id, nombre: l.nombre, lat: l.lat, lng: l.lng, nota: l.nota || '' }))
      .sort((a, b) => (a.nombre || '').localeCompare(b.nombre || '')),
  }));
}
export async function crearSuplidor({ nombre, rnc = null, odooPartnerId = null }) {
  const id = 'sup_' + Date.now();
  const { error } = await supabase.from('suplidores').insert({ id, nombre, rnc: rnc || null, odoo_partner_id: odooPartnerId ?? null });
  if (error) throw error;
  return id;
}
// v8.49.12 (pedido de Leo): autorización de retiro en suplidor — queda registrada y se
// envía por correo al suplidor con quién retira.
export async function crearAutorizacionRetiro(a) {
  const id = 'aut_' + Date.now() + Math.random().toString(36).slice(2, 5);
  const { error } = await supabase.from('autorizaciones_retiro').insert({
    id, suplidor_id: a.suplidorId || null, suplidor_nombre: a.suplidorNombre, suplidor_email: a.suplidorEmail,
    materiales: a.materiales, referencia: a.referencia || null,
    retira_nombre: a.retiraNombre, retira_cedula: a.retiraCedula || null, vehiculo: a.vehiculo || null,
    enviado_por_id: a.enviadoPorId || null, enviado_por_nombre: a.enviadoPorNombre || null,
  });
  if (error) throw error;
  return id;
}
export async function marcarAutorizacionEnviada(id) {
  await supabase.from('autorizaciones_retiro').update({ email_enviado: true }).eq('id', id);
}
export async function listarAutorizacionesRetiro({ limit = 30 } = {}) {
  const { data, error } = await supabase.from('autorizaciones_retiro').select('*').order('created_at', { ascending: false }).limit(limit);
  if (error) { console.warn('autorizaciones:', error.message); return []; }
  return (data || []).map(a => ({
    id: a.id, createdAt: a.created_at, suplidorNombre: a.suplidor_nombre, suplidorEmail: a.suplidor_email,
    materiales: a.materiales, referencia: a.referencia || '', retiraNombre: a.retira_nombre,
    retiraCedula: a.retira_cedula || '', vehiculo: a.vehiculo || '', enviadoPorNombre: a.enviado_por_nombre || '',
    emailEnviado: !!a.email_enviado,
  }));
}
export async function actualizarSuplidor(id, campos) {
  const u = {};
  if (campos.email !== undefined) u.email = campos.email || null; // v8.49.12
  if (campos.nombre !== undefined) u.nombre = campos.nombre;
  if (campos.rnc !== undefined) u.rnc = campos.rnc || null;
  if (campos.odooPartnerId !== undefined) u.odoo_partner_id = campos.odooPartnerId ?? null;
  if (campos.activo !== undefined) u.activo = campos.activo;
  const { error } = await supabase.from('suplidores').update(u).eq('id', id);
  if (error) throw error;
}
export async function crearLocacionSuplidor({ suplidorId, nombre, lat = null, lng = null, nota = null }) {
  const id = 'sl_' + Date.now();
  const { error } = await supabase.from('suplidor_locaciones').insert({ id, suplidor_id: suplidorId, nombre, lat, lng, nota });
  if (error) throw error;
  return id;
}
export async function eliminarLocacionSuplidor(id) {
  const { error } = await supabase.from('suplidor_locaciones').update({ activo: false }).eq('id', id);
  if (error) throw error;
}

export async function listarLugaresLogisticos() {
  const { data, error } = await supabase.from('lugares_logisticos').select('*').eq('activo', true).order('tipo').order('nombre');
  if (error) throw error;
  return (data || []).map(l => ({ id: l.id, tipo: l.tipo, nombre: l.nombre, lat: l.lat, lng: l.lng, nota: l.nota || '' }));
}
export async function crearLugarLogistico({ tipo, nombre, lat, lng, nota }) {
  const id = 'lug_' + Date.now();
  const { error } = await supabase.from('lugares_logisticos').insert({ id, tipo: tipo || 'suplidor', nombre, lat: lat ?? null, lng: lng ?? null, nota: nota || null });
  if (error) throw error;
  return id;
}
export async function eliminarLugarLogistico(id) {
  const { error } = await supabase.from('lugares_logisticos').update({ activo: false }).eq('id', id);
  if (error) throw error;
}

function viajeRowToObj(v) {
  return {
    id: v.id, fecha: v.fecha, choferId: v.chofer_id, choferNombre: v.chofer_nombre || '',
    vehiculo: v.vehiculo || '', vehiculoId: v.vehiculo_id || null, tipoEnvio: v.tipo_envio || 'camion', estado: v.estado,
    horaInicio: v.hora_inicio, horaFin: v.hora_fin, notas: v.notas || '', creadoPorId: v.creado_por_id,
    paradas: (v.viaje_paradas || []).map(p => ({
      id: p.id, orden: p.orden || 0, tipo: p.tipo, lugar: p.lugar || '',
      proyectoId: p.proyecto_id, requisicionId: p.requisicion_id,
      diligenciaId: p.diligencia_id || null, lat: p.lat ?? null, lng: p.lng ?? null, // v8.40.0
      docUrl: p.doc_url || null, docNombre: p.doc_nombre || null, // v8.42.0
      entregaFotoUrl: p.entrega_foto_url || null, entregaFirmaUrl: p.entrega_firma_url || null, recibidoPorNombre: p.recibido_por_nombre || null, // v8.42.0 prueba de entrega
      descripcion: p.descripcion || '', estado: p.estado, completadaAt: p.completada_at, notas: p.notas || '',
    })).sort((a, b) => a.orden - b.orden),
  };
}
export async function listarViajes({ fecha = null, desde = null, hasta = null, choferId = null, vehiculoId = null } = {}) {
  let q = supabase.from('viajes').select('*, viaje_paradas(*)').order('fecha', { ascending: false });
  if (fecha) q = q.eq('fecha', fecha);
  if (desde) q = q.gte('fecha', desde);
  if (hasta) q = q.lte('fecha', hasta);
  if (choferId) q = q.eq('chofer_id', choferId);
  if (vehiculoId) q = q.eq('vehiculo_id', vehiculoId); // v8.41.0
  const { data, error } = await q;
  if (error) throw error;
  return (data || []).map(viajeRowToObj);
}
export async function crearViaje({ fecha, choferId, choferNombre, vehiculo, vehiculoId = null, tipoEnvio, notas, creadoPorId }) {
  const id = 'vj_' + Date.now() + Math.random().toString(36).slice(2, 6);
  const { error } = await supabase.from('viajes').insert({
    id, fecha, chofer_id: choferId || null, chofer_nombre: choferNombre || null,
    vehiculo: vehiculo || null, vehiculo_id: vehiculoId || null, // v8.41.0: vehículo real de la flota
    tipo_envio: tipoEnvio || 'camion', notas: notas || null,
    creado_por_id: creadoPorId || null,
  });
  if (error) throw error;
  return id;
}
export async function actualizarViaje(id, campos) {
  const u = { updated_at: new Date().toISOString() };
  if (campos.estado !== undefined) u.estado = campos.estado;
  if (campos.horaInicio !== undefined) u.hora_inicio = campos.horaInicio;
  if (campos.horaFin !== undefined) u.hora_fin = campos.horaFin;
  if (campos.notas !== undefined) u.notas = campos.notas || null;
  if (campos.vehiculo !== undefined) u.vehiculo = campos.vehiculo || null;
  const { error } = await supabase.from('viajes').update(u).eq('id', id);
  if (error) throw error;
}
...
export async function agregarParada({ viajeId, orden, tipo, lugar, proyectoId, requisicionId, descripcion, diligenciaId = null, lat = null, lng = null, docUrl = null, docNombre = null }) {
  const id = 'vp_' + Date.now() + Math.random().toString(36).slice(2, 6);
  const { error } = await supabase.from('viaje_paradas').insert({
    id, viaje_id: viajeId, orden: orden || 0, tipo: tipo || 'entrega',
    lugar: lugar || null, proyecto_id: proyectoId || null, requisicion_id: requisicionId || null,
    diligencia_id: diligenciaId || null, lat, lng, // v8.40.0
    doc_url: docUrl || null, doc_nombre: docNombre || null, // v8.42.0: OC/cotización para mostrar al retirar
    descripcion: descripcion || null,
  });
  if (error) throw error;
  return id;
}
export async function actualizarParada(id, campos) {
  const u = {};
  if (campos.estado !== undefined) {
    u.estado = campos.estado;
    u.completada_at = campos.estado === 'completada' ? new Date().toISOString() : null;
  }
  if (campos.orden !== undefined) u.orden = campos.orden;
  if (campos.notas !== undefined) u.notas = campos.notas || null;
  // v8.42.0: prueba de entrega (foto + firma del que recibe)
  if (campos.entregaFotoUrl !== undefined) u.entrega_foto_url = campos.entregaFotoUrl || null;
  if (campos.entregaFirmaUrl !== undefined) u.entrega_firma_url = campos.entregaFirmaUrl || null;
  if (campos.recibidoPorNombre !== undefined) u.recibido_por_nombre = campos.recibidoPorNombre || null;
  const { error } = await supabase.from('viaje_paradas').update(u).eq('id', id);
  if (error) throw error;
  // v8.40.0: si la parada completada venía de una diligencia (retiro de sobrante),
  // la diligencia se cierra sola — el recordatorio desaparece de Rutas.
  if (campos.estado === 'completada') {
    try {
      const { data: p } = await supabase.from('viaje_paradas').select('diligencia_id').eq('id', id).maybeSingle();
      if (p?.diligencia_id) {
        await supabase.from('diligencias').update({ estado: 'completada', completada_at: new Date().toISOString() }).eq('id', p.diligencia_id);
      }
    } catch (e) { /* no bloquear la parada */ }
  }
}
export async function eliminarParada(id) {
  const { error } = await supabase.from('viaje_paradas').delete().eq('id', id);
  if (error) throw error;
}

// ============================================================
// v8.30.0: Órdenes de Cambio (aumentos de volumen / alcance)
// ============================================================
function ordenCambioRowToObj(r) {
  return {
    id: r.id, proyectoId: r.proyecto_id, numero: r.numero, tipo: r.tipo, estado: r.estado,
    motivo: r.motivo || '', lineas: r.lineas || [], montoTotal: Number(r.monto_total) || 0,
    creadoPorId: r.creado_por_id, creadoPorNombre: r.creado_por_nombre || '',
    enviadaAt: r.enviada_at, enviadaA: r.enviada_a || '',
    aprobadaPorCliente: r.aprobada_por_cliente || '', aprobadaVia: r.aprobada_via || '',
    aprobadaAt: r.aprobada_at, aprobadaNota: r.aprobada_nota || '',
    aplicadaAt: r.aplicada_at, createdAt: r.created_at,
  };
}
export async function listarOrdenesCambio(proyectoId = null) {
  let q = supabase.from('ordenes_cambio').select('*').order('created_at', { ascending: false });
  if (proyectoId) q = q.eq('proyecto_id', proyectoId);
  const { data, error } = await q;
  if (error) throw error;
  return (data || []).map(ordenCambioRowToObj);
}
export async function crearOrdenCambio(oc) {
  const { data: prev } = await supabase.from('ordenes_cambio')
    .select('numero').eq('proyecto_id', oc.proyectoId).order('numero', { ascending: false }).limit(1);
  const numero = ((prev && prev[0]?.numero) || 0) + 1;
  const id = 'oc_' + Date.now() + Math.random().toString(36).slice(2, 6);
  const { error } = await supabase.from('ordenes_cambio').insert({
    id, proyecto_id: oc.proyectoId, numero, tipo: oc.tipo || 'aumento',
    motivo: oc.motivo || null, lineas: oc.lineas || [],
    monto_total: Number(oc.montoTotal) || 0,
    creado_por_id: oc.creadoPorId || null, creado_por_nombre: oc.creadoPorNombre || null,
  });
  if (error) throw error;
  const ctx = getAuditContext();
  registrarAudit({
    usuarioId: ctx.usuarioId, usuarioNombre: ctx.usuarioNombre,
    accion: 'orden_cambio.creada', recursoTipo: 'orden_cambio', recursoId: id,
    recursoNombre: `OC-${numero}`, datosDespues: { proyectoId: oc.proyectoId, montoTotal: oc.montoTotal },
    severidad: 'info',
  });
  return { id, numero };
}
export async function actualizarOrdenCambio(id, campos) {
  const u = { updated_at: new Date().toISOString() };
  if (campos.estado !== undefined) u.estado = campos.estado;
  if (campos.enviadaA !== undefined) { u.enviada_a = campos.enviadaA || null; u.enviada_at = new Date().toISOString(); }
  if (campos.aprobadaPorCliente !== undefined) u.aprobada_por_cliente = campos.aprobadaPorCliente || null;
  if (campos.aprobadaVia !== undefined) u.aprobada_via = campos.aprobadaVia || null;
  if (campos.aprobadaNota !== undefined) u.aprobada_nota = campos.aprobadaNota || null;
  if (campos.estado === 'aprobada_cliente') u.aprobada_at = new Date().toISOString();
  if (campos.estado === 'aplicada') { u.aplicada_at = new Date().toISOString(); u.aplicada_por_id = campos.aplicadaPorId || null; }
  const { error } = await supabase.from('ordenes_cambio').update(u).eq('id', id);
  if (error) throw error;
  const ctx = getAuditContext();
  registrarAudit({
    usuarioId: ctx.usuarioId, usuarioNombre: ctx.usuarioNombre,
    accion: 'orden_cambio.' + (campos.estado || 'actualizada'), recursoTipo: 'orden_cambio', recursoId: id,
    severidad: campos.estado === 'aplicada' ? 'warning' : 'info',
  });
}
export async function eliminarOrdenCambio(id) {
  const { error } = await supabase.from('ordenes_cambio').delete().eq('id', id).eq('estado', 'borrador');
  if (error) throw error;
}

// ============================================================
// v8.30.2: Resumen de actividad ERP (tablero Carga y Actividad)
// Quién hizo qué y cuándo — todo sale de los autores/fechas que las
// tablas ya guardan; nadie reporta nada a mano.
// ============================================================
export async function resumenActividadERP(desdeFecha) {
  const iso = desdeFecha + 'T00:00:00';
  const [jor, rep, aju, his, req, via, fac, caja, tar, cub, oc, colaReq] = await Promise.all([
    supabase.from('jornadas').select('iniciada_por_id, finalizada_por_id').gte('fecha', desdeFecha),
    supabase.from('reportes').select('supervisor_id').gte('fecha', desdeFecha),
    supabase.from('ajustes_nomina').select('creado_por_id').gte('created_at', iso),
    supabase.from('historial_estados').select('cambiado_por_id').gte('created_at', iso),
    supabase.from('requisiciones').select('solicitado_por_id, estado, created_at, lista_at, entregada_at').gte('created_at', iso),
    supabase.from('viajes').select('chofer_id, viaje_paradas(estado)').gte('fecha', desdeFecha),
    supabase.from('facturas_odoo').select('creado_por_id').gte('created_at', iso),
    supabase.from('caja_chica_movimientos').select('creado_por_id, aprobado_por_id').gte('created_at', iso),
    supabase.from('tareas').select('completada_por_id').eq('completada', true).gte('completada_at', iso),
    supabase.from('cubicaciones').select('creado_por_id').gte('created_at', iso),
    supabase.from('ordenes_cambio').select('creado_por_id').gte('created_at', iso),
    supabase.from('requisiciones').select('id', { count: 'exact', head: true }).in('estado', ['pendiente', 'preparando', 'lista']),
  ]);
  const por = {};
  const add = (id, k) => { if (!id) return; por[id] = por[id] || {}; por[id][k] = (por[id][k] || 0) + 1; };
  (jor.data || []).forEach(j => { add(j.iniciada_por_id, 'jornadasAbiertas'); add(j.finalizada_por_id, 'jornadasCerradas'); });
  (rep.data || []).forEach(r => add(r.supervisor_id, 'reportes'));
  (aju.data || []).forEach(x => add(x.creado_por_id, 'ajustes'));
  (his.data || []).forEach(x => add(x.cambiado_por_id, 'estados'));
  (req.data || []).forEach(x => add(x.solicitado_por_id, 'pedidos'));
  (via.data || []).forEach(v => (v.viaje_paradas || []).forEach(p => { if (p.estado === 'completada') add(v.chofer_id, 'paradas'); }));
  (fac.data || []).forEach(x => add(x.creado_por_id, 'facturas'));
  (caja.data || []).forEach(x => { add(x.creado_por_id, 'cajaReportada'); add(x.aprobado_por_id, 'cajaAprobada'); });
  (tar.data || []).forEach(x => add(x.completada_por_id, 'tareas'));
  (cub.data || []).forEach(x => add(x.creado_por_id, 'cubicaciones'));
  (oc.data || []).forEach(x => add(x.creado_por_id, 'ordenesCambio'));
  ...
  return {
    porPersona: por,
    ciclos: {
      pedidoALista: prom(reqs.filter(r => r.lista_at).map(r => hrs(r.created_at, r.lista_at))),
      listaAEntrega: prom(reqs.filter(r => r.lista_at && r.entregada_at).map(r => hrs(r.lista_at, r.entregada_at))),
    },
    colas: { pedidosEnCola: colaReq.count || 0 },
  };
}

// ============================================================
// v8.30.4: Revisión de reportes por el supervisor
// ============================================================
export async function revisarReporte(reporteId, { revisadoPorId, revisadoPorNombre, observacion = null }) {
  const { error } = await supabase.from('reportes').update({
    revisado_por_id: revisadoPorId,
    revisado_por_nombre: revisadoPorNombre || null,
...
// ============================================================
// v8.31.0: Bono comercial (Edwin) — fuentes de datos
// ============================================================
export async function listarSolicitudesLevantamiento({ desde = null } = {}) {
  let q = supabase.from('solicitudes_levantamiento').select('id, created_at, resuelto_at, resuelto_por_id, estado');
  if (desde) q = q.gte('created_at', desde + 'T00:00:00');
  const { data, error } = await q;
  if (error) throw error;
  return data || [];
}
export async function listarTareasPorTipo(tipo, { desde = null } = {}) {
  let q = supabase.from('tareas').select('*').eq('tipo', tipo);
  if (desde) q = q.gte('created_at', desde + 'T00:00:00');
  const { data, error } = await q;
  if (error) throw error;
  return (data || []).map(t => ({
    id: t.id, tipo: t.tipo, titulo: t.titulo,
    asignadaAId: t.asignada_a_id, fechaLimite: t.fecha_limite,
    completada: t.completada, completadaAt: t.completada_at, createdAt: t.created_at,
  }));
}

// ============================================================
// v8.31.1: Proyecto COMPLETO — creación correcta + notificación inmediata
// Regla: proyecto aprobado en Odoo = completo en el ERP el mismo día.
// ============================================================
export async function listarCajaMovimientosRango(desde) {
  const { data, error } = await supabase.from('caja_chica_movimientos')
    .select('creado_por_id, fecha, created_at').gte('fecha', desde);
  if (error) throw error;
  return data || [];
}

// Chequeo server-shape (row de proyectos) — espejo de lib/helpers/proyectoCompleto.
export async function faltantesProyectoServidor(proyectoId) {
  ...
  if (!p) return { faltantes: ['proyecto no encontrado'], p: null };
  const faltantes = [];
  if (!((p.cliente || p.nombre || '').trim())) faltantes.push('nombre del cliente');
  const legacy = `${p.contacto_cliente_nombre || ''}${p.contacto_cliente_telefono || ''}${p.contacto_cliente_email || ''}`.trim();
  if (!p.contacto_principal_id && !legacy) faltantes.push('contacto del cliente');
  if (p.ubicacion_lat == null || p.ubicacion_lng == null) faltantes.push('ubicación GPS');
  const areas = p.areas || [];
  if (areas.length === 0) faltantes.push('áreas del proyecto');
  else {
    if (areas.some(a => !(Number(a.m2) > 0))) faltantes.push('m² en todas las áreas');
    if (areas.some(a => !(a.sistemaId || p.sistema_id))) faltantes.push('sistema en todas las áreas');
  }
  ...
  if (sids.length === 0) faltantes.push('sistema del proyecto');
    ...
    const { data: ss } = await supabase.from('sistemas').select('id, data').in('id', sids);
    const sinTareas = sids.some(sid => !((ss || []).find(s => s.id === sid)?.data?.tareas?.length > 0));
    if (sinTareas) faltantes.push('tareas del sistema');
  }
  if (!(Number(p.valor_cotizacion) > 0)) faltantes.push('valor de la cotización');
  if (!p.supervisor_id) faltantes.push('supervisor asignado');
  if (!p.maestro_id) faltantes.push('maestro asignado');
  return { faltantes, p };
}

// Auto-llenar desde el LEVANTAMIENTO: si el proyecto no tiene ubicación, la toma
// del site del levantamiento cuya referencia_odoo coincide con la del proyecto.
export async function enriquecerProyectoDesdeLevantamiento(proyectoId) {
  try {
    const { data: p } = await supabase.from('proyectos')
      .select('id, referencia_odoo, ubicacion_lat, ubicacion_lng').eq('id', proyectoId).maybeSingle();
    if (!p?.referencia_odoo || (p.ubicacion_lat != null && p.ubicacion_lng != null)) return false;
    const { data: sites } = await supabase.schema('surveys').from('sites')
      .select('latitude, longitude, address').eq('referencia_odoo', p.referencia_odoo)
      .not('latitude', 'is', null).limit(1);
    const s = sites?.[0];
    if (!s) return false;
    await supabase.from('proyectos').update({
      ubicacion_lat: Number(s.latitude), ubicacion_lng: Number(s.longitude),
    }).eq('id', proyectoId);
    return true;
  } catch (e) { console.warn('enriquecerDesdeLevantamiento:', e?.message); return false; }
}

// Notificación INMEDIATA a Miguel y Erisdania cuando un proyecto aprobado quedó
// incompleto (primero intenta auto-llenar desde el levantamiento).
const EMAILS_CREACION_PROYECTOS = ['mmartinez@supertechos.com.do', 'eperez@supertechos.com.do'];
export async function notificarProyectoIncompleto(proyectoId) {
  try {
    await enriquecerProyectoDesdeLevantamiento(proyectoId);
    const { faltantes, p } = await faltantesProyectoServidor(proyectoId);
    if (!p || faltantes.length === 0) return { completo: true };
    const etiqueta = [p.referencia_odoo, p.cliente || p.nombre].filter(Boolean).join(' · ');
    const html = `<div style="font-family:Arial,sans-serif;max-width:560px">
      <h2 style="color:#D71920">🧩 Proyecto aprobado con información incompleta</h2>
      <p style="font-size:14px"><b>${etiqueta}</b></p>
      <p style="font-size:13px">La regla: un proyecto aprobado queda completo en el ERP <b>el mismo día</b>. Le falta:</p>
      <ul style="font-size:13px">${faltantes.map(f => `<li>${f}</li>`).join('')}</ul>
      <p style="font-size:12px;color:#666">Complétenlo hoy — este pendiente cuenta para el KPI "Proyectos creados completos". Si la ubicación existe en el levantamiento, el sistema la toma sola al reintentar.</p>
      <p style="font-size:12px;color:#666">— ERP Super Techos</p></div>`;
    enviarCorreoReporte(EMAILS_CREACION_PROYECTOS, `🧩 Completar proyecto: ${etiqueta}`, html);
    return { completo: false, faltantes };
  } catch (e) { console.warn('notificarProyectoIncompleto:', e?.message); return { error: e?.message }; }
}

// ============================================================
// v8.33.0: Log de eventos del vehículo (mantenimientos, fallas, choques, daños)
// ============================================================
function eventoVehiculoRowToObj(r) {
  return {
    id: r.id, vehiculoId: r.vehiculo_id, tipo: r.tipo, fecha: r.fecha,
    km: r.km != null ? Number(r.km) : null, descripcion: r.descripcion || '',
    costoRd: r.costo_rd != null ? Number(r.costo_rd) : null, taller: r.taller || '',
    reportadoPorId: r.reportado_por_id, reportadoPorNombre: r.reportado_por_nombre || '',
    estado: r.estado || 'abierto', resueltoAt: r.resuelto_at, resueltoNota: r.resuelto_nota || '',
    // v8.51.0: protocolo de averías — trazabilidad completa
    sintoma: r.sintoma || null, gravedad: r.gravedad || null,
    lat: r.lat != null ? Number(r.lat) : null, lng: r.lng != null ? Number(r.lng) : null,
    fotos: Array.isArray(r.fotos) ? r.fotos : [],
    diagnostico: r.diagnostico || '', enTallerAt: r.en_taller_at || null,
    retornoChecklist: r.retorno_checklist || null,
    citaFecha: r.cita_fecha || null, citaTaller: r.cita_taller || null, // v8.51.0: cita de mantenimiento
    createdAt: r.created_at,
  };
}
export async function listarEventosVehiculo({ vehiculoId = null, soloAbiertos = false } = {}) {
  let q = supabase.from('vehiculo_eventos').select('*').order('fecha', { ascending: false });
  if (vehiculoId) q = q.eq('vehiculo_id', vehiculoId);
  if (soloAbiertos) q = q.neq('estado', 'resuelto');
  const { data, error } = await q;
  if (error) throw error;
  return (data || []).map(eventoVehiculoRowToObj);
}
export async function crearEventoVehiculo(e) {
  const id = 've_' + Date.now() + Math.random().toString(36).slice(2, 6);
  const { error } = await supabase.from('vehiculo_eventos').insert({
    id, vehiculo_id: e.vehiculoId, tipo: e.tipo, fecha: e.fecha,
    km: e.km ? Number(e.km) : null, descripcion: e.descripcion,
    costo_rd: e.costoRd ? Number(e.costoRd) : null, taller: e.taller || null,
    reportado_por_id: e.reportadoPorId || null, reportado_por_nombre: e.reportadoPorNombre || null,
    estado: e.estado || 'abierto',
    // v8.51.0: protocolo de averías
    ...(e.sintoma ? { sintoma: e.sintoma } : {}),
    ...(e.gravedad ? { gravedad: e.gravedad } : {}),
    ...(e.lat != null ? { lat: e.lat, lng: e.lng } : {}),
  });
  if (error) throw error;
  // v8.51.0: avería CRÍTICA (no debe moverse) → el vehículo sale de servicio solo.
  if (e.gravedad === 'critica') {
    await supabase.from('vehiculos').update({ estado_operativo: 'fuera_servicio' }).eq('id', e.vehiculoId).then(() => {}, () => {});
  }
  // Si el evento trae km, actualiza el odómetro del vehículo (solo hacia adelante).
  if (e.km) {
    const { data: v } = await supabase.from('vehiculos').select('odometro_km').eq('id', e.vehiculoId).maybeSingle();
    if (!v?.odometro_km || Number(e.km) > Number(v.odometro_km)) {
      await supabase.from('vehiculos').update({ odometro_km: Number(e.km), odometro_fecha: e.fecha }).eq('id', e.vehiculoId);
    }
  }
  return id;
}
export async function actualizarEventoVehiculo(id, campos) {
  const u = {};
  if (campos.estado !== undefined) {
    u.estado = campos.estado;
    if (campos.estado === 'en_taller') u.en_taller_at = new Date().toISOString(); // v8.51.0
    if (campos.estado === 'resuelto') { u.resuelto_at = new Date().toISOString(); u.resuelto_nota = campos.resueltoNota || null; }
  }
  // v8.51.0: cita de mantenimiento (Erisdania la programa y queda en el ERP)
  if (campos.citaFecha !== undefined) u.cita_fecha = campos.citaFecha || null;
  if (campos.citaTaller !== undefined) u.cita_taller = campos.citaTaller || null;
  // v8.51.0: cierre con trazabilidad — diagnóstico/causa, costo, taller, checklist retorno
  if (campos.diagnostico !== undefined) u.diagnostico = campos.diagnostico || null;
  if (campos.costoRd !== undefined) u.costo_rd = campos.costoRd != null ? Number(campos.costoRd) : null;
  if (campos.taller !== undefined) u.taller = campos.taller || null;
  if (campos.retornoChecklist !== undefined) u.retorno_checklist = campos.retornoChecklist || null;
  const { error } = await supabase.from('vehiculo_eventos').update(u).eq('id', id);
  if (error) throw error;
}

// v8.51.0: fotos de la avería (mismo bucket de vehículos que las inspecciones).
export async function subirFotoEventoVehiculo({ file, eventoId }) {
  if (!(file instanceof Blob)) throw new Error('file debe ser File o Blob');
  const ext = extFromMime(file.type) || 'jpg';
  const path = `averias/${eventoId}/${Date.now()}.${ext}`;
  const { error } = await supabase.storage.from(BUCKET_VEHICULOS).upload(path, file, {
    contentType: file.type || 'image/jpeg', upsert: true,
  });
  if (error) throw error;
  // Anexar el path al array fotos del evento (leer + escribir; un solo editor a la vez).
  const { data: ev } = await supabase.from('vehiculo_eventos').select('fotos').eq('id', eventoId).maybeSingle();
  const fotos = Array.isArray(ev?.fotos) ? ev.fotos : [];
  await supabase.from('vehiculo_eventos').update({ fotos: [...fotos, path] }).eq('id', eventoId);
  return path;
}

// v8.51.0: reactivar el vehículo al cerrar la avería (retorno a servicio).
export async function reactivarVehiculo(vehiculoId) {
  const { error } = await supabase.from('vehiculos').update({ estado_operativo: 'activo' }).eq('id', vehiculoId);
  if (error) throw error;
}

// v8.51.0 (caso "¿está de mantenimiento el KIA?"): marcar EN TALLER — la vista
// Flota y Rutas lo reflejan al momento, y el equipo deja de preguntar por WhatsApp.
export async function marcarVehiculoEnTaller(vehiculoId) {
  const { error } = await supabase.from('vehiculos').update({ estado_operativo: 'en_taller' }).eq('id', vehiculoId);
  if (error) throw error;
}

// v8.33.0: delegar una tarea / cambiar fecha límite o supervisor.
// v8.37.0: + hora, fecha planificada, mover de obra/espacio, historial de asignaciones.
export async function actualizarTarea(id, campos) {
  const ctx = getAuditContext();
  const u = {};
  if (campos.asignadaAId !== undefined) { u.asignada_a_id = campos.asignadaAId || null; u.asignada_a_nombre = campos.asignadaANombre || null; }
  if (campos.supervisorId !== undefined) { u.supervisor_id = campos.supervisorId || null; u.supervisor_nombre = campos.supervisorNombre || null; }
  if (campos.fechaLimite !== undefined) u.fecha_limite = campos.fechaLimite || null;
  if (campos.horaLimite !== undefined) u.hora_limite = campos.horaLimite || null;
  if (campos.fechaPlanificada !== undefined) u.fecha_planificada = campos.fechaPlanificada || null;
  if (campos.proyectoId !== undefined) u.proyecto_id = campos.proyectoId || null;
  if (campos.proyectoInternoId !== undefined) u.proyecto_interno_id = campos.proyectoInternoId || null;
  if (campos.prioridad !== undefined) u.prioridad = campos.prioridad || 'normal'; // v8.33.2
  // Reasignación → queda constancia en el historial de la tarea.
  if (campos.asignadaAId !== undefined) {
    const { data: t } = await supabase.from('tareas').select('historial, asignada_a_id').eq('id', id).maybeSingle();
    if (t && t.asignada_a_id !== (campos.asignadaAId || null)) {
      u.historial = [...(t.historial || []), { tipo: 'asignada', a: campos.asignadaANombre || '(sin responsable)', por: ctx.usuarioNombre || null, at: new Date().toISOString() }];
    }
  }
  const { error } = await supabase.from('tareas').update(u).eq('id', id);
  if (error) throw error;
  registrarAudit({
    usuarioId: ctx.usuarioId, usuarioNombre: ctx.usuarioNombre,
    accion: 'tarea.actualizada', recursoTipo: 'tarea', recursoId: id,
    datosDespues: { delegadaA: campos.asignadaANombre, fechaLimite: campos.fechaLimite },
    severidad: 'info',
  });
}

// v8.33.2: comentarios en la tarea (append al jsonb).
export async function comentarTarea(tareaId, { porId, porNombre, texto }) {
  const { data: t } = await supabase.from('tareas').select('comentarios').eq('id', tareaId).maybeSingle();
  const comentarios = [...(t?.comentarios || []), { porId, porNombre, texto, at: new Date().toISOString() }];
  const { error } = await supabase.from('tareas').update({ comentarios }).eq('id', tareaId);
  if (error) throw error;
  return comentarios;
}

// v8.33.4: subtareas (checklist) y likes de la tarea.
export async function actualizarSubtareas(tareaId, subtareas) {
  const { error } = await supabase.from('tareas').update({ subtareas }).eq('id', tareaId);
  if (error) throw error;
}
export async function toggleLikeTarea(tareaId, { porId, porNombre }) {
  const { data: t } = await supabase.from('tareas').select('likes').eq('id', tareaId).maybeSingle();
  const likes = t?.likes || [];
  const ya = likes.some(l => l.porId === porId);
  const nuevos = ya ? likes.filter(l => l.porId !== porId) : [...likes, { porId, porNombre }];
  const { error } = await supabase.from('tareas').update({ likes: nuevos }).eq('id', tareaId);
  if (error) throw error;
  return nuevos;
}

// ============================================================
// v8.42.1: Resumen GPS de flota (para la pestaña Flota GPS en Vehículos).
// Lo llena el agente diario desde los reportes de Pressto.
// ============================================================
export async function listarGpsResumen({ periodo = null } = {}) {
  let q = supabase.from('vehiculo_gps_resumen').select('*');
  if (periodo) q = q.eq('periodo', periodo);
  const { data, error } = await q;
  if (error) { console.warn('listarGpsResumen:', error.message); return []; }
  return (data || []).map(r => ({
    id: r.id, vehiculoId: r.vehiculo_id, deviceName: r.device_name, periodo: r.periodo,
    km: r.km != null ? Number(r.km) : 0, viajes: r.viajes || 0, velMax: r.vel_max || 0,
    ralentiS: Number(r.ralenti_s || 0), ralentiPct: r.ralenti_pct || 0, motorS: Number(r.motor_s || 0),
    casaS: Number(r.casa_s || 0), baseS: Number(r.base_s || 0), obraS: Number(r.obra_s || 0), otrosS: Number(r.otros_s || 0),
    obras: Array.isArray(r.obras) ? r.obras : [], actualizadoAt: r.actualizado_at,
  }));
}

// Períodos disponibles en el resumen GPS (para el selector de fechas).
export async function periodosGpsDisponibles() {
  const { data, error } = await supabase.from('vehiculo_gps_resumen').select('periodo');
  if (error) return [];
  return [...new Set((data || []).map(r => r.periodo))].sort().reverse();
}

// ============================================================
// v8.46.0: PRESUPUESTOS DE OBRA (rentabilidad por partida)
// Una fila por versión; solo una 'aprobado' vigente por proyecto
// (unique index parcial uq_ppto_obra_aprobado, migración 126).
// ============================================================
const mapPresupuestoObra = (r) => ({
  id: r.id, proyectoId: r.proyecto_id, version: r.version || 1, estado: r.estado,
  venta: r.venta || {}, partidas: r.partidas || [], gastos: r.gastos || [],
  notas: r.notas || '', creadoPorId: r.creado_por_id || null,
  aprobadoPorId: r.aprobado_por_id || null, aprobadoAt: r.aprobado_at || null,
  createdAt: r.created_at, updatedAt: r.updated_at,
});

export async function listarPresupuestosObra(proyectoId) {
  const { data, error } = await supabase.from('presupuestos_obra').select('*')
    .eq('proyecto_id', proyectoId).order('version', { ascending: false });
  if (error) throw error;
  return (data || []).map(mapPresupuestoObra);
}

// UNA query para el portafolio de Finanzas: los presupuestos vigentes de todas las obras.
export async function listarPresupuestosAprobados() {
  const { data, error } = await supabase.from('presupuestos_obra').select('*').eq('estado', 'aprobado');
  if (error) throw error;
  return (data || []).map(mapPresupuestoObra);
}

export async function crearPresupuestoObra(p) {
  const { error } = await supabase.from('presupuestos_obra').insert({
    id: p.id, proyecto_id: p.proyectoId, version: p.version || 1, estado: 'borrador',
    venta: p.venta || {}, partidas: p.partidas || [], gastos: p.gastos || [],
    notas: p.notas || null, creado_por_id: p.creadoPorId || null,
  });
  if (error) throw error;
}

// Solo borradores (la UI lo garantiza; el aprobado es un snapshot congelado).
export async function actualizarPresupuestoObra(id, { venta, partidas, gastos, notas }) {
  const patch = { updated_at: new Date().toISOString() };
  if (venta !== undefined) patch.venta = venta;
  if (partidas !== undefined) patch.partidas = partidas;
  if (gastos !== undefined) patch.gastos = gastos;
  if (notas !== undefined) patch.notas = notas;
  const { error } = await supabase.from('presupuestos_obra').update(patch)
    .eq('id', id).eq('estado', 'borrador');
  if (error) throw error;
}

// Aprueba una versión: la vigente anterior pasa a 'superseded'. El unique index
// parcial evita que una carrera deje dos aprobadas.
export async function aprobarPresupuestoObra(presupuestoId, proyectoId, usuarioId) {
  const { error: e1 } = await supabase.from('presupuestos_obra')
    .update({ estado: 'superseded', updated_at: new Date().toISOString() })
    .eq('proyecto_id', proyectoId).eq('estado', 'aprobado');
  if (e1) throw e1;
  const { error: e2 } = await supabase.from('presupuestos_obra')
    .update({ estado: 'aprobado', aprobado_por_id: usuarioId || null, aprobado_at: new Date().toISOString(), updated_at: new Date().toISOString() })
    .eq('id', presupuestoId);
  if (e2) throw e2;
}

export async function eliminarPresupuestoObra(id) {
  const { error } = await supabase.from('presupuestos_obra').delete()
    .eq('id', id).eq('estado', 'borrador');
  if (error) throw error;
}

// ============================================================
// PEAJES (Paso Rápido) — v8.50.0
// ============================================================
// El export del portal trae: ID | Fecha | Monto | Tag | Tipo | Estación.
// El monto viene NEGATIVO en los pases y positivo en las recargas; aquí se
// guarda el gasto en positivo para no tener que acordarse del signo después.
//
// Identificación: un tag se engancha a un vehículo por placa. El que no cruza
// queda pendiente y se resuelve a mano (asignar / crear / fuera de flota).

// Las placas se escriben de mil formas entre el portal y el ERP.
const normPlaca = (p) => String(p || '').toUpperCase().replace(/[^A-Z0-9]/g, '');

export async function listarPeajeTags() {
  const { data, error } = await supabase
    .from('peaje_tags')
    .select('*, vehiculo:vehiculos(id, marca, modelo, placa)')
    .order('tag');
  if (error) throw error;
  return (data || []).map(r => ({
    tag: r.tag,
    nombrePortal: r.nombre_portal || '',
    placaPortal: r.placa_portal || '',
    vehiculoId: r.vehiculo_id,
    vehiculo: r.vehiculo || null,
    estado: r.estado,
    nota: r.nota || '',
    fusionarCon: r.fusionar_con || null,
  }));
}

// Consumo acumulado de cada tag. Es lo que decide qué pendiente urge:
// un tag sin un solo pase no es un problema, uno con RD$28,000 al año sí.
export async function consumoPorTag() {
  const { data, error } = await supabase.from('peaje_consumo_tag').select('*');
  if (error) throw error;
  const map = {};
  (data || []).forEach(r => {
    map[r.tag] = {
      pases: Number(r.pases || 0), total: Number(r.total || 0),
      primerMes: r.primer_mes, ultimoMes: r.ultimo_mes,
    };
  });
  return map;
}

export async function listarPeajePeriodos() {
  const { data, error } = await supabase.from('peaje_periodos')
    .select('*').order('mes', { ascending: false });
  if (error) throw error;
  return data || [];
}

export async function resumenPeajeMeses() {
  const { data, error } = await supabase.from('peaje_resumen_mes')
    .select('*').order('mes', { ascending: false });
  if (error) throw error;
  return (data || []).map(r => ({
    mes: r.mes,
    ...
    flota: Number(r.flota || 0),
    particular: Number(r.particular || 0),
    sinAsignar: Number(r.sin_asignar || 0),
    tags: Number(r.tags || 0),
    tagsSinVehiculo: Number(r.tags_sin_vehiculo || 0),
    pases: Number(r.pases || 0),
  }));
}

// Consumo mensual de un vehículo (para su ficha). Devuelve los meses con datos.
export async function consumoPeajeVehiculo(vehiculoId) {
  const { data, error } = await supabase.from('peaje_consumo_vehiculo')
    .select('*').eq('vehiculo_id', vehiculoId).order('mes');
  if (error) throw error;
  return (data || []).map(r => ({
    mes: r.mes,
    pases: Number(r.pases || 0),
    monto: Number(r.monto || 0),
    pasesFinde: Number(r.pases_finde || 0),
    pasesNoche: Number(r.pases_noche || 0),
  }));
}

// Consumo del mes de TODOS los vehículos, para la columna de la lista.
export async function consumoPeajeDelMes(mes) {
  const { data, error } = await supabase.from('peaje_consumo_vehiculo')
    .select('vehiculo_id, monto, pases').eq('mes', mes);
  if (error) throw error;
  const map = {};
  (data || []).forEach(r => {
    map[r.vehiculo_id] = { monto: Number(r.monto || 0), pases: Number(r.pases || 0) };
  });
  return map;
}

export async function pasesPeajeVehiculo(vehiculoId, mes = null, limite = 200) {
  let q = supabase.from('peaje_transacciones')
    .select('id, fecha, caseta, monto, tag')
    .eq('vehiculo_id', vehiculoId).eq('tipo', 'PASE')
...
export async function pasesPeajeTag(tag, limite = 200) {
  const { data, error } = await supabase.from('peaje_transacciones')
    .select('id, fecha, caseta, monto, mes')
    .eq('tag', tag).eq('tipo', 'PASE')
    .order('fecha', { ascending: false }).limit(limite);
  if (error) throw error;
  return data || [];
}
...
// ------------------------------------------------------------
// Import del .xlsx del portal
// ------------------------------------------------------------
// `filas` viene de leerArchivo() de lib/imports.js, con las claves en minúscula.
// Devuelve un resumen para mostrar en pantalla ANTES de dar por bueno el import.
export async function importarPeajes({ filas, archivo, usuario }) {
  const vehiculos = await listarVehiculos();
  const porPlaca = {};
  vehiculos.forEach(v => { if (v.placa) porPlaca[normPlaca(v.placa)] = v; });

  const tagsPrevios = await listarPeajeTags();
  const previo = {};
  tagsPrevios.forEach(t => { previo[t.tag] = t; });

  // --- normalizar las filas del portal ---
  const tx = [];
  for (const f of filas) {
    const id = Number(f.id);
    const crudo = f.fecha;
    if (!id || !crudo) continue;
    const fecha = crudo instanceof Date ? crudo : new Date(crudo);
    if (isNaN(fecha.getTime())) continue;
    const tipo = String(f.tipo || 'PASE').toUpperCase();
    const monto = Math.abs(Number(f.monto) || 0);   // el gasto se guarda positivo
    const tag = String(f.tag || '').trim();
    if (!tag) continue;
    const mes = `${fecha.getFullYear()}-${String(fecha.getMonth() + 1).padStart(2, '0')}`;
    tx.push({ id, fecha: fecha.toISOString(), mes, tag, caseta: f['estación'] || f.estacion || '', monto, tipo });
  }
  if (!tx.length) throw new Error('El archivo no trae ningún movimiento legible.');

  // --- catálogo de tags: los nuevos entran como pendientes ---
  const tagsArchivo = [...new Set(tx.map(t => t.tag))];
  const nuevos = [];
  for (const tag of tagsArchivo) {
    if (previo[tag]) continue;
    nuevos.push({ tag, estado: 'pendiente', updated_at: new Date().toISOString() });
  }
  if (nuevos.length) {
    const { error } = await supabase.from('peaje_tags').upsert(nuevos, { onConflict: 'tag' });
    if (error) throw error;
  }

  // --- resolver a qué vehículo va cada tag ---
  // Cuando a un vehículo le cambian el tag, el viejo apunta al nuevo con
  // 'fusionar_con'. Se sigue la cadena ANTES de resolver, para que el consumo
  // histórico del tag viejo caiga en el mismo vehículo.
  const canonico = (tag) => {
    const visto = new Set();
    let t = tag;
    while (previo[t]?.fusionarCon && !visto.has(t)) {
      visto.add(t);
      t = previo[t].fusionarCon;
    }
    return t;
  };

  // Prioridad: lo que ya se decidió a mano > cruce por placa del portal.
  const resolver = (tag) => {
    const p = previo[tag];
    if (p?.vehiculoId) return p.vehiculoId;
    if (p?.estado === 'fuera_flota') return null;
    const v = porPlaca[normPlaca(p?.placaPortal)];
    return v ? v.id : null;
  };

  const destino = {};
  for (const tag of tagsArchivo) destino[tag] = resolver(canonico(tag));
  // el que cruzó por placa queda asignado en el catálogo, sin intervención
  const autoasignados = [];
  for (const tag of tagsArchivo) {
    if (destino[tag] && !previo[tag]?.vehiculoId) {
      autoasignados.push({ tag, vehiculo_id: destino[tag], estado: 'asignado',
                           updated_at: new Date().toISOString() });
    }
  }
  if (autoasignados.length) {
    const { error } = await supabase.from('peaje_tags').upsert(autoasignados, { onConflict: 'tag' });
    if (error) throw error;
  }

  tx.forEach(t => { t.vehiculo_id = destino[t.tag] || null; });

  // --- guardar: upsert por ID del portal, así reimportar no duplica ---
  const LOTE = 500;
  for (let i = 0; i < tx.length; i += LOTE) {
    const { error } = await supabase.from('peaje_transacciones')
      .upsert(tx.slice(i, i + LOTE), { onConflict: 'id' });
    if (error) throw error;
  }

  // --- periodos ---
  const meses = {};
  tx.filter(t => t.tipo === 'PASE').forEach(t => {
    meses[t.mes] = meses[t.mes] || { filas: 0, total: 0 };
    meses[t.mes].filas += 1;
    meses[t.mes].total += t.monto;
  });
  const periodos = Object.entries(meses).map(([mes, v]) => ({
    id: `pp_${mes}`, mes, archivo: archivo || null,
    filas: v.filas, total: v.total,
    subido_por_id: usuario?.id || null, subido_por: usuario?.nombre || null,
  }));
  if (periodos.length) {
    const { error } = await supabase.from('peaje_periodos').upsert(periodos, { onConflict: 'mes' });
    if (error) throw error;
  }

  const sinVehiculo = tagsArchivo.filter(t => !destino[t] && previo[t]?.estado !== 'fuera_flota');
  return {
    movimientos: tx.length,
    pases: tx.filter(t => t.tipo === 'PASE').length,
    meses: Object.keys(meses).sort(),
    total: Object.values(meses).reduce((a, v) => a + v.total, 0),
    tagsNuevos: nuevos.map(n => n.tag),
    autoasignados: autoasignados.length,
    sinVehiculo,
  };
}

// ------------------------------------------------------------
// Resolver un tag pendiente
// ------------------------------------------------------------
// Al asignar se reescriben TODAS las transacciones del tag, también las viejas:
// el historial del vehículo aparece completo, no solo de hoy en adelante.
export async function asignarTagAVehiculo(tag, vehiculoId, usuario) {
  const { error } = await supabase.from('peaje_tags').upsert({
    tag, vehiculo_id: vehiculoId, estado: 'asignado', nota: null,
    resuelto_por_id: usuario?.id || null, resuelto_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  }, { onConflict: 'tag' });
  if (error) throw error;

  const e1 = await supabase.from('peaje_transacciones')
    .update({ vehiculo_id: vehiculoId }).eq('tag', tag);
  if (e1.error) throw e1.error;

  const e2 = await supabase.from('vehiculos')
    .update({ tag_peaje: tag, updated_at: new Date().toISOString() }).eq('id', vehiculoId);
  if (e2.error) throw e2.error;
}

// Asigna el tag al carro personal de un empleado. Si el vehículo no existe en
// el ERP se crea, marcado como particular: cuenta como gasto de la empresa pero
// no como gasto de flota, y no aparece en los reportes de flota operativa.
export async function asignarTagAPersona({ tag, personaId, vehiculoId, datos = {}, usuario }) {
  let vid = vehiculoId;
  if (!vid) {
    const v = await crearVehiculo({
      marca: datos.marca || '', modelo: datos.modelo || '', placa: datos.placa || '',
      color: datos.color || '', anio: datos.anio || null,
      responsableId: personaId, esParticular: true, tagPeaje: tag,
      notas: datos.notas || 'Carro personal. El peaje lo cubre la empresa.',
    });
    vid = v.id;
  } else {
    const { error } = await supabase.from('vehiculos')
      .update({ responsable_id: personaId, es_particular: true, updated_at: new Date().toISOString() })
      .eq('id', vid);
    if (error) throw error;
  }
  await asignarTagAVehiculo(tag, vid, usuario);
  return vid;
}

// Gasto de peaje por persona (solo carros particulares). Sustenta el beneficio.
export async function consumoPeajePorPersona(mes = null) {
  let q = supabase.from('peaje_consumo_persona').select('*');
  if (mes) q = q.eq('mes', mes);
  const { data, error } = await q;
  if (error) throw error;
  return (data || []).map(r => ({
    personaId: r.persona_id, mes: r.mes,
    pases: Number(r.pases || 0), monto: Number(r.monto || 0),
    vehiculos: Number(r.vehiculos || 0),
  }));
}

export async function marcarTagFueraDeFlota(tag, nota, usuario) {
  if (!nota || !nota.trim()) throw new Error('Hay que decir por qué queda fuera de flota.');
  const { error } = await supabase.from('peaje_tags').upsert({
    tag, vehiculo_id: null, estado: 'fuera_flota', nota: nota.trim(),
    resuelto_por_id: usuario?.id || null, resuelto_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  }, { onConflict: 'tag' });
  if (error) throw error;
  const e1 = await supabase.from('peaje_transacciones')
    .update({ vehiculo_id: null }).eq('tag', tag);
  if (e1.error) throw e1.error;
}

// Unificar una placa que no coincide entre el ERP y el portal.
// `placaFinal` es la que se queda; se graba en el ERP y se engancha el tag.
export async function unificarPlacaVehiculo({ tag, vehiculoId, placaFinal, usuario }) {
  const { error } = await supabase.from('vehiculos')
    .update({ placa: placaFinal, updated_at: new Date().toISOString() }).eq('id', vehiculoId);
  if (error) throw error;
  await asignarTagAVehiculo(tag, vehiculoId, usuario);
}

// Sugiere a qué vehículo del ERP podría pertenecer un tag sin identificar.
// Cruza por marca/modelo cuando la placa no coincide — el caso "misma guagua,
// placa distinta en cada sistema", que hay que unificar.
export function sugerirVehiculoParaTag(tagInfo, vehiculos) {
  const txt = `${tagInfo.nombrePortal || ''}`.toLowerCase();
  if (!txt) return [];
  const palabras = txt.split(/[^a-z0-9]+/).filter(p => p.length > 2);
  return vehiculos
    .map(v => {
      const cand = `${v.marca || ''} ${v.modelo || ''}`.toLowerCase();
      const puntos = palabras.filter(p => cand.includes(p)).length;
      return { vehiculo: v, puntos };
    })
    .filter(x => x.puntos > 0)
