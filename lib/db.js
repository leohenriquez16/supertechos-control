...
export async function validarRequisitosCambioEstado(proyectoId, estadoNuevo) {
  if (estadoNuevo !== 'planificado' && estadoNuevo !== 'en_ejecucion') return;

  const { data: p } = await supabase
    .from('proyectos')
    .select('ubicacion_lat, ubicacion_lng, contacto_principal_id, supervisor_id, maestro_id, fecha_estimada_inicio')
