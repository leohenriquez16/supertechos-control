...
export function construirCorreoTorre({ ayer, activos, atascados, enSla, cuello, nuevos, movidos, enviados, nombreProy, totalMes, totalAnio, ciclo = null, sinEnviar = [], reclam = null }) {
  ...
  const filasReclam = reclam ? reclam.atascadas.map(({ r, sla }) => `
    <tr>
      ${td(`<span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${semColR[sla.semaforo] || C.verde};margin-right:6px"></span><b>${esc(r.cliente_nombre || r.codigo || 'Sin cliente')}</b>${r.codigo ? `<span style="color:${C.gris};font-size:11px"> · ${esc(r.codigo)}</span>` : ''}`)}
    ...
    <div style="margin-top:26px;border-top:3px solid ${C.marca};padding-top:14px">
      ...
      ${reclam.sinInforme.length ? `
        <div style="background:#ecfdf5;border:1px solid #a7f3d0;border-left:3px solid ${C.verde};border-radius:10px;padding:12px 14px;margin:14px 0 4px">
          ...
          <ul style="margin:0;padding-left:18px;font-size:12px;color:#064e3b">${reclam.sinInforme.map(({ r }) => `<li style="margin:2px 0"><b>${esc(r.cliente_nombre || r.codigo || 'Sin cliente')}</b>${r.codigo ? ` · ${esc(r.codigo)}` : ''}</li>`).join('')}</ul>
        </div>` : ''}
      ...
      <table width="100%" cellspacing="0" cellpadding="0"><tr>
        <td width="50%" valign="top" style="padding-right:10px">
          ...
          <ul style="margin:0;padding-left:18px;font-size:12px;color:#444">${reclam.nuevas.slice(0, 12).map((r) => li(`${esc(r.cliente_nombre || r.codigo || 'Sin cliente')}${r.severidad ? ` · ${esc(r.severidad)}` : ''}`)).join('') || '<li style="color:#aaa">—</li>'}</ul>
        </td>
        <td width="50%" valign="top" style="padding-left:10px">
          <h4 style="margin:16px 0 6px;color:${C.verde};font-size:13px">📄 Informes entregados ayer (${reclam.informesAyer.length})</h4>
          <ul style="margin:0;padding-left:18px;font-size:12px;color:#444">${reclam.informesAyer.slice(0, 12).map((r) => li(esc(r.cliente_nombre || r.codigo || 'Sin cliente'))).join('') || '<li style="color:#aaa">—</li>'}</ul>
        </td>
      </tr></table>
