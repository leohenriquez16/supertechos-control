...
function toCSV(facturas, catPorId = {}) {
  const headers = [
...
export async function generarZipFacturasOdoo({ facturas, categorias = [], onProgreso }) {
  ...
  if (st.length) zip.file('facturas-super-techos.csv', toCSV(st, catPorId));
  if (pg.length) zip.file('facturas-prouco.csv', toCSV(pg, catPorId));
  if (sin.length) zip.file('facturas-sin-empresa.csv', toCSV(sin, catPorId));
