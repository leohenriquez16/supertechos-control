...
export const APP_VERSION = '8.55.0';
// v8.55.0: [Rendimiento en campo · Fase A — carga perezosa de vistas] app/page.jsx cargaba las 52
//     vistas de golpe (72 imports estáticos): el celular bajaba 915 kB al abrir el ERP. Ahora 51
//     vistas/modales/tabs condicionales se cargan BAJO DEMANDA (helper cargarVista sobre
//     next/dynamic) y el cascarón (Sidebar, Toasts, aterrizajes, primitivas, y 7 componentes que
//     igual viajan en el bundle inicial por imports transitivos) queda estático. First Load de '/'
//     baja de 915 kB a ~560 kB (−39%). Resiliencia en campo: si un chunk no baja (señal caída o
//     versión nueva tras un deploy) se muestra "No se pudo cargar · Reintentar" en vez de tumbar la
//     app (antes no había error boundary); modales usan fallback overlay (CargandoModal) sin salto de
//     layout; nuevo app/error.jsx global con "Actualizar app". 2 imports muertos eliminados.
//     Clasificación con doble clasificador + juez y revisión adversaria (3 lentes + refutadores);
//     transformación por script determinista. Fase B (rutas reales → deep links) pendiente.
// v8.54.2: [Comunicación · C2 — Respuestas del cliente (WhatsApp entrante)] El webhook de WhatsApp
//     ahora, cuando el cliente responde y tiene una reclamación ABIERTA reciente (≤7 días), PEGA el
//     mensaje a esa reclamación en vez de crear una nueva (antes duplicaba con cada mensaje). Todo
//     mensaje entrante y el acuse automático saliente quedan en la BITÁCORA de comunicación del
//     ticket (canal whatsapp). Así el ida-y-vuelta con el cliente queda visible en el histórico.
//     Pendiente C2b: correo entrante (depende del agente IMAP local ~/correos-agente escribiendo a
//     Supabase). C4 se integra dentro de la Torre (CSAT ya está; falta última comunicación / sin responder).
// v8.54.1: [Comunicación · C3 — Retroalimentación del cliente (CSAT)] Cada reclamación y
//     levantamiento puede pedir una CALIFICACIÓN al cliente: se genera un link público
//     /calificar/<token> (página branded sin login, 1-5 estrellas + comentario) para compartir
//     por WhatsApp/correo. La respuesta se guarda, aparece en el ticket y en la BITÁCORA (entrante)
//     y alimenta el indicador CSAT dentro de la Torre de Control (no un tablero aparte). Migración
//     135: tabla retroalimentacion_cliente. API pública /api/calificar/[token] (service_role).
//     Componente SolicitarCalificacion en el detalle de reclamación y levantamiento.
// v8.54.0: [Comunicación con el cliente · C1 — Bitácora unificada] El chatter de cada
//     reclamación/levantamiento pasa a ser la BITÁCORA DE COMUNICACIÓN: además de notas internas
//     y eventos, se registra la comunicación con el cliente con CANAL (WhatsApp/Llamada/Correo/
//     Presencial) y DIRECCIÓN (enviado al cliente / recibido del cliente), visible en la línea de
//     tiempo con badges. Se registra SOLO al abrir WhatsApp (reclamaciones y levantamientos) y al
//     entregar el informe de solución. Migración 134: chatter_mensajes.canal/direccion. Helper
//     registrarComunicacion() en lib/chatter.js. Base para C2 (respuestas del cliente), C3 (CSAT)
//     y C4 (tablero de comunicación).
// v8.53.4: [Reclamaciones · contacto amarrado a contactos del cliente] La reclamación ahora
//     referencia a un CONTACTO del cliente (tabla contactos, la misma de los proyectos):
//     - EMPRESA → contacto OBLIGATORIO elegido de sus contactos, con WhatsApp o correo (selector
//       en Nueva reclamación + panel para asignar/cambiar en el detalle). Sin contacto no se crea.
//     - PERSONA → el cliente ES el contacto (su propio teléfono/correo); no se asigna contacto.
//     La Torre y el correo resuelven el contacto por contacto_id (con respaldo ubicación/cliente)
//     y marcan "sin contacto localizable" y "falta asignar contacto". Helper puro
//     lib/helpers/contactoCliente.js. Migración 133: reclamaciones.contacto_id.
// v8.53.3: [fix Torre · Reclamaciones sin nombre] La pestaña Reclamaciones y el correo mostraban
//     "Sin cliente" en casi todas: leían el campo cliente_nombre (casi siempre vacío) en vez de
//     resolver el nombre por cliente_id contra la tabla clientes. Ahora la Torre resuelve el
//     nombre por cliente_id (o el cliente del proyecto) y muestra la UBICACIÓN del cliente
//     (cliente_ubicaciones) a la que está amarrada la reclamación, en la vista y en el correo.
//     TorreControl pasa `data` (clientes/proyectos) a TorreReclamaciones; el cron resuelve
//     cliente + ubicación server-side. Además muestra la PERSONA DE CONTACTO del cliente
//     (WhatsApp/correo, de la ubicación o el cliente) y marca las reclamaciones SIN CONTACTO
//     localizable (ni WS ni correo) en la vista y el correo, para poder dar seguimiento.
// v8.53.2: [Torre de Control · Fase 3B-2 — Reclamaciones en el correo diario] El correo de las
//     7 AM ahora trae, además de Levantamientos, una sección de RECLAMACIONES: activas, vencidas/
//     en riesgo (tabla con severidad/estado/antigüedad vs SLA), RESUELTAS SIN INFORME entregado,
//     nuevas de ayer, informes entregados ayer y tiempo de ciclo del mes (creada → informe
//     entregado). Mismos destinatarios (Edwin, CC Miguel/Leo). Cierra la Fase 3.
// v8.53.1: [Torre de Control · Fase 3B-1 — Reclamaciones] Se replica la Torre para RECLAMACIONES
//     (de creada → el cliente recibe el informe de solución) y se agrega el HITO que faltaba:
//     "Informe de solución entregado al cliente" (fecha + PDF del informe), en el detalle de cada
//     reclamación (panel InformeSolucionReclamacion, sube el PDF al bucket documentos-fotos). SLA
//     POR SEVERIDAD (propuesto): alta 48h · media 5d · baja 10d, medido de la apertura a la entrega
//     del informe. Motor puro lib/helpers/slaReclamaciones.js. Nueva pestaña "Reclamaciones" en la
//     Torre de Control (TorreReclamaciones): activas/vencidas/en riesgo, sub-alerta "RESUELTAS SIN
//     INFORME entregado" (arreglo hecho pero el cliente no lo tiene) y tiempo de ciclo del mes.
//     Migración 132: reclamaciones.informe_entregado_at / informe_url / informe_nombre.
//     Pendiente Fase 3B-2: sección de reclamaciones en el correo diario 7 AM.
// v8.53.0: [Torre de Control · Fase 3A — cerrar el embudo con Odoo] La Torre ahora lee el
//     ESTADO REAL de la cotización en Odoo (draft|sent|sale) por referencia_odoo de los sites,
//     tanto en la vista viva (nueva ruta /api/torre-control/estado-odoo) como en el correo diario.
//     (1) FUGA detectada: levantamientos marcados "Cotización Realizada" pero cuya cotización
//     sigue en BORRADOR en Odoo (nunca enviada al cliente) → alerta en la vista y sección en el
//     correo; el embudo no cuenta como cerrado hasta 'sent'/'sale'. (2) TIEMPO DE CICLO del mes
//     (recepción del formulario → cotización enviada): mediana, promedio, % dentro de 72h y #
//     cotizadas, en la vista y el correo. Motor slaLevantamiento.js gana odooCotState/cotizadaSinEnviar/
//     enviadaOdoo + metricasCiclo(). El auto-cierre desde Odoo ya existía (sincronizarLevantamientosOdoo).
// v8.52.4: [Tickets Gotera 8-sep] (1) Reclamaciones (Edwin): "Bucket not found" al
//     subir fotos — el bucket `documentos-fotos` se usaba desde v8.39 pero nunca se
//     creó en prod (migración 20260910120000, ya aplicada). (2) Almacén (Erisdania):
//     nuevos estados de compra por renglón para lo que NO está en almacén pero no
//     pasa por Compras — 🚚 "se compra en la ruta" y 👷 "maestro lo compra" (caja
//     chica); ninguno dispara el correo a Compras.
// v8.52.3: [fix cron Torre] el correo diario ahora lee solicitudes_levantamiento con
//     SUPABASE_SERVICE_ROLE_KEY (esa tabla tiene RLS) — antes con la llave anon los
//     tickets nuevos y los totales del mes/año salían vacíos.
// v8.52.2: [Torre de Control · correo Fase 2 rediseñado] Correo diario más bonito
//     (encabezado de marca + tarjetas de resumen), tickets NUEVOS con toda su info
//     (tipo servicio/inmueble, área, urgencia, locación, teléfono), INFO DEL PROYECTO
//     junto al cliente en cada fila, y pie con TICKETS DEL MES y DEL AÑO. HTML extraído
//     a lib/helpers/torreControlEmail.js (compartido con la vista previa).
// v8.52.1: [Torre de Control · Fase 2] Correo diario 7:00 AM (cron torre-control-diario,
//     Resend) a Edwin (comercial), CC Miguel Martínez y Leonardo: ATASCADOS (🟡/🔴) con
//     etapa/antigüedad/responsable, cuello de botella, y nuevos/movidos/enviados de ayer.
//     Schedule en vercel.json (0 11 * * * = 7 AM RD). Reusa el motor slaLevantamiento.
// v8.52.0: [Torre de Control de Levantamientos · Fase 1] Vista nueva (menú COMERCIAL,
//     admin/supervisor) con el embudo del formulario → cotización: cada levantamiento
//     con su etapa, ANTIGÜEDAD en la etapa y SEMÁFORO SLA. Política: meta 72h de tiempo
//     controlable; 🔵 pausa cuando la fecha de visita la puso el cliente (visita_por_cliente);
//     sistema complejo → consulta técnica 24h (requiere_consulta_tecnica). Motor puro
//     lib/helpers/slaLevantamiento.js. Migración 131: surveys.stage_history (historial de
//     cambios de etapa, para medir tiempo por etapa) + campos del stop-the-clock/complejos;
//     setEtapaSurvey/finalizar/reabrir ahora registran cada transición. Pendiente Fase 2
//     (correo diario 7 AM a Edwin, CC Miguel/Leo) y Fase 3 (cerrar con cotización 'enviada' Odoo).
// v8.51.6: [Dashboard · KPI de actividad] Tarjeta nueva arriba del Dashboard:
//     "Levantamientos hechos" (survey_status completed/validated) y "Reclamaciones
//     atendidas" (estado ≠ abierta), con toggle Hoy / Semana / Mes. Derivado de datos
//     (db.metricasActividad, últimos 40 días) — sin reportes manuales.
// v8.51.5: [Vehículos · selector] Arriba de las fichas, un selector para elegir un
//     vehículo (abre su ficha y filtra la lista a ese) o "Todos (ver la lista)". El
//     clic en el card para abrir la ficha y el modal ancho ya estaban en 8.51.x.
// v8.51.4: [Rutas · Lugares] El alta de suplidores, sus locaciones y los lugares
//     frecuentes ahora aceptan links CORTOS de Google Maps (maps.app.goo.gl) — se
//     expanden con expandirYExtraer antes de fallar (igual que Proyectos y
//     Levantamientos). Antes daba "No entendí la ubicación" con el link corto.
// v8.51.3: [Bombas para el chofer + alcance de Flota] ⛽ Las bombas de la tarjeta
//     flotilla (TotalEnergies, ordenadas por cercanía, Waze directo) ahora las ve
//     el que MANEJA: botón en "Mi vehículo" (responsables) y en el inicio del
//     chofer. Se RETIRA el menú "Flota" del equipo de campo (decisión Leonardo:
//     cada quien ve SOLO su vehículo; la flota completa es de oficina —
//     módulo Vehículos de admin/almacén). + DOCUMENTOS del vehículo en "Mi
//     vehículo": el responsable abre la matrícula y el seguro desde su celular
//     (para cuando AMET/DIGESETT los pida).
// v8.51.2: [Guardrail nombre de proyecto] El campo "Proyecto" (nombre) se normaliza al
//     guardar/importar: quita el prefijo de código (ST/PG-Cxxxx -), colapsa espacios y,
//     si queda vacío o es solo un teléfono/número, cae al nombre del cliente. Choke point
//     en crearProyecto/actualizarProyecto (lib/db.js) → cubre form e import de Odoo.
//     Helper puro lib/helpers/proyectos.js. (Limpieza de 28 filas existentes ya aplicada en prod.)
// v8.51.0: [Protocolo de averías de vehículos] Caso disparador: un vehículo se
//     sobrecalentó sin proceso escrito. (1) 🚨 "Se dañó / avería" en Mi Vehículo:
//     flujo guiado que muestra PRIMERO el protocolo del síntoma (qué hacer / qué
//     NO — sobrecalentamiento, frenos, goma, no enciende, choque, eléctrico,
//     ruido) y captura gravedad, odómetro, fotos y GPS. Crítica ⇒ vehículo FUERA
//     DE SERVICIO automático (Rutas no lo ofrece) + correo a gerencia. (2) Ciclo
//     de taller trazable: cierre exige diagnóstico/causa raíz, costo, taller y
//     CHECKLIST de retorno a servicio; al cerrar la última crítica el vehículo se
//     reactiva. (3) Inspección mensual con CHECKLIST de niveles obligatorio
//     (coolant, aceite, frenos, correas, gomas, luces, fugas) — ítem en "malo"
//     abre el evento solo. (4) 🖨 Protocolo imprimible para la guantera.
//     (5) Vista FLOTA para TODO el equipo (solo lectura): estado en vivo de cada
//     vehículo (disponible / en taller / fuera de servicio), por qué y desde
//     cuándo — caso "¿está de mantenimiento el KIA?" por WhatsApp. Jonathan
//     registra mantenimiento con "está en el taller AHORA" y el vehículo queda
//     EN TALLER visible para todos. (6) SOLICITUD DE MANTENIMIENTO → CITA (caso
//     Wilfin/KIA): el responsable la pide desde "Mi vehículo" (correo a Erisdania
//     y Miguel), cae a Mis Pendientes → Vehículos, la oficina la programa con
//     "📅 Programar cita" (fecha + taller) y el responsable la ve en su celular.
//     Ciclo: solicitado → 🗓 cita → en taller → resuelto.
//     lib/protocolosAverias.js + migración 130.
// v8.50.6: [Tickets Gotera 7-sep] (1) El SUPERVISOR puede tener su propio modo de
//     pago en la obra (p.ej. POR DÍA) aunque haya varios maestros — entra a la
//     sección "Modo de pago por persona" del editor (antes solo brigadas; caso
//     Osman, ticket Miguel M.). También entran los maestros de brigadas v8.27.22
//     que nunca se incluyeron. Su fila ya no se oculta con el filtro "solo
//     maestros" de la nómina. (2) Nómina: EXCLUIR DEL PAGO cualquier reporte del
//     corte a mano (buscador por obra) con MOTIVO OBLIGATORIO y constancia
//     (migración 129) — para trabajo pagado por adelantado que el detector de
//     duplicados no ve (caso Escuela Despertar). El reporte excluido sigue
//     contando avance y producción.
// v8.50.2: [Tickets Gotera 4-sep] (1) Caja Chica: PROYECTO OBLIGATORIO al reportar
//     gastos (bloqueante Felvison) — modal simple, sin-factura masivo y masivo; sin
//     opción "genérico" (queda "Otro proyecto + Nº cotización"; el modal de admin
//     conserva el escape). (2) Rutas: cuenta como chofer el RESPONSABLE de un vehículo
//     activo aunque no tenga el rol (maestros que llevan materiales — Erisdania).
//     (3) Almacén: 🛒 SOLICITUD DE COMPRA interna sin obra (tipo 'compra', renglones
//     nacen "solicitado a compras", modo retiro, no ensucia Rutas) + FIX: la alerta
//     por correo a compras nunca disparaba (comparaba 'comprar' que no existe).
//     (4) Reclamaciones (Edwin): cambiar estado ahora SÍ mueve la tarjeta (sincroniza
//     odoo_stage) y el drag&drop del Kanban quedó funcional. (5) Reportes: modo
//     🔧 REPARACIÓN/RETOQUE (Miguel M.) — no suma avance, producción, rentabilidad ni
//     nómina (excluir_nomina + datos_ia.reparacion; simple y lote). (6) Garantías:
//     CARTA DE GARANTÍA en PDF con membrete (Miguel M.) — desde la ficha de la garantía
//     y desde Info del proyecto tras "Recibido conforme"; lee la garantía real creada
//     al cambiar de estado (duración, cobertura, condición, mantenimientos obligatorios).
// v8.49.0: [Personal / RR.HH.] Ficha de personal con MEDIDOR DE COMPLETITUD (% + qué
//     campos faltan) y filtros "Solo activos" / "Fichas incompletas" en la lista. El
//     editor de perfil (MiPerfil) ahora captura TODOS los datos RR.HH. que antes solo
//     salían en el onboarding: WhatsApp, fecha nacimiento, tipo de sangre, contacto de
//     emergencia, puesto/departamento y datos bancarios. Nuevo flag `activo` (migración
//     128: marca inactivo al ayudante puro sin PIN y sin jornada en 60 días → 29
//     inactivos). Menú reorganizado: nueva sección PERSONAL/RR.HH. (Personal, Equipo en
//     obra, Estadísticas, Nómina, Bonos, Carga); Clientes/Ubicaciones→Comercial;
//     Garantías→Operación; Configuración solo catálogos. Manual IA actualizado.
// v8.48.0: [Almacén · Alistos: despachos y retiros] El almacén ahora alista no solo
//     pedidos de OBRA sino DESPACHOS/VENTAS de material a un cliente. Botón "Nuevo
//     despacho" (almacén/admin) con MODO DE ENTREGA: 🚚 envío (cae a Rutas como hoy)
//     o 🙋 retiro (el cliente pasa a buscarlo → se firma en el almacén con nombre,
//     cédula, firma y foto opcional; NO va a Rutas). Se extiende `requisiciones`
//     (migración 127: proyecto_id nullable + tipo/modo_entrega/cliente_nombre/
//     referencia + campos de sign-off de retiro). El ERP NO escribe a Odoo: la salida
//     real de inventario se valida allá aparte (regla read-only). Manual IA actualizado.
// v8.47.0: [Rentabilidad · salidas de Odoo + fixes] (1) El REAL de materiales ahora
//     hala las SALIDAS DE ALMACÉN de Odoo (delivery orders done por referencia de
//     cotización, endpoint /api/odoo/salidas-almacen) matcheadas a los materiales del
//     sistema por keywords_odoo/nombre — en Dalan: 397 rollos despachados. (2) La
//     proyección de una línea sin gasto registrado ya NO cae a 0 (mantiene el ppto);
//     materiales con PISO en el presupuesto y subida por ritmo real. (3) MDO proyecta
//     su valor contractual. (4) Caja chica: fix clasificación (leía datosIa pero el
//     mapper entrega datosIA; + keywords gasoil/avitación) y los gastos de caja que
//     son un material del presupuesto (gas propano) van a esa línea — sin doble conteo.
//     Aviso de salidas Odoo sin matchear para mapear con keywords.
// v8.46.2: [Rentabilidad · semáforo 4 bandas] Escala de margen definida por Leonardo:
//     🔴 debajo de 30% · 🟠 30-35% · 🟢 35-45% · ⭐ +45% (con objetivo global 30%).
//     Las bandas se anclan al objetivo del sistema (rojo bajo objetivo, naranja +5,
//     verde +15, estrella de ahí en adelante). Aplica por partida, por obra y en el
//     portafolio (leyenda en el header).
// v8.46.1: [Rentabilidad · acceso y objetivo] (1) Todo lo de rentabilidad (tab de la
//     ficha + portafolio Finanzas) SOLO lo ven el owner y quien tenga el nuevo rol
//     'rentabilidad' (asignable en Personal; para Felvison y Miguel Martínez) — ya no
//     todo admin. (2) Margen objetivo POR SISTEMA: campo "Margen objetivo %" en la
//     ficha del sistema (vacío = usa el global de Configuración); el semáforo por
//     partida usa el del sistema y el de la obra/portafolio el promedio ponderado por
//     venta. (3) Portafolio: esconder obras (ojito por fila, chip "N ocultas",
//     preferencia por usuario en localStorage) en lo que ponen su presupuesto al día.
// v8.46.0: [Rentabilidad por obra] Presupuesto de costos POR PARTIDA por obra
//     (tabla presupuestos_obra, migración 126): se genera desde los costos por
//     sistema + los precios de MDO cuadrados con el maestro (precios_tareas_m2),
//     editable en borrador y CONGELADO con versionado al aprobar (solo 1 vigente).
//     Materiales sin costo quedan "por definir" (manual, en lo que Odoo se pone
//     a la par). La pestaña "Rentabilidad" reemplaza TabCosto en la ficha: ppto |
//     real | proyección por partida (venta neteada de ITBIS), MDO REAL desde
//     nómina (salda el TODO del TabCosto viejo), caja chica clasificada y compras
//     facturadas por cuenta analítica de Odoo como referencia (endpoint nuevo
//     /api/odoo/costos-analitica). + Portafolio "Rentabilidad" en FINANZAS (owner):
//     todas las obras con margen proyectado y semáforo vs margen_objetivo_pct.
// v8.45.2: [Ubicación desde el cliente] Las locaciones del cliente también se
//     ubican con CLIC EN EL MAPA (además del link de Maps/WhatsApp): mapa
//     interactivo dentro del modal de locación, y chip "📍 sin GPS — asignar"
//     en la lista de ubicaciones del cliente que abre el modal con el mapa.
// v8.45.1: [Ubicación desde el detalle] La ubicación también se asigna/corrige
//     DENTRO del levantamiento: botón "📍 Asignar / ✎ Corregir ubicación" en la
//     sección Ubicación, y el recuadro "Sin ubicación" y el aviso ámbar abren el
//     mismo modal (mapa o link de Google Maps → locación del cliente).
// v8.45.0: [Ubicación de levantamientos] Levantamiento SIN ubicación ⇒ chip
//     "📍 sin ubicación" en la tarjeta: se asigna con CLIC EN EL MAPA o pegando
//     el link de Google Maps. La ubicación NACE COMO LOCACIÓN DEL CLIENTE
//     (cliente_ubicaciones — Cliente 360) y queda amarrada al site
//     (ubicacion_id), lista para proyectos, garantías, mantenimientos y
//     reclamaciones. Sin migraciones (modelo de la 040).
// v8.44.1: [Fix] El rol ALMACÉN ve su módulo "Almacén" con nombre propio en el
//     menú (antes solo "Inicio") y chofer/almacén aterrizan en su inicio al
//     restaurar sesión.
// v8.44.0: [Vehículos completos] FICHA COMPLETA (clic en el nombre): toda la info
//     explícita con vencimientos en color, chofer y licencia, 🛡 GARANTÍA
//     (vence/km/casa — vigente ⇒ "mantenimiento en la casa" en ficha y tarjeta) y
//     GPS. "GPS en vivo" funciona con la UNIDAD amarrada (mapa propio, sin
//     iframe). Tab Flota GPS abre con el MAPA EN VIVO de toda la flota y los
//     marcadores son MINI-VEHÍCULOS a escala por tipo (también en Rutas y en el
//     mapa del chofer): 🟢 andando · 🟡 encendido detenido · 🔴 apagado.
//     Migración 125 (garantía).
// v8.43.1: [Legibilidad] Inputs bg-zinc-950 (se fundían con los modales), labels
//     más claros, date/time en color-scheme dark, modal de vehículo aireado.
// v8.43.0: [GPS en vivo] Conexión al API de Pressto (GPSWOX): camiones EN VIVO en
//     el mapa de Rutas, unidad GPS por vehículo (migración 124), "🛰 Tu camión"
//     en la ruta del chofer. lib/gps.js + /api/gps/posiciones (GPS_API_URL/KEY).
// v8.42.0: [Entrega con prueba] GPS OBLIGATORIO en toda parada (obras sin
//     ubicación no se montan; lugar libre exige link de Google Maps) — el chofer
//     ve claro a dónde va. Parada de RECOGIDA acepta DOCUMENTO adjunto (OC /
//     cotización / factura, imagen o PDF) que el chofer muestra al retirar
//     ("📄 Ver documento" en su celular). Y al ENTREGAR en la obra: PRUEBA DE
//     ENTREGA obligatoria — foto del material + FIRMA de quien recibe (pad táctil
//     en el celular del chofer) + nombre; queda visible en Rutas (📷/✍️/recibió X).
//     Migraciones 122 (doc parada) y 123 (prueba de entrega).
// v8.41.1: [Rutas más claras] Nuevo Viaje: se elige SOLO el vehículo de la flota
//     y el chofer responsable entra automático (link "cambiar" si hace falta);
//     tercer tipo "Viaje SUB-CONTRATADO". Nueva parada en flujo 1-2-3: ¿recoger,
//     entregar o RECOGER Y ENTREGAR? (el par crea las 2 paradas encadenadas, ej.
//     suplidor → obra) + buscador "¿dónde?" con SUGERENCIAS en vivo al escribir
//     (obras CON ubicación, suplidores con sus locaciones, puertos, almacenes;
//     texto libre también vale) + ¿qué lleva?.
// v8.41.0: [Suplidores + ruta en mapa + rutas por vehículo] SUPLIDORES como
//     entidad del ERP (espejo de Odoo: 🔗 vincular alinea nombre/RNC y guarda
//     odoo_partner_id) con UNA O VARIAS LOCACIONES; en la parada libre de Rutas
//     se elige el suplidor por nombre y, si tiene varias locaciones, cuál. El
//     CHOFER ve su RUTA DEL DÍA EN EL MAPA con pines numerados en orden y la
//     línea del recorrido (también en el mapa de Rutas para el viaje elegido).
//     Los viajes se amarran al VEHÍCULO REAL de la flota (select al crear) →
//     "🚚 Rutas del vehículo" (futuras y pasadas) en Vehículos y en Mi Vehículo.
//     + Usuario del chofer Miguel Ángel Ventura (Odoo) creado en prod.
//     NOTA: las locaciones de CLIENTE ya existían (cliente_ubicaciones v8.19.54,
//     amarradas a proyectos/levantamientos/garantías/mantenimientos/reclamaciones).
//     Migración 121 (suplidores, suplidor_locaciones, viajes.vehiculo_id).
// v8.40.0: [Diligencias + lugares] MATERIAL SOBRANTE: el maestro lo reporta desde
//     el tab Pedidos con FOTO (comprimida) y listado → queda como RETIRO "sin
//     planificar" en Rutas (recordatorio que no se pierde), se monta en un viaje
//     como recogida y se cierra solo cuando el chofer completa la parada. Rutas
//     además AVISA de los pedidos en preparación en almacén (🔔), tiene vista de
//     MAPA 🗺 (rojo sin asignar / naranja lista / azul en viaje / verde hecha) y
//     LUGARES FRECUENTES 📍 (suplidores, puertos, almacenes fiscales con coords
//     por link de Google Maps; Haina/Caucedo/Sans Soucí precargados) que
//     autollenan la parada libre. Migración 120.
// v8.39.0: [Compras en requisiciones] Renglón sin stock → estado de COMPRA por
//     item (🛒 solicitado a compras → 💲 cotizado → ⏳ esperando aprobación →
//     ✓ comprado) editable en Almacén y visible en la obra. El pedido puede nacer
//     como "🛒 Es una compra" con COTIZACIÓN adjunta (foto/PDF, storage), y con
//     un clic la IA la lee (Claude Vision) y crea la ORDEN DE COMPRA en BORRADOR
//     en Odoo (proveedor por RNC/nombre, productos purchase_ok por similitud, lo
//     no matcheado va a las notas de la OC; nunca se confirma sola). Lily (rol
//     facturas) gana "Almacén · Compras" en su menú. Migración 119.
// v8.38.0: [Desktop-first 3] ALMACÉN y RUTAS a DOS PANELES en lg+ — cola/viajes
//     compactos a la izquierda + detalle sticky a la derecha (checklist de despacho
//     y ruta con paradas siempre a la vista); auto-selección del primero; todo
//     táctil (iPad de Jonathan sin hover). Móvil idéntico. + FINANZAS densas:
//     Nómina (recibos 2-3 col, supervisores 2 col) y Caja Chica (bandeja 2 col,
//     por persona 2 col, por proyecto 2-3 col) — wrappers de presentación sobre
//     los mismos maps, sin tocar el motor de cálculo ni la aprobación.
// v8.37.1: [Vehículos] GPS en vivo (link Pressto embebido + refrescar + hora),
//     Chofer + LICENCIA de conducir con vencimiento (vive en la persona, se ve en
//     la ficha), y módulo de INSPECCIONES mensuales: captura guiada de 6 fotos
//     fijas (alante/lados/atrás/bonete/odómetro/interior), autorización remota
//     (solo Jhonathan) para que el chofer la haga desde el móvil, historial y
//     COMPARACIÓN POR AI de daños/limpieza/km. Migraciones 115-118.
// v8.37.0: [Tareas centro de trabajo] La vista deja de ser una lista plana:
//     (1) RAIL DE CONTEXTOS en desktop — Mis tareas / Superviso / Todas, cada
//     ESPACIO y cada OBRA con tareas, con contadores; el título y la lista siguen
//     al contexto elegido (3 paneles: contextos → lista → detalle). (2) Filtros
//     por responsable (todos) y por obra. (3) FECHAS modernas: modal con
//     calendario oscuro + HORA opcional + fecha PLANIFICADA (📌 cuándo piensas
//     hacerla) + atajos Hoy/Mañana/Próx. lunes — muere el prompt de texto.
//     (4) Detalle: responsable editable con select (avisa por correo), mover la
//     tarea de obra/espacio, e HISTÓRICO (quién la creó, a quién se asignó).
//     (5) Levantamientos: asignar es de Edwin (comercial) y SOLO se exige cuando
//     la visita está coordinada — sin cita no castiga el pendiente ni el KPI.
//     Migración 114 (hora_limite, fecha_planificada, historial).
// v8.36.0: [Desktop-first 2] DASHBOARD en grid de 12 columnas en lg+ (selector de
//     periodo junto a los 4 KPIs que controla; Importar Odoo junto a Tareas
//     pendientes a 2 col; alerta de atascados a 2 col) y DETALLE DE PROYECTO:
//     identidad a la izquierda + panel financiero Avance/Producido/Contrato a la
//     derecha, barra de tabs sticky arriba, Tab Info a 2 columnas (ubicación+mapa
//     junto a cliente/contactos/datos), CTA Reportar ya no cruza toda la pantalla.
//     TODO aditivo con clases lg: — móvil (campo) idéntico.
// v8.35.0: [Desktop-first 1] El ERP como plataforma de computadora (el campo no
//     cambia): (1) MENÚ admin reorganizado por ÁREAS del organigrama — Operación /
//     Comercial / Logística / Finanzas / Configuración / Seguridad (Vehículos pasa
//     a Logística; Levantamientos, Solicitudes y Citas a Comercial). (2) TAREAS
//     piloto de densidad: en pantalla grande la vista es de DOS PANELES — lista +
//     detalle sticky a la derecha (sin modal), fila seleccionada resaltada y
//     descripción visible en la fila; en celular igual que antes. (3) Vistas de
//     oficina liberan ancho en lg+ (Pendientes 2 columnas, Bonos/Almacén/Rutas a
//     6xl, Producción a 7xl, Solicitudes a 5xl).
// v8.34.0: [Tareas nivel Asana 4] ESPACIOS administrativos (proyectos internos):
//     proyectos NO-obra (cierre fiscal, implementación, traspasos) que agrupan
//     tareas con área, responsable, fecha meta y barra de progreso — SIN tocar
//     producción, bonos ni analíticas. Chips 📁 en Tareas, espacio en el modal de
//     crear (excluyente con la obra). + TAREAS RECURRENTES 🔁: obligaciones con
//     fecha fija (TSS, IR-17, 606/607, IT-1, nómina, cierres) que abren su tarea
//     SOLAS N días antes — mensual/quincenal/semanal, atajos del calendario
//     fiscal RD, se generan en el cron diario y al abrir Tareas (id determinístico
//     por ocurrencia = sin duplicados). Migración 113 (proyectos_internos,
//     tareas_recurrentes, tareas.proyecto_interno_id).
// v8.33.4: [Tareas nivel Asana 3] SUBTAREAS (checklist dentro de la tarea, con
//     progreso ☑ 2/5 en la fila) · LIKES 👍 (reconocer/agradecer una tarea) ·
//     PLANTILLAS de tareas por obra (lib/helpers/plantillasTareas.js): 🚀 Arranque
//     de obra, 🏁 Cierre y entrega, 💰 Seguimiento de cobro — se crean en lote
//     desde el tab Tareas del proyecto, asignadas al supervisor de la obra con
//     fechas escalonadas. Migración 112 (subtareas + likes jsonb).
// v8.33.3: [Tareas ↔ Proyectos] Tab "Tareas" en cada obra: las tareas amarradas
//     al proyecto se ven y se CREAN desde la obra misma (proyecto preseleccionado)
//     o desde el módulo Tareas eligiendo el proyecto — mismo dato, dos puertas.
//     Con responsable+supervisor, prioridad, fecha, delegar y aviso por correo.
// v8.33.2: [Tareas nivel Asana 2] Prioridad (🔥 alta/normal/baja, ordena dentro de
//     cada sección) · DETALLE de tarea al hacer clic (descripción completa,
//     prioridad editable, acciones) con COMENTARIOS (hilo 💬 por tarea) ·
//     "INBOX": correo automático al responsable cuando le asignan o delegan una
//     tarea (si su ficha tiene email) · el menú Tareas ahora es visible para
//     TODOS los miembros (antes solo aparecía si tenías tareas asignadas).
//     Migración 111 (tareas.prioridad + comentarios jsonb).
// v8.33.1: [Tareas estilo Asana] La vista de Tareas se rediseña como task manager:
//     secciones por vencimiento colapsables (Vencidas / Para hoy / Esta semana /
//     Más adelante / Sin fecha), buscador, filtros Me tocan / Superviso / Todas
//     (+ filtro por responsable para admin), filas limpias con acciones al hover
//     (completar, delegar ↪, fecha 📅, eliminar) y toggle de Completadas.
//     El componente sale del monolito a components/tareas/VistaTareas.jsx.
// v8.33.0: [Ficha vehicular completa + tareas con supervisor]
//     VEHÍCULOS: ficha ampliada (responsable, tipo, combustible, capacidad,
//     odómetro, vencimientos de placa/marbete y revisión, tag de peaje, estado
//     operativo, próximo mantenimiento) + LOG de eventos por vehículo
//     (mantenimientos, fallas mecánicas, choques, daños, gomas, inspecciones —
//     con costo, taller, estados abierto/en_taller/resuelto). "MI VEHÍCULO":
//     el responsable ve su ficha con alertas de vencimiento y reporta todo desde
//     ahí; el reporte cae al log que administra flota. Mis Pendientes suma el
//     grupo "Vehículos y flota" (eventos abiertos + vencimientos ≤15 días) y
//     FILTRO POR ÁREAS (Proyectos/Logística/Comercial/Postventa/Gerencia/Tareas).
//     TAREAS: además del responsable ahora llevan SUPERVISOR; filtros "Me tocan /
//     Superviso / Todas"; DELEGAR (quien delega queda de supervisor) y cambiar
//     fecha de vencimiento. Migración 110.
// v8.32.2: [Transportación] El rol `almacen` (Jonathan) ADMINISTRA los VEHÍCULOS
//     (flota, matrícula/seguro) — su menú suma Vehículos. La planificación de
//     RUTAS queda con Erisdania con apoyo de Miguel (ambos admin).
// v8.32.1: [Fix analíticas] El valor_cotizacion del ERP incluye ITBIS — la
//     comparación del cron ahora usa amount_total de Odoo (antes amount_untaxed
//     daba falsos descuadres de exactamente 18%). Además los proyectos con valor
//     0 en el ERP se listan aparte ("sin valor en el ERP — Odoo sí lo tiene") en
//     vez de contarse como descuadre. Tolerancia sube a ±2%.
// v8.32.0: [Analíticas Odoo ↔ Proyectos + sub-cotizaciones] Presupuesto 100% al
//     día entre el ERP y la cuenta analítica (Miguel + Felvison responsables).
//     - CONVENCIÓN DE NOMBRE: la analítica en Odoo se nombra EMPEZANDO con la
//       referencia de la cot ORIGINAL (ej. "ST-C1234 - Cliente") → el ERP la
//       encuentra y vincula solo (proyectos.analitica_odoo_id/nombre, migr. 109).
//     - SUB-COTIZACIONES: cotizaciones nuevas del mismo proyecto (ampliación,
//       etapa, orden de cambio) tienen su número propio pero en Odoo se les elige
//       LA MISMA analítica; la sincronización diaria las detecta y registra sola
//       en proyectos.sub_cotizaciones.
//     - DESCUADRES: proyecto sin analítica que matchee, o valor del ERP ≠ suma
//       de las cotizaciones de la analítica (±1%) → correo diario a Miguel y
//       Felvison con el detalle para cuadrarlo.
//     - La tarea de la Orden de Cambio ahora instruye crear la sub-cot eligiendo
//       la analítica original. Manual IA actualizado.
// v8.31.1: [ERP 100% al día] Principio de Leonardo para todos.
//     - KPI nuevo "Proyectos creados completos" (Miguel/Erisdania, 15%): un
//       proyecto aprobado en Odoo queda completo en el ERP EL MISMO DÍA (cliente,
//       contacto, ubicación GPS, áreas con m² y sistema, tareas, valor, equipo).
//       Definición en lib/helpers/proyectoCompleto.js.
//     - NOTIFICACIÓN INMEDIATA: al aprobar (o crear aprobado) un proyecto, se
//       intenta auto-llenar la ubicación DESDE EL LEVANTAMIENTO (site con la misma
//       referencia ST-C…) y si aún falta info se les envía correo al momento a
//       Miguel y Erisdania con la lista exacta de lo que falta.
//     - El correo diario de las 10:30 ahora incluye la sección "🧩 Proyectos
//       aprobados con información incompleta".
//     - KPI nuevo "Caja chica al día" (supervisores con caja, 10%): gasto hecho
//       hoy se reporta hoy o máximo mañana (fecha del gasto vs fecha de captura).
//     Rebalance: supervisor 20/15/15/15/10/15/10 · gerente 10/15/15/15/15/10/10/10.
// v8.31.0: [Bono de Edwin + KPI 48h de equipo] El deber de Edwin (rol de bono
//     'comercial'): recibir toda solicitud, crearla en el ERP y asignarla; los
//     SUPERVISORES realizan el levantamiento; Edwin cotiza el MISMO DÍA; cuando la
//     cotización sale ENVIADA en Odoo el proceso CIERRA solo (cron) y nace la tarea
//     de CONFIRMAR con el cliente que la recibió en ≤24h (asegura su KPI).
//     - KPI COMPARTIDO "Levantamientos ≤48h": todo levantamiento se realiza en
//       ≤48 horas desde que se recibe; pega en el bono de Edwin, TODOS los
//       supervisores (15%), Miguel (10%) y Erisdania cuando tenga bono.
//     - KPIs de Edwin: solicitudes ≤1 día 20 · nada sin asignar 15 · 48h equipo 20
//       · cotización mismo día 25 · recepción confirmada ≤24h 20.
//     - Migración 108: surveys.projects.realizado_at + cotizado_at (se estampan al
//       finalizar levantamiento y al pasar a Cotización Realizada).
//     - Cron sync-facturas ahora también cierra levantamientos con cotización
//       enviada/confirmada en Odoo y crea la tarea de confirmación (a Edwin).
// v8.30.4: [Reportar premia] Promover y premiar antes que castigar:
//     - Celebración al guardar un reporte: "+X m² a tu producción 💰", racha 🔥
//       de días seguidos reportando y Brigada de la Semana 🏆 (auto-cierra en 6s).
//     - Tarjeta de racha + Brigada de la Semana en Mi Producción del maestro.
//     - El SUPERVISOR REVISA los reportes del día desde su Cierre del día:
//       "✓ OK" (visto bueno) o "⚠ Obs" (observación) — queda quién y cuándo
//       (migración 107: reportes.revisado_por_id/nombre/at + observacion).
//     - lib/helpers/reconocimiento.js: racha (lun-sáb, hoy no rompe hasta acabar)
//       y Brigada de la Semana (más días con reporte, desempate por m²).
//     Pendiente (decisión de monto): bono de cumplimiento por quincena.
// v8.30.3: [Alerta de reportes] Cron diario 10:30 AM RD (lun-sáb) que envía a la
//     gerencia (Leonardo + Miguel + Erisdania cuando su ficha tenga email; lista
//     configurable con env ALERTA_REPORTES_EMAILS) el correo de obras EN EJECUCIÓN
//     sin reporte del día anterior — con si hubo jornada ese día (trabajaron y no
//     reportaron vs no se registró) y el último reporte de cada una. Si ayer fue
//     domingo se evalúa el sábado. Regla: reporte del día antes de las 10:30 am
//     del día siguiente. Ruta /api/cron/reporte-diario-obras + cron en vercel.json.
// v8.30.2: [Carga y Actividad] Tablero (menú Finanzas, admin) que mide quién hace
//     qué y cuánto tarda cada cosa, SIN que nadie reporte nada: actividad por
//     persona en el ERP (jornadas, reportes, estados, ajustes, pedidos, paradas,
//     facturas, caja, tareas, cubicaciones, órdenes de cambio) + actividad por
//     usuario en Odoo vía /api/carga solo-lectura (OCs, facturas prov/cliente,
//     pagos, contactos nuevos, líneas de banco) + tiempos de ciclo (pedido→listo,
//     listo→entregado, OC borrador→confirmada) + colas actuales. Rangos 7/30/90
//     días. Para detectar duplicidad de funciones, redistribuir y decidir
//     contrataciones con datos (tabla de señales del organigrama).
// v8.30.1: [Plan de Obras] Planificación de OBRAS en el tiempo (la vista Planificación
//     existente es de personas por día). Línea de tiempo por semanas (10 sem, navegable)
//     de todas las obras activas con su barra inicio→entrega por estado, marcador de
//     hoy, alerta de entregas vencidas, y cola "Sin programar" (toda obra activa
//     necesita fechas — es la cola de planificación del gerente). Clic en la barra =
//     mover fechas; guarda en proyectos.fecha_inicio/fecha_entrega (mismos campos del
//     Cronograma). Menú Operación → "Plan de Obras" (admin). Base del futuro KPI de
//     entregas a tiempo (fecha_entrega = fecha comprometida).
// v8.30.0: [Órdenes de Cambio] Aumentos de volumen y cambios de alcance con
//     constancia (caso La Sirena: el alcance crece y el ERP se queda con la
//     volumetría original). Pestaña "Cambios" del proyecto (admin+supervisor):
//     OC con líneas por área (m² adicionales × precio), correo formal al cliente
//     desde el ERP (o marcar enviada por otra vía), registro de QUIÉN aprobó y por
//     qué vía, y "Aplicar" (admin): suma m² a las áreas / crea áreas nuevas,
//     actualiza valor_cotizacion y crea la tarea de ajustar la cotización en Odoo
//     asignada a facturación. Migración 106 (ordenes_cambio, RLS off). Manual IA
//     actualizado. Regla: sin OC no se ejecuta volumen extra.
// v8.29.2: [Bonos] Rebalance de KPIs para no castigar a operaciones por falta de
//     cartera vendida: Producción baja de 40% a 20% y entra "Obras avanzando" (20%)
//     — % de obras en ejecución con avance reportado en los últimos 5 días (una obra
//     que no puede avanzar por causa externa se marca "parado" con razón y sale del
//     cálculo). Aplica a supervisores y gerente. El KPI de "arranque a tiempo" de
//     obras aprobadas queda pendiente hasta registrar el motivo de espera.
//     GERENTE — 3 KPIs nuevos (pedido de Leonardo): "ERP al día" 15% (obras activas
//     en su estado correcto y con equipo completo), "Terminadas → facturadas" 20%
//     (hacer no basta: +15 días en finalizado sin facturar resta) y "Cubicaciones
//     mensuales" 15% (obras de +30 días en ejecución con cubicación reciente o fecha
//     fijada). Pesos gerente: prod 15 / avance 15 / ERP 15 / facturación 20 /
//     cubicaciones 15 / levantamientos 10 / reclamaciones 10.
// v8.29.1: [Manual IA] manualERP.js aprende los flujos nuevos: Pedidos/requisiciones,
//     Almacén, Rutas, ruta del chofer (horas extras), Mis Pendientes y Mi Bono —
//     el asistente del ERP ya puede explicárselos a cualquier usuario.
// v8.29.0: [Logística] Requisiciones + Rutas + usuario Chofer (adiós WhatsApp y Excel):
//     - Tab "Pedidos" en el proyecto: la obra pide materiales (renglones libres,
//       urgente); ve el estado real: pendiente → preparando → lista → en ruta →
//       entregada.
//     - Vista "Almacén" (admin o rol nuevo `almacen`): cola de requisiciones,
//       marcar renglones despachados y avanzar a LISTA para envío.
//     - Vista "Rutas" (admin): viajes por fecha — camión propio (chofer) o envío
//       pagado; montar requisiciones listas y paradas libres (puertos, almacenes
//       fiscales, suplidores) con orden; resumen de horas por chofer (extras >8h).
//     - Rol nuevo `chofer`: su inicio muestra la ruta del día — inicia jornada,
//       marca cada parada hecha y termina jornada (horas para nómina/extras).
//     - "Mis Pendientes": nuevas reglas de almacén (preparar/coordinar envío).
//     - Migración 105 (requisiciones, requisicion_items, viajes, viaje_paradas).
// v8.28.3: [Mis Pendientes] Task manager del ERP (estilo Asana, sin mantenimiento):
//     las tareas DIARIAS se generan solas por responsable desde el estado real —
//     jornadas sin abrir/cerrar, obras sin reporte hoy, levantamientos asignados
//     (cita vencida), reclamaciones asignadas (+14d), tareas manuales del módulo
//     Tareas, y reglas de gerencia para admin (levantamientos/reclamaciones sin
//     asignar, obras aprobadas sin equipo, corte de nómina vencido). Desaparecen
//     solas cuando lo pendiente se hace en el sistema. Tarjeta en el inicio del
//     supervisor + vista "Mis Pendientes" en el menú (admin). Motor en
//     lib/helpers/pendientes.js — sin migraciones.
// v8.28.2: [Bonos por KPIs] "Mi bono" — bono trimestral variable calculado EN VIVO
//     desde el ERP. Supervisores: jornadas propias (20%), reportes al día (20%),
//     producción de sus obras vs meta (40%), reclamaciones (20%). Gerente de
//     operaciones: producción global, levantamientos a tiempo (≤7d), reclamaciones
//     sin envejecer (+14d) y disciplina de reporte. Gate 70 pts, tope 120 pts.
//     - Tarjeta "Mi bono" en el inicio del supervisor (puntaje y qué le resta, en vivo).
//     - Vista "Bonos" (menú Finanzas, admin): puntajes de todos; el owner configura
//       monto objetivo, meta de producción y rol del bono por persona.
//     - Migración 104 (bonos_config). Motor de cálculo en lib/helpers/bonos.js.
// v8.28.1: [Despeje de Operaciones] (1) Inicio del supervisor: nuevo bloque "Jornadas de
//     hoy" — sus obras en ejecución con estado de jornada y arranque en un toque (GPS +
//     brigada completa preseleccionada); el cierre navega a la obra donde vive el modal
//     de dieta/estadía. (2) "Programar jornada (pasada o futura)" ahora también la puede
//     usar el SUPERVISOR del proyecto (antes solo admin; RLS ya lo permitía).
//     Objetivo: que jornadas las manejen los supervisores y no la gerencia de operaciones.
// v8.27.48: [Facturación] el correo "Listo para facturar" ahora va a yharris@ (Yamel)
//     con copia a mmartinez@, fcalcano@ y ljaime@ (antes iba a todos los admins).
//     Se agrega soporte de CC en /api/enviar-reporte y en db.enviarCorreoReporte.
// v8.27.47: [Facturas → Odoo] el CSV manda el PRODUCTO (invoice_line_ids/product_id, por
//     nombre: Gasoil/Gas/Materiales Varios) para que Odoo reconozca el producto existente y
//     autocomplete cuenta e ITBIS. No crea productos nuevos (ya existen).
// v8.27.46: [Facturas → Odoo] el CSV de importación usa los NOMBRES TÉCNICOS de campo de
//     Odoo como encabezados (move_type, company_id, invoice_date, partner_id,
//     l10n_latam_document_number, currency_id, invoice_line_ids/name, .../price_unit,
//     .../account_id, .../analytic_distribution) → Odoo auto-mapea sin intervención.
//     company_id sale con el nombre exacto de la compañía; tax_ids y tipo de NCF los ajusta
//     contabilidad. Columnas ref_* son informativas.
// v8.27.45: hotfix del build de 8.27.44 (un comentario JSX antes del elemento raíz rompía
//     la compilación). Mismos cambios: scroll del modal + "volver a leer con IA" / re-tomar foto.
// v8.27.44: [Facturas UI] (1) FIX scroll: el modal se alinea arriba (no centrado) y los botones
//     Guardar/Cancelar quedan FIJOS abajo (sticky) — antes el form largo no dejaba alcanzar
//     "Guardar". (2) Botones "Volver a leer con IA" (re-parsea la misma foto/PDF, ej. si leyó
//     mal el e-NCF) y "Re-tomar foto", para corregir rápido sin re-subir.
// v8.27.43: [Facturas → Odoo] (1) ITBIS muestra el MONTO desglosado (la IA lo lee); "incluido"
//     solo cuando la factura no lo desglosa. (2) SUBIR PDF (desde computadora) además de foto:
//     el PDF va sin comprimir, la IA lo lee nativo (Sonnet 4.5), y se guarda/exporta igual.
//     Aplica en subida individual y en carga masiva.
// v8.27.42: [Facturas → Odoo] MONEDA (DOP/USD/EUR): selector en el modal + la IA la lee de
//     la factura + columna moneda en el CSV. Odoo la necesita (hay facturas en US$); el monto
//     se captura tal cual y Odoo convierte con su tasa del día. Migración 101 (default DOP).
// v8.27.41: [Facturas → Odoo] (1) CARGA MASIVA: elegir muchas fotos de una — la IA parsea
//     cada factura y crea borradores para revisar. (2) La IA ahora LEE la cuenta analítica
//     escrita a mano en lapicero azul sobre la factura y la pre-llena (revisable).
// v8.27.40: [Facturas → Odoo] campo "Cuenta analítica" (el número en lapicero azul = el
//     proyecto) en el modal de subir factura + columna cuenta_analitica en el CSV de import.
//     La IA ahora pone el concepto como producto Odoo: "Materiales Varios" (ferretería),
//     "Gasoil"/"Gasolina" (combustible, leído de la factura) o "Gas". Migración 100.
// v8.27.39: [Vehículos] módulo de flota (marca/modelo/año/color/chasis/placa con copiar,
//     matrícula y seguro adjuntos con vencimiento). Elegibles en las cartas de acceso.
//     Accesible por admin y por la capturista (rol 'facturas', Lily).
// v8.27.38: [Personal] FIX: la lista de Gestión de Personal solo agrupaba admin/supervisor/
//     maestro/ayudante, así que una persona con rol dedicado 'Facturas' (Lily) no aparecía
//     aunque estuviera creada. Se agrega el grupo "Facturas" + catch-all "Otros" para que
//     ningún rol quede invisible.
// v8.27.34: [Asistente IA] FIX: usaba el modelo claude-sonnet-4-20250514 (retirado →
//     not_found_error). Actualizado a claude-sonnet-4-5-20250929, el mismo que ya usan
//     las demás funciones de IA del ERP (caja chica, reportes, etc.).
// v8.27.33: [Solicitudes] la tarjeta de la bandeja abre el detalle/edición con un clic en
//     cualquier parte (los botones Aprobar/Descartar y los enlaces no lo disparan).
// v8.27.32: [Solicitudes] fix del modal de ver/editar: altura máxima (92vh) con encabezado y
//     botones fijos y el cuerpo con scroll interno — ya no se recorta la parte de arriba (Cliente).
// v8.27.31: [Solicitudes] el modal de ver/editar es más ancho (max-w-4xl) y muestra un
//     MAPA embebido (Leaflet/OSM) con el pin de la ubicación — se puede tocar para
//     corregir el pin (guarda lat/lng en la solicitud).
// v8.27.30: [Solicitudes] la bandeja permite ABRIR una solicitud ("Ver / editar todos los
//     datos"): modal con todos los campos editables (cliente, locación, trabajo, fotos).
//     Se puede "Guardar" correcciones o "Guardar y aprobar" — al aprobar, el cliente/
//     locación/levantamiento se crean con los datos ya corregidos. API /api/solicitudes
//     acepta accion 'guardar' y 'aprobar' con {cambios} (lista blanca de campos).
// v8.27.29: [Solicitudes] (1) Edwin siempre recibe el correo de aviso al llegar una
//     solicitud nueva (además de los admins). (2) El menú "Solicitudes" muestra un badge
//     rojo con el número de solicitudes NUEVAS (endpoint /api/solicitudes/count, refresca
//     al navegar y cada 60s).
// v8.27.28: [Solicitudes] la vista previa del link ahora usa un banner 1200×630 con el
//     logo real de Super Techos (public/og-solicitud.png) + card grande (summary_large_image).
// v8.27.27: [Solicitudes] vista previa (Open Graph) propia para /solicitud — al compartir
//     el link por WhatsApp/redes muestra "¿Tienes una filtración? Pásanos los datos aquí
//     y te ayudamos" en vez del título interno del ERP (app/solicitud/layout.jsx).
// v8.27.26: [Gotera] (#3) caja chica: la bandeja ordena por fecha de SUBIDA (created_at),
//     no por la fecha de la factura — así una factura con la fecha mal leída por la IA no
//     "desaparece"; + alerta al subir si la fecha es muy vieja/futura. (#9) el supervisor
//     (o admin) agrega/quita operarios de su proyecto desde el tab Equipo, sin esperar al admin.
// v8.27.25: [Gotera quick wins] (1) reporte de metraje: confirmación antes de aplicar todo
//     el pendiente y finalizar la tarea (ticket Miguel H., evita marcarla completa por error).
//     (2) Onboarding acepta PASAPORTE además de cédula (ticket "Caso Onix" — maestros sin
//     cédula). Migración 099 agrega personal.tipo_documento.
// v8.27.24: [Proyectos · Brigadas] (ticket Miguel tk_1782411961144c4ch "brigadas distintas
//     para la asistencia") un proyecto ahora puede tener 2-3 brigadas (cuadrillas), cada
//     una con su maestro y ayudantes (Editar Proyecto → Equipo → "Brigadas / cuadrillas").
//     En Asistencia, cuando hay brigadas la lista de personal presente se agrupa por
//     brigada (con su maestro y un botón "Marcar/Quitar todos"); se guarda qué brigadas
//     estuvieron presentes en la jornada. La nómina NO cambia: el pago sigue por persona.
//     Migración 20260626140000 (proyectos.brigadas jsonb + jornadas.brigadas_presentes).
// v8.27.23: [Solicitudes · Levantamiento público] (ticket Edwin tk_1782313060571gglf)
//     Formulario público /solicitud (sin login) para que clientes pidan un levantamiento:
//     RNC/Cédula (valida formato), ubicación por dirección/GPS/mapa Leaflet, punto de
//     referencia, persona que recibe, acceso al techo, fotos, etc. Cae a una BANDEJA
//     (tabla solicitudes_levantamiento, RLS ON; API service_role /api/solicitud con
//     honeypot + rate-limit). Módulo "Solicitudes" (admin): revisar → aprobar (resuelve/
//     crea cliente + crea locación + genera el levantamiento en el Kanban con su ticket
//     LEV-AAAA-MM-###) o descartar. Regla: exige el # de cotización (ST-C…) para pasar un
//     levantamiento a "Cotización realizada". Migración 20260702130000 + bucket 'solicitudes'.
// v8.27.21: [Nómina · UX] (ticket Miguel "suma todos los m² de todas las tareas y genera
//     confusión") en m²-fijo el m² REPORTADO suma cada paso sobre la misma área (limpieza
//     + imprimación + instalación de un techo de 100 m² → 300 "m²"). En el desglose,
//     impresión y filas compactas ahora se muestra el m² EFECTIVO (área real ponderada,
//     = lo que sustenta el monto). Solo display: el dinero ya venía ponderado y no cambia.
//     Las vistas detalladas ya mostraban ambos (efectivo + "Σ reportados").
// v8.27.20: [Nómina · FIX] (ticket Miguel "copia los ajustes de la quincena anterior en
//     la actual") el preview del corte abierto se calculaba con TODOS los ajustes
//     pendientes (cualquier fecha), no solo los del periodo → los de quincenas viejas aún
//     sin aplicar se sumaban al corte actual e inflaban el total (y ni siquiera salían en
//     la lista de ajustes, que sí filtra por fecha). Ahora el cálculo usa solo los ajustes
//     del rango del corte, igual que cerrarCorte() y que el live-total del dashboard.
// v8.27.19: [Proyectos · FIX] (ticket "se crean dobles y triples") doble/triple clic en
//     "Crear" creaba 2-3 proyectos. El chequeo anti-duplicado leía la lista en memoria
//     que no se actualiza hasta después de crear+recargar → los clics rápidos pasaban
//     todos antes del 1er insert. Fix: guard síncrono (useRef) que bloquea el 2º clic al
//     instante + botón se deshabilita y muestra "Creando…" mientras corre.
// v8.27.18: [Nómina · FIX] los ajustes se atribuían solo por proyecto, no por persona
//     → se apilaban en la 1ª fila del proyecto (caso Pablo Tejas: 19,500 que eran de
//     Vladimir/Santo/Juan Carlos en ST-C5108). Ahora el ajuste cae en la fila de SU
//     dueño (o en su propia fila "(Ajustes)" si no trabajó esa obra el corte).
// v8.27.17: [Nómina · Tipos de pago] (ticket Miguel) — (3.2) PAGO POR PAQUETE: agrupa
//     varias tareas bajo un precio/m² y un maestro (paga una vez sobre los m² del área,
//     proporcional al avance). (4) AJUSTE POR TAREA definido en el proyecto: monto fijo
//     RD$ a un maestro, sale como línea aparte en la nómina. Migración 098.
// v8.27.15: [Design System] Nueva ruta pública /design-system: guía de estilo viva con
//     los tokens reales (tailwind.config: brand/surface/state) y los componentes del
//     producto (botones, badges, Campo+Input, toggle densidad, KPIs, tarjeta de proyecto,
//     fila de tabla editable, modal). Usa el componente real components/common/Badge.
//     Base para subir a Claude Design (claude.ai/design) cuando haya /login.
// v8.27.13: [Nómina · M2 fijo] En Editar Proyecto, modo "M2 fijo (precio propio)", al
//     poner el precio/m² ahora se ve el DESGLOSE por tarea: cada tarea muestra su peso %
//     y los RD$/m² efectivos que asume (ej. pintura 10% → RD$X/m²). Aviso ámbar si los
//     pesos no suman 100%. Esto ya funcionaba en el cálculo (calculos.js, m² ponderados
//     por peso); ahora es VISIBLE al configurar. Responde el ticket de Miguel.
// v8.27.12: [Levantamientos] El campo "# de ticket (Odoo / cliente)" ahora va de
//     PRIMERO en el modal de crear levantamiento (antes del tipo de servicio).
// v8.27.11: [Levantamientos] DOS identificadores: (1) CÓDIGO INTERNO automático
//     LEV-AAAA-MM-### (external_code) y (2) # DE TICKET del CLIENTE / Odoo (manual,
//     nueva columna surveys.sites.referencia_odoo, migración 097 — ej. ST-C5477).
//     En el modal de crear y en Editar datos hay un campo para el ticket de Odoo; el
//     interno se asigna solo. En el detalle se ven ambos badges (interno gris,
//     Odoo azul 🎫).
// v8.27.10: [Levantamientos] Código AUTOMÁTICO al crear: LEV-AAAA-MM-### (correlativo
//     por mes, en hora RD; ej. LEV-2026-06-001). Secuencia atómica en la BD (RPC
//     siguiente_codigo_levantamiento + tabla levantamiento_codigo_contador, migración
//     096). Si se escribe un # manual, se respeta; si se deja vacío, se asigna solo.
//     El código se muestra como badge en el detalle del levantamiento.
// v8.27.9: [Rendimiento arranque] loadAllData ahora hace TODAS las cargas secundarias
//     (permisos, clientes, contactos, checkins, documentos, credenciales, dieta,
//     propiedades, categorías, capacidades, estados de área, asignaciones) en PARALELO
//     en vez de en serie — mucho más rápido en celular de alta latencia (era lo que
//     disparaba el "Error de conexión" del timeout). Timeout subido a 40s de margen.
// v8.27.8: [Rendimiento/arranque] (1) Los avatares (foto_2x2 base64, ~1.6MB) ya NO
//     se traen en la carga inicial: se bajan en SEGUNDO PLANO (cargarAvatares) y se
//     mezclan al llegar, con caché para sobrevivir recargas. Arranque más liviano,
//     clave en celular. (2) Guardia de TIMEOUT en la carga inicial: si no termina en
//     25s (conexión lenta/intermitente), muestra "Reintentar" en vez de quedarse
//     girando "Conectando…" para siempre.
// v8.27.7: [Levantamientos] Se puede poner un "# de ticket" (external_code) al CREAR
//     el levantamiento (modal nuevo) y también editarlo después (Editar datos). Ya
//     se mostraba como badge en el detalle del site; ahora es capturable/editable.
// v8.27.6: [Levantamientos] En el detalle del site ahora se muestra el NOMBRE DEL
//     CLIENTE (empresa, del cliente vinculado o del proyecto) arriba del título, y
//     el CONTACTO del cliente (nombre + cargo + móvil) junto a la localidad — tanto
//     en el header como en el overlay de la foto de portada. La localidad sigue
//     siendo el título.
// v8.27.5: [Gotera/Planificación FIX] No se podía reportar los días de personas que
//     no son maestro/ayudante (p.ej. Yamel, admin que va a obra): la planificación
//     filtraba SOLO maestros/ayudantes. Ahora: (1) quien ya tiene jornada/asignación
//     se muestra aunque su rol sea admin/supervisor; (2) nuevo botón "+ Agregar
//     persona" (admin) que abre un buscador de TODO el personal activo para meterlo a
//     la planificación y reportarle/asignarle días.
// v8.27.4: [Nómina] El "+ Ajuste" permite cargar el ajuste a "🔧 Reclamación
//     (genérico)" en el selector "Cargar a". Marca el ajuste como reclamación
//     (ajustes_nomina.es_reclamacion, migración 095) y cae en la fila "Reclamaciones"
//     del corte, sin requerir una reclamación específica (la tabla aún está vacía).
//     Cuando se desarrolle el módulo de reclamaciones, se podrá elegir la
//     reclamación puntual (reclamacion_id) en vez del genérico.
// v8.27.3: [Gotera/Nómina FIX] Los ajustes "+ Ajuste" SIN proyecto (o de una obra
//     que no está en el ERP, p.ej. pagos de reclamaciones) se absorbían dentro de
//     la fila de la obra con más días y "no se reflejaban" como línea propia. Ahora
//     calcularDetalle los pone en una fila dedicada y VISIBLE por persona:
//     "Reclamaciones" si el ajuste viene de una reclamación, o "(Ajustes)" en
//     general. Los ajustes con un proyecto que SÍ se trabajó siguen yendo a su obra.
// v8.27.2: [Gotera/Nómina FIX] Una persona que trabaja en 2+ obras el MISMO día
//     y cobra POR DÍA cobraba un día por CADA proyecto (doble/triple pago por un
//     solo día calendario). Ahora calcularDetalle deduplica las fechas pago-por-día
//     entre las obras de cada persona: el proyecto con más días conserva la fecha
//     en conflicto y los demás la sueltan, así cobra UN solo día. Los modos m²/tarea
//     no se tocan (pagan por producción, no por día).
// v8.27.1: [Limpieza grande] Eliminado todo lo de planificación estratégica sin
//     uso (0 filas en prod): (1) la sección colapsable "PLANIFICACIÓN" del menú
//     (Disponibilidad, Ausencias, Equipos, Credenciales, Autorizaciones, Brigadas)
//     y sus 6 vistas; (2) el subsistema completo de CREDENCIALES / AUTORIZACIONES /
//     ACCESO A OBRAS DE CLIENTES: el modal "Accesos/Credenciales/Autorizaciones"
//     por persona (3 pestañas) en el módulo Personal, el filtro "por acceso a
//     cliente", la "sugerencia de personal por área" y el motor de elegibilidad.
//     Total ~1900 líneas + ~25 funciones db (equipos/maquinaria, ausencias,
//     brigadas, credenciales, tipos_credenciales, requisitos cliente/ubicación,
//     autorizaciones, accesos). Se conservan: Capacidades técnicas (sistemas que
//     domina cada persona), estados de área y asignaciones de la Planificación real.
// v8.27.0: [Gotera] Nuevo módulo de feedback / reporte de errores del ERP en modo
//     ticket. Cualquier usuario abre un ticket (error o idea), indica módulo +
//     pantalla, impacto (bloqueante/alta/media/baja), adjunta screenshots y nota
//     de voz. El owner/admin lo gestiona en un kanban (Abierto/En proceso/Resuelto/
//     Cerrado), prioriza por impacto, abre la ficha (todo ordenado: quién, rol,
//     qué, módulo, pantalla), copia el ticket al portapapeles o lo resuelve. Al
//     resolver se notifica al solicitante y, al confirmar, pasa a Cerrado. Tabla
//     tickets_soporte (migración 094); adjuntos en buckets documentos-fotos
//     (soporte/) y reportes-audio (soporte/).
// v8.26.13: [Levantamientos] En el detalle del site: (1) nueva tarjeta "Ubicación"
//     con dirección/ciudad/provincia/coordenadas visibles + mini-mapa Leaflet
//     (160px) al lado cuando hay geo (placeholder cuando no). (2) Admin/owner
//     cambian la ETAPA con píldoras clicables arriba en el header (cambio rápido
//     dentro del ticket); el selector de etapa de abajo queda solo para supervisor.
// v8.26.12: [Nómina] Nuevo tipo "Ajuste (suma al pago)" en el modal de Nuevo
//     ajuste — para cargar trabajos puntuales (PA, partidas, reclamaciones) al
//     corte; suma al pago igual que un bono pero queda etiquetado como ajuste.
// v8.26.11: [Nómina] El modal "Nuevo ajuste" permite elegir a QUÉ PROYECTO se
//     carga el ajuste (opcional). Si se elige, el ajuste cae en la fila de esa
//     persona en ESE proyecto dentro del corte (antes siempre iba a la fila con
//     más días). "— Sin proyecto (general) —" mantiene el comportamiento anterior.
// v8.26.10: [Nómina — modo de pago POR BRIGADA] Cada maestro de la obra puede
//     cobrar con su propio tipo de MDO: una brigada por día y otra por m² en la
//     MISMA obra. En Editar Proyecto, sección "Modo de pago por brigada": por
//     maestro (principal/área/tarea) se elige día / m² fijo (con precio propio) /
//     m² por tarea / por tarea. "Por día" se propaga a sus ayudantes (persona.
//     maestroId) con tarifas editables inline. Se persiste como overrides en
//     costos_dia_proyecto (la nómina ya los respeta). Migración 093: costo_dia
//     DEFAULT 0 (el upsert parcial de overrides fallaba si la persona no tenía
//     fila previa — bug latente del selector v8.26.7).
// v8.26.9: [Nómina — maestro responsable POR TAREA] Caso Ayac Mercedes: dos
//     maestros repartiéndose tareas DENTRO de la misma área (Luis demolición,
//     Osman grietas/pulido). Nuevo campo proyectos.maestros_tareas (migración 092,
//     JSONB tareaId→personaId). En Editar Proyecto (modos m²/m² fijo/tarea) aparece
//     "Maestro responsable por tarea (opcional)". La nómina atribuye el m² de cada
//     reporte con prioridad: maestro de la TAREA > maestro del ÁREA > maestro
//     principal. TabManoDeObra muestra el badge "Maestro tarea".
// v8.26.8: [Nómina multi-sistema + reclamaciones pagables]
//     (1) FIX: en nómina, el sistema de cada reporte se resuelve por el ÁREA
//     (area.sistemaId), no solo por el sistema principal del proyecto — antes las
//     tareas de un 2.º sistema (p.ej. cementicio en obra de lona, caso Urtecho)
//     tenían peso 0 y pagaban RD$0 en modo m² fijo. Los pesos ahora se buscan en
//     TODOS los sistemas del proyecto. (2) El selector de modo de pago POR PERSONA
//     suma "Por m² por tarea" y "Por tarea". (3) En Editar Proyecto, los precios
//     por tarea (modos m²/tarea) listan también las tareas de los sistemas de las
//     áreas (etiquetadas con su sistema). (4) RECLAMACIONES PAGABLES: en la ficha
//     de la reclamación, sección "Pago de mano de obra" (owner/admin): días ×
//     tarifa por persona → crea un ajuste de nómina vinculado (migración 091,
//     ajustes_nomina.reclamacion_id) que cae al corte abierto por su fecha; la
//     ficha acumula el costo MDO de la reclamación.
// v8.26.0: [Contabilidad · Fase 1] Nuevo módulo Contabilidad (sección FINANZAS,
//     SOLO el usuario dueño por ahora — en validación con el contador) con reportes
//     DGII: 606 (compras, desde caja chica), 607 (ventas, leídas de Odoo vía
//     /api/contabilidad/ventas-odoo), 608 (NCF anulados) y resumen ITBIS/IT-1, todo
//     por empresa (Super Techos/Prouco). La LÓGICA FISCAL vive en un MÓDULO
//     INDEPENDIENTE Y PORTÁTIL en TypeScript (lib/contadom, @supertechos/contadom,
//     agnóstico de plataforma) que compila a lib/contadom/dist; el ERP solo aporta
//     datos (dgiiAdapter.js) y la descarga. Migración 086 (cont_ncf_secuencias,
//     cont_ncf_anulados). VistaContabilidad con tabs Reportes DGII / NCF anulados /
//     Secuencias NCF. Las secuencias y la facturación nativa se cablean en Fase 3.
// v8.25.47: el m² de "AVANCE DEL PERIODO" del reporte se PONDERA por el peso de la
//     tarea (Σ m² × peso/100) en vez de sumar el m² crudo de cada tarea (capas sobre
//     la misma superficie) — antes inflaba el total.
// v8.25.46: Obras PARADAS — al pasar a "parado" se exige razón + siguiente paso (van
//     al chatter); el cambio de estado se registra en el chatter del proyecto (no solo
//     historial_estados); y en el detalle el chatter sube arriba (banner "OBRA PARADA").
// v8.25.40: Planificación a futuro — al asignar desde una celda del calendario
//     semanal se puede elegir "Hasta (incluido)" para crear una ASIGNACIÓN
//     programada de varios días (asignaciones_personal, estado confirmada, con
//     rol maestro/ayudante/supervisor). Un solo día sigue creando/uniéndose a la
//     jornada como antes. Admin puede asignar a CUALQUIER proyecto activo (antes
//     solo a proyectos donde la persona ya estaba en el equipo).
// v8.22.0: Settings de Notificaciones por módulo+etapa. Por etapa de Proyectos/
//     Levantamientos/Reclamaciones/Garantías se decide si envía correo al CLIENTE y/o
//     INTERNO (roles + lista de correos), con plantilla. Editor de plantillas (asunto+HTML
//     con variables). Tablas email_templates + stage_notif_configs (migración 064). Nueva
//     entrada de menú 'Notificaciones'. (El envío real se cablea en la siguiente fase.)
// v8.21.0: (1) Chatter tipo Odoo — bitácora de eventos (creación, cambios de estado) +
//     notas manuales por entidad. Tabla chatter_mensajes (migración 063), lib/chatter.js,
//     componente ChatterPanel. Montado en el detalle del levantamiento. (2) Correo
//     "Levantamiento recibido" al cliente al crear (Resend; ruta /api/email/levantamiento-
//     recibido con plantilla placeholder PENDIENTE de texto final). En LOCAL no envía.
