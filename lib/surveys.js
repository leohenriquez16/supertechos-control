...
export async function finalizarLevantamientoSurvey(projectId) {
  const { error } = await supabase.schema('surveys').from('projects')
    .update({ status: 'survey_completed', odoo_stage: 'Realizado' }).eq('id', projectId);
...
export async function reabrirLevantamientoSurvey(projectId) {
  const { error } = await supabase.schema('surveys').from('projects')
    .update({ status: 'survey_in_progress', odoo_stage: 'Asignado' }).eq('id', projectId);
...
export const FAMILIAS_SERVICIO = [
  ...
  { key: 'impermeable', label: 'Impermeabilizante / Aislante', tipoLabel: 'Tipo de techo', lines: ['waterproofing', 'thermal_insulation'] },
