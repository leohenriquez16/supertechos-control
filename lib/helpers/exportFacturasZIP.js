...
export async function generarZipFacturas({ movimientos, data, onProgreso }) {
  // v8.16.1: excluir gastos sin factura — no son facturas formales y no tienen foto
  const facturas = movimientos.filter(m => m.tipo === 'gasto_factura' && m.status === 'aprobado' && !m.datosIA?.sin_factura);
  if (facturas.length === 0) {
    throw new Error('No hay facturas aprobadas con factura formal en el período seleccionado. Los gastos "sin factura" no se incluyen en este export.');
  }
