...
export default function PhotoCapture({ visitId, areaId = null, field, value, onChange }) {
  ...
  return (
    <div>
      ...
      {!maxAlcanzado && (
        <label
          className={`block w-full bg-zinc-900 border-2 border-dashed border-zinc-700 hover:border-red-600 px-4 py-3 text-center cursor-pointer transition-colors ${
        ...
        >
          ...
          <div className="flex items-center justify-center gap-2 text-sm">
            {subiendo ? <Loader2 className="w-4 h-4 animate-spin text-red-500" /> : <Camera className="w-4 h-4 text-red-500" />}
            <span className="text-zinc-300 font-bold">
              {subiendo ? 'Subiendo...' : 'Tomar / agregar foto'}
            </span>
          </div>
          {field.required_labels && (
            <div className="text-[10px] text-zinc-500 mt-1">
              Sugeridas: {field.required_labels.join(', ')}
            </div>
          )}
        </label>
      )}
