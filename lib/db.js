...
export async function loadAllData() {
  try {
    const [personalRes, sistemasRes, proyectosRes, reportesRes, enviosRes, configRes, categoriasCajaRes] = await Promise.all([
      ...
      supabase.from('personal').select('id, nombre, pin, roles, maestro_id, telefono, direccion, cedula_numero, fecha_ingreso, recomendado_por, email, foto_2x2, cedula_frente, cedula_reverso, reporte_audio_habilitado, modo_pago, tarifa_m2, tarifa_dia, caja_chica_habilitada, limite_caja_chica, max_transaccion_caja_chica, pin_temporal, onboarding_completado, invitado_at, invitado_por_id, fecha_nacimiento, whatsapp, tipo_sangre, contacto_emergencia_nombre, contacto_emergencia_telefono, contacto_emergencia_relacion, banco, banco_tipo_cuenta, banco_numero_cuenta, banco_titular_nombre, banco_titular_cedula').order('nombre'),
    ...
    const personal = (personalRes.data || []).map(p => ({
      ...
      cajaChicaHabilitada: !!p.caja_chica_habilitada, // v8.12: toggle módulo Caja Chica
      limiteCajaChica: p.limite_caja_chica != null ? Number(p.limite_caja_chica) : null, // v8.12: límite RD$
      maxTransaccionCajaChica: p.max_transaccion_caja_chica != null ? Number(p.max_transaccion_caja_chica) : null, // v8.13: máximo por transacción
      // v8.14: onboarding obligatorio para maestros
      pinTemporal: !!p.pin_temporal,
      onboardingCompletado: p.onboarding_completado !== false, // default true para los existentes
    ...
    try {
      ...
      // v8.18.0: marcar pre-jornadas vencidas como no_asistio antes de listar asignaciones
      try { await marcarNoAsistioVencidas(); } catch (e) { console.warn('marcarNoAsistioVencidas:', e.message); }
      asignaciones = await listarAsignaciones(); // v8.18.0: reconstruido desde jornadas
      accesosClientes = await listarAccesosClientes(); // v8.17.0
...
// JORNADAS (v7.2b)
// ============================================================
// v8.18.0: Mapper común para filas de jornadas (incluye estado y origen)
const jornadaRowToObj = (j) => ({
  ...
  estado: j.estado || 'completada',        // v8.18.0
  origen: j.origen || 'manual',             // v8.18.0
});
...
export async function listarJornadasProyecto(proyectoId) {
  const { data, error } = await supabase
    ...
    .order('fecha', { ascending: false });
  if (error) throw error;
  return (data || []).map(jornadaRowToObj);
}

// v8.18.0: Listar todas las jornadas en un rango (para grid semanal de planificación)
export async function listarJornadasRango(fechaDesde, fechaHasta) {
  const { data, error } = await supabase
    .from('jornadas')
    .select('*')
...
// v8.18.0: Filtra solo en_curso (programadas no son "abiertas")
...
export async function obtenerJornadaHoy(proyectoId, fecha) {
  ...
  if (error) throw error;
  if (!data) return null;
  return jornadaRowToObj(data);
}

// v8.18.0: Si existe pre-jornada (estado='programada') para (proyecto, fecha), la promueve a en_curso
// Sino, crea jornada nueva con estado='en_curso'
export async function iniciarJornada(j) {
  // ¿Existe pre-jornada programada?
  const { data: existente } = await supabase
    .from('jornadas')
    .select('*')
    .eq('proyecto_id', j.proyectoId)
    .eq('fecha', j.fecha)
    .maybeSingle();

  if (existente && existente.estado === 'programada') {
    // Promover pre-jornada a en_curso
    const personasIds = (j.personasPresentesIds && j.personasPresentesIds.length > 0)
      ? j.personasPresentesIds
      : (existente.personas_presentes_ids || []);
    const { error } = await supabase.from('jornadas').update({
      hora_inicio: j.horaInicio,
      iniciada_por_id: j.iniciadaPorId,
      iniciada_por_nombre: j.iniciadaPorNombre,
      ...
      nota: j.nota || existente.nota || null,
      estado: 'en_curso',
      updated_at: new Date().toISOString(),
    }).eq('id', existente.id);
    if (error) throw error;
  ...
  // Sin pre-jornada → crear nueva en_curso
  const row = {
...
// ============================================================
export async function listarAjustes({ personaId = null, sinCorte = false, proyectoId = null } = {}) {
...
// --- ACCESOS PERSONA ↔ CLIENTE (v8.17.0, Opción C híbrida) ---
// Sin row → derivado de credenciales+cliente_requisitos.
// Row activo=true → forzado activo. Row activo=false → forzado denegado.
export async function listarAccesosClientes() {
  const { data, error } = await supabase.from('personal_acceso_cliente').select('*');
  if (error) return [];
  return (data || []).map(a => ({
    personaId: a.persona_id, clienteId: a.cliente_id,
    activo: a.activo !== false, fuente: a.fuente || 'manual',
    notas: a.notas || '',
    actualizadoEn: a.actualizado_en, actualizadoPorId: a.actualizado_por_id || null,
  }));
}
export async function setAccesoCliente({ personaId, clienteId, activo, fuente, notas }) {
  const ctx = getAuditContext();
  const { error } = await supabase.from('personal_acceso_cliente').upsert({
    persona_id: personaId, cliente_id: clienteId,
    activo: activo !== false,
    fuente: fuente || 'manual',
    notas: notas || null,
    actualizado_en: new Date().toISOString(),
    actualizado_por_id: ctx.usuarioId || null,
  });
  if (error) throw error;
  registrarAudit({
    usuarioId: ctx.usuarioId, usuarioNombre: ctx.usuarioNombre,
    accion: 'acceso_cliente.actualizado',
    recursoTipo: 'acceso_cliente', recursoId: `${personaId}:${clienteId}`,
    datosDespues: { personaId, clienteId, activo: activo !== false, fuente: fuente || 'manual' },
    severidad: 'info',
  });
}
export async function quitarAccesoCliente(personaId, clienteId) {
  const ctx = getAuditContext();
  const { error } = await supabase.from('personal_acceso_cliente').delete()
    .eq('persona_id', personaId).eq('cliente_id', clienteId);
  if (error) throw error;
  registrarAudit({
    usuarioId: ctx.usuarioId, usuarioNombre: ctx.usuarioNombre,
    accion: 'acceso_cliente.quitado_override',
    recursoTipo: 'acceso_cliente', recursoId: `${personaId}:${clienteId}`,
    datosAntes: { personaId, clienteId },
    severidad: 'info',
  });
}
...
// ============================================================
// v8.18.0: ASIGNACIONES como pre-jornadas (FUSIÓN con jornadas)
// ============================================================
// El concepto "asignación" deja de ser una tabla separada. Una asignación
// = N pre-jornadas (una por día del rango) en la tabla `jornadas` con
// estado='programada'. Las funciones de abajo reconstruyen el shape
// compatible para los consumers existentes.
//
// Domingos se EXCLUYEN automáticamente (en RD las obras trabajan sábados).

// Helper: enumera días [desde, hasta] excluyendo domingos. Devuelve array YYYY-MM-DD.
function diasDeRango(fechaDesde, fechaHasta) {
  const dias = [];
...
// Reconstruye "asignaciones" virtuales agrupando pre-jornadas consecutivas por (personaId, proyectoId).
// Cada asignación virtual tiene un id estable: "vasig_{personaId}_{proyectoId}_{primeraFecha}"
export async function listarAsignaciones({ personaId, proyectoId, desde, hasta } = {}) {
  ...
  // Agrupar: (personaId, proyectoId) → [fechas ordenadas]
  const map = new Map();
  (data || []).forEach(j => {
    (j.personas_presentes_ids || []).forEach(pid => {
      if (personaId && pid !== personaId) return;
      const key = `${pid}|${j.proyecto_id}`;
      if (!map.has(key)) map.set(key, { personaId: pid, proyectoId: j.proyecto_id, fechas: [], jornadaIds: [] });
      map.get(key).fechas.push(j.fecha);
      map.get(key).jornadaIds.push(j.id);
    });
  });

  // Convertir cada grupo en una o varias asignaciones (rangos contiguos)
  const out = [];
  for (const grupo of map.values()) {
    grupo.fechas.sort();
    let actualDesde = grupo.fechas[0];
    let actualHasta = grupo.fechas[0];
    let actualIds = [grupo.jornadaIds[0]];
    for (let i = 1; i < grupo.fechas.length; i++) {
      const prev = new Date(actualHasta + 'T00:00:00');
      const curr = new Date(grupo.fechas[i] + 'T00:00:00');
      // ¿Son consecutivos (saltando domingos)?
      const diff = Math.round((curr - prev) / (1000 * 60 * 60 * 24));
      const esConsecutivo = (diff === 1) || (diff === 2 && prev.getDay() === 6); // sábado→lunes
      if (esConsecutivo) {
        actualHasta = grupo.fechas[i];
        actualIds.push(grupo.jornadaIds[i]);
      } else {
        out.push({
          id: `vasig_${grupo.personaId}_${grupo.proyectoId}_${actualDesde}`,
          personaId: grupo.personaId, proyectoId: grupo.proyectoId,
          areaId: null, fechaDesde: actualDesde, fechaHasta: actualHasta,
          rol: 'maestro', estado: 'planificada', notas: '',
          jornadaIds: [...actualIds], // v8.18.0: referencias a las pre-jornadas
        });
        actualDesde = grupo.fechas[i];
        actualHasta = grupo.fechas[i];
        actualIds = [grupo.jornadaIds[i]];
      }
    }
    out.push({
      id: `vasig_${grupo.personaId}_${grupo.proyectoId}_${actualDesde}`,
      ...
      areaId: null, fechaDesde: actualDesde, fechaHasta: actualHasta,
      rol: 'maestro', estado: 'planificada', notas: '',
      jornadaIds: [...actualIds],
    });
  }
  return out;
}

// v8.18.0: Crear "asignación" = materializar pre-jornadas en el rango
export async function crearAsignacion(a) {
  const dias = diasDeRango(a.fechaDesde, a.fechaHasta);
  const idsAfectados = [];

  for (const fecha of dias) {
    // ¿Existe jornada para este (proyecto, fecha)?
    const { data: existente } = await supabase
      .from('jornadas')
      .select('*')
      .eq('proyecto_id', a.proyectoId)
      .eq('fecha', fecha)
      .maybeSingle();

    if (existente) {
      // No tocar jornadas completadas o no_asistio (son históricas)
      if (existente.estado === 'completada' || existente.estado === 'no_asistio') continue;
      // Agregar persona al array si no está
      const personas = existente.personas_presentes_ids || [];
      if (!personas.includes(a.personaId)) {
        const { error } = await supabase.from('jornadas').update({
          personas_presentes_ids: [...personas, a.personaId],
          updated_at: new Date().toISOString(),
        }).eq('id', existente.id);
        if (error) throw error;
      }
      idsAfectados.push(existente.id);
    } else {
      // Crear pre-jornada
      const id = 'j_pre_' + Date.now() + '_' + Math.random().toString(36).slice(2, 7);
      const { error } = await supabase.from('jornadas').insert({
        id, proyecto_id: a.proyectoId, fecha,
        estado: 'programada', origen: 'asignacion',
        personas_presentes_ids: [a.personaId],
        iniciada_por_id: a.creadoPorId || null,
        iniciada_por_nombre: 'Planificación',
        nota: a.notas || null,
      });
      if (error) throw error;
      idsAfectados.push(id);
    }
  }
  ...
  registrarAudit({
    ...
    recursoId: `vasig_${a.personaId}_${a.proyectoId}_${a.fechaDesde}`,
    datosDespues: { personaId: a.personaId, proyectoId: a.proyectoId, fechaDesde: a.fechaDesde, fechaHasta: a.fechaHasta, jornadasAfectadas: idsAfectados.length },
    severidad: 'info',
  });
  return `vasig_${a.personaId}_${a.proyectoId}_${a.fechaDesde}`;
}

// v8.18.0: Actualizar asignación virtual = ajustar pre-jornadas (quita días fuera del rango nuevo, agrega los que faltan)
// El id virtual contiene personaId, proyectoId y fechaDesde original.
export async function actualizarAsignacion(idVirtual, campos) {
  // Parsear id virtual: vasig_{personaId}_{proyectoId}_{fechaDesde}
  const parts = idVirtual.split('_');
  if (parts[0] !== 'vasig' || parts.length < 4) {
    throw new Error('ID de asignación inválido: ' + idVirtual);
  }
  const personaId = parts[1];
  const proyectoId = parts[2];
  const fechaDesdeOriginal = parts.slice(3).join('_');

  // 1. Encontrar pre-jornadas actuales de este (persona, proyecto) cerca del rango
  const { data: jornadasActuales } = await supabase.from('jornadas')
    .select('*')
    .eq('proyecto_id', proyectoId)
    .in('estado', ['programada'])  // solo programadas, no tocar en_curso
    .order('fecha', { ascending: true });

  const jornadasDeEsta = (jornadasActuales || []).filter(j =>
    (j.personas_presentes_ids || []).includes(personaId)
  );

  const fechaDesdeNueva = campos.fechaDesde || fechaDesdeOriginal;
  const fechaHastaNueva = campos.fechaHasta;
  const diasNuevos = new Set(diasDeRango(fechaDesdeNueva, fechaHastaNueva));

  // 2. Quitar persona de pre-jornadas que ya no están en el rango
  for (const j of jornadasDeEsta) {
    if (!diasNuevos.has(j.fecha)) {
      const personasFiltradas = (j.personas_presentes_ids || []).filter(p => p !== personaId);
      if (personasFiltradas.length === 0) {
        // Si era la única persona y origen='asignacion', borrar la pre-jornada
        if (j.origen === 'asignacion') {
          await supabase.from('jornadas').delete().eq('id', j.id);
        } else {
          await supabase.from('jornadas').update({ personas_presentes_ids: [] }).eq('id', j.id);
        }
      } else {
        await supabase.from('jornadas').update({ personas_presentes_ids: personasFiltradas }).eq('id', j.id);
      }
    }
  }

  // 3. Agregar persona a pre-jornadas que faltan
  await crearAsignacion({
    personaId, proyectoId,
    fechaDesde: fechaDesdeNueva, fechaHasta: fechaHastaNueva,
    notas: campos.notas || null,
    creadoPorId: getAuditContext().usuarioId,
  });

  const ctx = getAuditContext();
  registrarAudit({
    usuarioId: ctx.usuarioId, usuarioNombre: ctx.usuarioNombre,
    accion: 'asignacion.editada', recursoTipo: 'asignacion', recursoId: idVirtual,
    datosDespues: campos, severidad: 'info',
  });
}

// v8.18.0: Eliminar = quitar persona de pre-jornadas de ese rango (no toca en_curso ni completadas)
export async function eliminarAsignacion(idVirtual) {
  const parts = idVirtual.split('_');
  if (parts[0] !== 'vasig' || parts.length < 4) {
    throw new Error('ID de asignación inválido: ' + idVirtual);
  }
  ...
  const { data: jornadas } = await supabase.from('jornadas')
    .select('*')
    .eq('proyecto_id', proyectoId)
    .eq('estado', 'programada');

  for (const j of (jornadas || [])) {
    const personas = j.personas_presentes_ids || [];
    if (personas.includes(personaId)) {
      const filtradas = personas.filter(p => p !== personaId);
      if (filtradas.length === 0 && j.origen === 'asignacion') {
        await supabase.from('jornadas').delete().eq('id', j.id);
      } else {
        await supabase.from('jornadas').update({ personas_presentes_ids: filtradas }).eq('id', j.id);
      }
    }
  }
  ...
  registrarAudit({
    usuarioId: ctx.usuarioId, usuarioNombre: ctx.usuarioNombre,
    accion: 'asignacion.eliminada', recursoTipo: 'asignacion', recursoId: idVirtual,
    severidad: 'warning',
  });
}

// v8.18.0: Detectar conflictos = otras pre-jornadas de la misma persona en el rango
export async function detectarConflictosAsignacion(personaId, fechaDesde, fechaHasta, excluirProyectoId = null) {
  const { data } = await supabase.from('jornadas').select('*')
    .in('estado', ['programada', 'en_curso'])
    .gte('fecha', fechaDesde)
    .lte('fecha', fechaHasta);
  ...
  const conflictos = [];
  for (const j of (data || [])) {
    if (excluirProyectoId && j.proyecto_id === excluirProyectoId) continue;
    if ((j.personas_presentes_ids || []).includes(personaId)) {
      conflictos.push({
        id: j.id,
...
// v8.18.0: Job que marca jornadas programadas vencidas como no_asistio.
// Se llama desde loadAllData (al cargar la app). Idempotente.
export async function marcarNoAsistioVencidas() {
  const hoy = new Date().toISOString().split('T')[0];
  const { error, count } = await supabase.from('jornadas')
    .update({ estado: 'no_asistio', updated_at: new Date().toISOString() }, { count: 'exact' })
    .eq('estado', 'programada')
    .lt('fecha', hoy);
  if (error) {
    console.warn('marcarNoAsistioVencidas:', error.message);
    return 0;
  }
  return count || 0;
}
...
const SELECT_CAJA = 'id, persona_id, proyecto_id, fecha, tipo, monto, signo_ajuste, foto_path, proveedor, rnc, concepto, datos_ia, status, motivo_rechazo, aprobado_por_id, aprobado_at, creado_por_id, created_at';
...
export async function actualizarStatusMovimientoCajaChica(id, { status, motivoRechazo = null, aprobadoPorId = null }) {
  const updates = {
    status,
...
// Editar campos de un movimiento (antes de aprobar). Útil cuando el maestro
// corrige datos que la AI extrajo mal.
export async function actualizarMovimientoCajaChica(id, campos) {
  const updates = {};
  if (campos.fecha !== undefined) updates.fecha = campos.fecha;
  if (campos.monto !== undefined) updates.monto = campos.monto;
  if (campos.proyectoId !== undefined) updates.proyecto_id = campos.proyectoId;
  if (campos.proveedor !== undefined) updates.proveedor = campos.proveedor;
  if (campos.rnc !== undefined) updates.rnc = campos.rnc;
  if (campos.concepto !== undefined) updates.concepto = campos.concepto;
...
export async function crearCategoriaCajaChica({ nombre, color, icono, descripcion, orden }) {
...
// "+1 (809) 555-1234" → "8095551234". Devuelve los últimos 10 dígitos si hay
// más (caso "+1 8095551234" llega como 11). Si tiene menos de 10, retorna
// como esté para que la validación lo rechace.
...
// Genera un PIN numérico aleatorio de la longitud indicada (default 6).
...
// Invita a un maestro al app: crea persona con rol maestro, PIN temporal y
// onboarding pendiente. El admin comparte el teléfono + PIN por WhatsApp.
//
// Si se pasa `personaIdExistente`, actualiza esa persona específica
// (preservando su ID y los registros vinculados — proyectos, ajustes, caja
// chica, etc.). Sirve para activar al app a personas que ya estaban en el
// sistema antes del flujo de onboarding.
//
// Si NO se pasa `personaIdExistente`:
//   - busca match por teléfono → si encuentra, actualiza esa persona
//   - si no encuentra, crea persona nueva con rol maestro
export async function invitarMaestroAlApp({ nombre, telefono, invitadoPorId, pinTemporal = null, rolesExtra = [], personaIdExistente = null }) {
  ...
  const pin = pinTemporal || generarPinAleatorio(6);
  const ahora = new Date().toISOString();

  // Modo "activar persona existente" — preserva ID, roles y resto de datos
  if (personaIdExistente) {
    const { data: existente, error: errFetch } = await supabase.from('personal')
      .select('id, nombre, roles')
      .eq('id', personaIdExistente)
      .maybeSingle();
    if (errFetch) throw errFetch;
    if (!existente) throw new Error('La persona ya no existe en la base');
    ...
    const { error } = await supabase.from('personal').update({
      ...
      invitado_at: ahora,
      invitado_por_id: invitadoPorId || null,
    }).eq('id', personaIdExistente);
    if (error) throw error;

    registrarAudit({
      ...getAuditContext(), accion: 'persona.activada_para_app',
      recursoTipo: 'persona', recursoId: personaIdExistente, recursoNombre: existente.nombre,
      severidad: 'warning',
    });
    return { id: personaIdExistente, nombre: existente.nombre, telefono: tel, pin, esNueva: false };
  }

  // Modo "nuevo" — match por teléfono o creación
  const { data: existentes } = await supabase.from('personal').select('id, nombre, telefono');
  const matchExistente = (existentes || []).find(p => normalizarTelefono(p.telefono) === tel);

  const roles = Array.from(new Set(['maestro', ...rolesExtra]));

  if (matchExistente) {
    // Re-invitar: resetea PIN temporal y onboarding sin pisar datos personales ya cargados
    const { error } = await supabase.from('personal').update({
      pin,
      pin_temporal: true,
      onboarding_completado: false,
    ...
    if (error) throw error;
    registrarAudit({
      ...getAuditContext(), accion: 'persona.reinvitada',
      recursoTipo: 'persona', recursoId: matchExistente.id, recursoNombre: matchExistente.nombre,
      severidad: 'warning',
    });
    return { id: matchExistente.id, nombre: matchExistente.nombre, telefono: tel, pin, esNueva: false };
  }
  ...
  if (error) throw error;
  registrarAudit({
    ...getAuditContext(), accion: 'persona.invitada',
    recursoTipo: 'persona', recursoId: id, recursoNombre: nombre.trim(),
    datosDespues: { telefono: tel, roles },
    severidad: 'warning',
  });
  return { id, nombre: nombre.trim(), telefono: tel, pin, esNueva: true };
}

// Login con teléfono + PIN. NO enumera usuarios — siempre devuelve el mismo
// mensaje de error genérico para no revelar si el teléfono existe.
// Devuelve la persona en el formato que usa el frontend (camelCase).
export async function loginConTelefono(telefono, pin) {
  ...
  // Buscamos por todos los teléfonos y normalizamos en memoria porque el
  // formato guardado puede tener variantes (con/sin espacios). Pequeño costo,
  // confiable.
  const { data: candidatos, error } = await supabase.from('personal')
    .select('*');
  if (error) throw error;

  const persona = (candidatos || []).find(p =>
    normalizarTelefono(p.telefono) === tel && p.pin === pin
  );
  if (!persona) throw new Error('Teléfono o PIN incorrecto');

  // Mapear a formato frontend (mismo shape que loadAllData → personal[i])
  return mapPersonaRow(persona);
}

// Helper: convierte una row de la tabla personal al formato camelCase del frontend.
...
// Cambia el PIN de una persona y desactiva el flag pin_temporal.
// Validaciones: 4-6 dígitos numéricos.
export async function cambiarPin(personaId, nuevoPin) {
