...
export function generarPendientes(ctx) {
  ...
  // ---- Asignar levantamientos: responsabilidad de Edwin (comercial) ----
  // v8.37.0: solo se EXIGE asignar cuando la visita YA está coordinada
  // (fecha_visita_programada). Sin coordinación no hay a quién mandar — la
  // lógica del negocio lo dice: si no se puede coordinar, no se asigna.
  const esComercial = /edwin/i.test(usuario.nombre || '') && /parra/i.test(usuario.nombre || '');
  if (esComercial) {
    surveys.filter(s => s.status === 'planning' && !s.asignado_a_id && s.fecha_visita_programada).forEach(s => {
      push({ id: 'lev_asig_' + s.id, grupo: 'levantamientos', titulo: `Asignar levantamiento: ${s.client_name || '(sin cliente)'}`,
        detalle: `Cita coordinada (${(s.fecha_visita_programada || '').slice(0, 10)}) y sin responsable`, urgente: (diasDesde(s.created_at) ?? 0) > 3,
