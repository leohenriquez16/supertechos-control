...
export const KPIS_SUPERVISOR = [
  ...
  { key: 'caja', label: 'Caja chica al día', pesoDefault: 10 },
  { key: 'produccion', label: 'Producción vs meta', pesoDefault: 15 },
  { key: 'calidad', label: 'Calidad (reclamaciones)', pesoDefault: 10 },
];
...
// v8.31.1 (principio de Leonardo: "para todos, lo más importante es que el ERP
// esté 100% al día"): entra "Creación correcta de proyectos" — un proyecto
// aprobado en Odoo queda completo en el ERP EL MISMO DÍA (áreas, tareas, cliente,
// contacto, ubicación, valor, equipo). Aplica a Miguel y a Erisdania (mismo rol).
export const KPIS_GERENTE = [
  { key: 'produccion', label: 'Producción vs meta', pesoDefault: 10 },
  { key: 'avance', label: 'Obras avanzando', pesoDefault: 15 },
  { key: 'erp_al_dia', label: 'ERP al día', pesoDefault: 15 },
  { key: 'creacion', label: 'Proyectos creados completos', pesoDefault: 15 },
  { key: 'facturacion', label: 'Terminadas → facturadas', pesoDefault: 15 },
  { key: 'cubicaciones', label: 'Cubicaciones mensuales', pesoDefault: 10 },
...
// v8.31.1: Caja chica al día — gasto hecho hoy se reporta hoy (o máximo mañana).
// Solo evalúa a quien tiene caja habilitada y tuvo movimientos en el trimestre.
function kpiCajaAlDia(cajaMovs, persona, trimestre, peso) {
  const habilitada = !!persona?.cajaChicaHabilitada;
  const mios = (cajaMovs || []).filter(m => m.creado_por_id === persona.id &&
    (m.fecha || '').slice(0, 10) >= trimestre.inicio && (m.fecha || '').slice(0, 10) <= trimestre.fin);
  if (!habilitada || mios.length === 0) {
    return { key: 'caja', label: 'Caja chica al día', peso, score: null,
      detalle: habilitada ? 'Sin gastos de caja en el trimestre' : 'Sin caja chica asignada' };
  }
  const aTiempo = mios.filter(m => {
    const dias = (new Date((m.created_at || '').slice(0, 10) + 'T12:00:00') - new Date((m.fecha || '').slice(0, 10) + 'T12:00:00')) / 86400000;
    return dias <= 1; // mismo día o el siguiente
  }).length;
  return { key: 'caja', label: 'Caja chica al día', peso, score: (aTiempo / mios.length) * 100,
    detalle: `${aTiempo}/${mios.length} gastos reportados el mismo día o el siguiente` };
}

// v8.31.1: Proyectos creados COMPLETOS — aprobado en Odoo = completo en el ERP el
// mismo día (áreas, tareas, cliente, contacto, ubicación, valor, equipo). Los
// aprobados hoy aún no evalúan (tienen el día para completarse).
export function kpiCreacionProyectos(data, trimestre, peso, faltantesFn) {
  const hoy = hoyRD();
  const aprobDelTrimestre = (data.proyectos || []).filter(p => !p.archivado &&
    (p.fechaAprobacion || '').slice(0, 10) >= trimestre.inicio && (p.fechaAprobacion || '').slice(0, 10) <= trimestre.fin);
  const evaluables = aprobDelTrimestre.filter(p => (p.fechaAprobacion || '').slice(0, 10) < hoy);
  if (evaluables.length === 0) {
    return { key: 'creacion', label: 'Proyectos creados completos', peso, score: null, detalle: 'Sin proyectos aprobados evaluables aún' };
  }
  const incompletos = evaluables.filter(p => faltantesFn(p, data.sistemas).length > 0);
  return { key: 'creacion', label: 'Proyectos creados completos', peso,
    score: ((evaluables.length - incompletos.length) / evaluables.length) * 100,
    detalle: incompletos.length
      ? `${incompletos.length}/${evaluables.length} aprobados con información incompleta`
      : `${evaluables.length} proyectos aprobados, todos completos 🎉` };
}

// ---------------- Supervisor ----------------
// ctx = { data, jornadas (del trimestre), reclamaciones, surveys, cajaMovs, trimestre, config }
export function calcularBonoSupervisor(persona, ctx) {
  const { data, jornadas, reclamaciones, surveys = [], cajaMovs = [], trimestre, config } = ctx;
  ...
  const kProd = { key: 'produccion', label: 'Producción vs meta', peso: 10,
  ...
  const kCalidad = { key: 'calidad', label: 'Calidad (reclamaciones)', peso: 10, score: clamp(100 - 25 * recs.length, 0, 100),
  ...
  const kCaja = kpiCajaAlDia(cajaMovs, persona, trimestre, 10); // v8.31.1: supervisores con caja

  return combinar(aplicarOverrides([kJornadas, kReportes, kAvance, kLev48, kCaja, kProd, kCalidad], config?.kpiOverrides));
}
...
export function calcularBonoGerente(persona, ctx) {
  ...
  const kFact = { key: 'facturacion', label: 'Terminadas → facturadas', peso: 15,
  ...
  const kCub = { key: 'cubicaciones', label: 'Cubicaciones mensuales', peso: 10,
  ...
  const kCreacion = kpiCreacionProyectos(data, trimestre, 15, ctx.faltantesFn || (() => [])); // v8.31.1

  return combinar(aplicarOverrides([kProd, kAvance, kErp, kCreacion, kFact, kCub, kLev, kRecl], config?.kpiOverrides));
}
...
export function calcularBonoComercial(persona, ctx) {
  ...
  // 2 · Nada sin asignar: levantamientos con CITA COORDINADA sin responsable +2 días.
  // v8.37.0: sin fecha_visita_programada NO castiga — si el cliente no ha podido
  // coordinar la visita, no hay razón para exigir asignación.
  const sinAsignar = surveys.filter(s => s.status === 'planning' && !s.asignado_a_id &&
    s.fecha_visita_programada && (ahora - new Date(s.created_at).getTime()) > 2 * 86400000).length;
  const kAsig = { key: 'asignacion', label: 'Nada sin asignar', peso: 15, score: clamp(100 - 25 * sinAsignar, 0, 100),
    detalle: sinAsignar ? `${sinAsignar} con cita coordinada sin asignar (+2 días)` : 'Todo lo coordinado está asignado 🎉' };
