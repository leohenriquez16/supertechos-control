...
export default function VistaRutas({ usuario, data, onVolver }) {
  ...
  const MapaDiligencias = useMemo(() => {
    ...
    return (
      <React.Suspense fallback={<div className="bg-zinc-950 border border-zinc-800 rounded-card flex items-center justify-center" style={{ height: 320 }}><span className="text-xs text-zinc-500">Cargando mapa…</span></div>}>
        <MapaLeaflet center={centro} zoom={10} height={320} markers={marcadoresMapa} scrollWheelZoom={false} className="border border-zinc-800" />
      </React.Suspense>
    );
  }, [verMapa, marcadoresMapa]);
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-card p-3 space-y-2">
      ...
      {paradaLibre?.viajeId === v.id && (
        <div className="bg-zinc-950 border border-cyan-800/50 rounded-card p-2 space-y-1.5">
          ...
          {/* v8.40.0: lugar frecuente → autollena nombre y coordenadas (la ruta se carga sola) */}
          {lugares.length > 0 && (
            <select defaultValue="" onChange={e => { const l = lugares.find(x => x.id === e.target.value); if (l) setParadaLibre({ ...paradaLibre, lugar: l.nombre, lat: l.lat, lng: l.lng }); e.target.value = ''; }}
              className="w-full bg-zinc-900 border border-cyan-800/60 rounded-card px-1.5 py-1.5 text-xs text-cyan-300">
              <option value="" disabled>📍 Elegir lugar frecuente (suplidor / puerto / almacén)…</option>
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl lg:max-w-[1400px] mx-auto space-y-4">
      ...
      {gestionLugares && <ModalLugares lugares={lugares} onCerrar={() => setGestionLugares(false)} onCambio={recargar} />}
    </div>
  );
}

// v8.40.0: gestor de LUGARES FRECUENTES — suplidores, puertos, almacenes fiscales
// y propios, con su ubicación (se pega el link de Google Maps o "lat, lng").
function ModalLugares({ lugares, onCerrar, onCambio }) {
  const [nombre, setNombre] = useState('');
  const [tipo, setTipo] = useState('suplidor');
