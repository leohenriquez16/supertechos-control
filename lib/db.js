...
export async function importarPeajes({ filas, archivo, usuario }) {
  ...
  for (const tag of tagsArchivo) {
    const p = previo[tag];
    if (p?.fusionarCon) { destino[tag] = destino[p.fusionarCon] ?? null; }
    if (p?.vehiculoId) { destino[tag] = p.vehiculoId; continue; }
    if (p?.estado === 'fuera_flota') { destino[tag] = null; continue; }
    const v = porPlaca[normPlaca(p?.placaPortal)];
    destino[tag] = v ? v.id : null;
  }
