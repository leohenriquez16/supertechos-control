...
export async function generarTareasRecurrentesCon(supabase, hoy) {
  ...
  for (const row of (data || [])) {
    ...
    const { error: e1 } = await supabase.from('tareas').insert({
      ...
      historial: [
        { tipo: 'creada', por: 'Sistema (recurrente)', at: new Date().toISOString() },
        ...(rec.responsableNombre ? [{ tipo: 'asignada', a: rec.responsableNombre, por: 'Sistema (recurrente)', at: new Date().toISOString() }] : []),
      ],
    });
