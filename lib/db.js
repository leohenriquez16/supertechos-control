...
export async function crearRequisicion({ proyectoId, solicitadoPorId, solicitadoPorNombre, urgente, notas, items, esCompra = false, cotizacionUrl = null }) {
  const id = 'req_' + Date.now() + Math.random().toString(36).slice(2, 6);
  const { error } = await supabase.from('requisiciones').insert({
    id, proyecto_id: proyectoId, solicitado_por_id: solicitadoPorId || null,
