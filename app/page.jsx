...
import { CheckCircle2, ArrowLeft, Calendar, Clock, Loader2, LogOut, UserCircle, Zap, Package, AlertTriangle, TrendingUp, Truck, Plus, FileUp, FileText, Sparkles, X, HelpCircle, Users, Edit2, Save, Trash2, Settings, DollarSign, Utensils, ChevronDown, ChevronUp } from 'lucide-react';
...
const SISTEMAS_DEFAULT = {
  lona_asfaltica: {
    id: 'lona_asfaltica',
    nombre: 'Lona Asfáltica',
    precio_m2: 690,
    costo_mo_m2: 150,
    keywords_cotizacion: ['lona asfáltica', 'lona asfaltica', 'AP - Lona'],
    tareas: [
      { id: 'prep', nombre: 'Preparación', peso: 10, reporta: 'm2' },
      { id: 'imp', nombre: 'Imprimante', peso: 15, reporta: 'm2_y_cubetas' },
      { id: 'inst', nombre: 'Instalación membrana', peso: 55, reporta: 'rollos' },
      { id: 'rem', nombre: 'Remates', peso: 15, reporta: 'm2' },
      { id: 'entr', nombre: 'Prueba y entrega', peso: 5, reporta: 'm2' },
    ],
    materiales: [
      { id: 'membrana', nombre: 'Membrana asfáltica', unidad: 'rollo', unidad_plural: 'rollos', rinde_m2: 8.5, costo_unidad: 3500, tarea_asociada: 'inst', modo_consumo: 'reportado', keywords_odoo: ['lona asfaltica', 'lona asfáltica', 'rollo lona', 'membrana', 'bitunil', 'nilobit'] },
      { id: 'gas', nombre: 'Gas propano', unidad: 'tanque', unidad_plural: 'tanques', rinde_m2: 100, costo_unidad: 1200, tarea_asociada: 'inst', modo_consumo: 'calculado', keywords_odoo: ['gas propano', 'gas', 'propano', 'tanque gas'] },
      { id: 'primer', nombre: 'Primer asfáltico', unidad: 'cubeta', unidad_plural: 'cubetas', rinde_m2: 40, costo_unidad: 2800, tarea_asociada: 'imp', modo_consumo: 'reportado', keywords_odoo: ['primer', 'imprimante', 'imprimador'] },
    ],
  },
};

const PERSONAL_INICIAL = [
  { id: 'admin', nombre: 'Leo Henriquez', pin: '0000', roles: ['admin'] },
  { id: 'p_juan', nombre: 'Juan Pérez', pin: '1234', roles: ['supervisor', 'maestro'] },
  { id: 'p_carlos', nombre: 'Carlos Martínez', pin: '5678', roles: ['supervisor'] },
  { id: 'p_miguel', nombre: 'Miguel Rodríguez', pin: '2468', roles: ['maestro'] },
  { id: 'p_pedro', nombre: 'Pedro Santana', roles: ['ayudante'], maestroId: 'p_juan' },
  { id: 'p_luis', nombre: 'Luis Mejía', roles: ['ayudante'], maestroId: 'p_juan' },
  { id: 'p_jose', nombre: 'José Ramírez', roles: ['ayudante'], maestroId: 'p_miguel' },
];

const PROYECTO_INICIAL = {
  id: 'hosp_seibo', nombre: 'Hospital del Seibo', cliente: 'Hospital del Seibo',
  sistema: 'lona_asfaltica', supervisorId: 'p_juan', maestroId: 'p_juan',
  ayudantesIds: ['p_pedro', 'p_luis'],
  fecha_inicio: '2026-04-13', fecha_entrega: '2026-04-23',
  referenciaOdoo: 'ST-C5437', referenciaProyecto: 'Hospital del Seibo',
  areas: [
    { id: 'techo_h', nombre: 'Techo Hombres', m2: 200 },
    { id: 'techo_m', nombre: 'Techo Mujeres', m2: 800 },
    { id: 'techo_t', nombre: 'Techo Total', m2: 400 },
  ],
  dieta: { habilitada: true, tarifa_dia_persona: 800, dias_hombre_presupuestados: 32, personasIds: ['p_juan', 'p_pedro', 'p_luis'] },
};

const CONFIG_DEFAULT = { costos_indirectos_pct: 15, margen_objetivo_pct: 30 };

// ============================================================
// STORAGE (localStorage para Vercel)
// ============================================================
const STORAGE_KEY = 'supertechos_data_v6';
...
const loadData = () => {
  ...
  return {
    proyectos: [PROYECTO_INICIAL], reportes: [], envios: [],
    personal: PERSONAL_INICIAL, sistemas: SISTEMAS_DEFAULT, config: CONFIG_DEFAULT,
  };
};
...
// ============================================================
// EXTRACCIÓN PDF (Claude API via nuestro backend)
// ============================================================
...
Sistemas disponibles: ${sistemasDescripcion || '(usa keywords de las partidas)'}
{
  ...
  "m2Principal": number (m² de la partida principal del sistema, NO de limpieza)
}`,
...
Responde ÚNICAMENTE con un objeto JSON válido, sin texto adicional, sin markdown fences.
{
  ...
  // Llama a nuestro endpoint de API (que protege el API key)
...
const calcAnalisisCosto = (proyecto, reportes, envios, sistema, config) => {
  ...
  return {
    valorContrato, m2Total,
    costoMaterialesTeorico, costoMaterialesReal, costoMO,
    costoDietaPresupuestado, costoDietaReal,
    costoIndirectoTeorico, costoIndirectoReal,
    costoTotalTeorico, costoTotalReal,
    margenTeorico, margenReal, margenPctTeorico, margenPctReal,
  };
};
...
export default function App() {
  ...
  if (!data) return <div className="min-h-screen bg-zinc-950 flex items-center justify-center"><Loader2 className="w-8 h-8 text-red-600 animate-spin" /></div>;
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        ...
        {esAdmin && vista === 'personal' && <GestionPersonal personal={data.personal} onVolver={() => setVista('dashboard')} onActualizar={(p) => update({ ...data, personal: p })} />}
        {esAdmin && vista === 'sistemas' && <GestionSistemas sistemas={data.sistemas} config={data.config} onVolver={() => setVista('dashboard')} onActualizarSistemas={(s) => update({ ...data, sistemas: s })} onActualizarConfig={(c) => update({ ...data, config: c })} />}
        {esAdmin && vista === 'nuevoProyecto' && <NuevoProyecto personal={data.personal} sistemas={data.sistemas} onCancelar={() => setVista('dashboard')} onCrear={(proy) => { update({ ...data, proyectos: [...data.proyectos, { ...proy, id: 'p_' + Date.now() }] }); setVista('dashboard'); }} />}
        {esAdmin && vista === 'proyecto' && proyectoActivo && (
          <DetalleProyecto proyecto={data.proyectos.find(p => p.id === proyectoActivo.id) || proyectoActivo} data={data} tab={tab} setTab={setTab}
            ...
            onActualizarProyecto={(pa) => { const n = data.proyectos.map(p => p.id === pa.id ? pa : p); update({ ...data, proyectos: n }); }}
            onRegistrarEnvio={(e) => update({ ...data, envios: [...data.envios, { ...e, id: 'e_' + Date.now() + Math.random() }] })}
            onRegistrarEnviosLote={(es) => { const n = es.map(e => ({ ...e, id: 'e_' + Date.now() + Math.random() })); update({ ...data, envios: [...data.envios, ...n] }); }}
          />
        )}
        ...
        {!esAdmin && vista === 'reportar' && proyectoActivo && <FormReporte usuario={usuario} proyecto={proyectoActivo} reportes={data.reportes} sistema={data.sistemas[proyectoActivo.sistema]} onCancelar={() => setVista('misProyectos')} onTerminar={() => setVista('misProyectos')} onGuardar={(r) => update({ ...data, reportes: [...data.reportes, { ...r, id: 'r_' + Date.now() + Math.random() }] })} />}
      </main>
    </div>
  );
}
      ...
      <div className="w-full max-w-sm">
        ...
        {!sel ? (
          ...
          <div className="space-y-3">
            ...
            <div className="text-[10px] text-zinc-600 text-center mt-4">Demo: Leo/0000 · Juan/1234 · Miguel/2468</div>
          </div>
        )}
      </div>
    </div>
...
function GestionSistemas({ sistemas, config, onVolver, onActualizarSistemas, onActualizarConfig }) {
  ...
  const nuevoSistema = () => {
    setSistemaEditando({
      id: 's_' + Date.now(), nombre: '', precio_m2: 0, costo_mo_m2: 0,
      tareas: [{ id: 't_' + Date.now(), nombre: '', peso: 100, reporta: 'm2' }],
      materiales: [], keywords_cotizacion: [],
    });
  };
  ...
  const guardarConfig = () => {
    onActualizarConfig({
      costos_indirectos_pct: parseFloat(configEditada.costos_indirectos_pct) || 0,
      margen_objetivo_pct: parseFloat(configEditada.margen_objetivo_pct) || 0,
    });
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-zinc-900 border border-zinc-800 p-4 space-y-3">
        ...
        <button onClick={guardarConfig} className="bg-red-600 hover:bg-red-700 text-white text-xs font-black uppercase px-4 py-2 flex items-center gap-1"><Save className="w-3 h-3" /> Guardar</button>
      </div>

      <div>
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-xs tracking-widest uppercase text-zinc-400 font-bold">Sistemas ({sistemasArray.length})</h2>
          <button onClick={nuevoSistema} className="text-xs text-red-500 flex items-center gap-1 font-bold uppercase tracking-wider"><Plus className="w-3 h-3" /> Nuevo</button>
        </div>
        {sistemasArray.length === 0 && <div className="text-xs text-zinc-500 text-center py-8 bg-zinc-900 border border-zinc-800">No hay sistemas.</div>}
        <div className="space-y-2">
          {sistemasArray.map(s => {
  ...
  return (
    <div className="space-y-5 max-w-4xl mx-auto">
      ...
      <div className="bg-zinc-900 border border-zinc-800 p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Campo label="Precio venta/m² (RD$)"><Input type="number" value={sistema.precio_m2} onChange={v => setSistema({ ...sistema, precio_m2: v })} /></Campo>
          <Campo label="Costo mano obra/m² (RD$)"><Input type="number" value={sistema.costo_mo_m2} onChange={v => setSistema({ ...sistema, costo_mo_m2: v })} /></Campo>
        </div>
        <Campo label="Keywords cotización (comas)"><Input value={Array.isArray(sistema.keywords_cotizacion) ? sistema.keywords_cotizacion.join(', ') : sistema.keywords_cotizacion} onChange={v => setSistema({ ...sistema, keywords_cotizacion: v })} /></Campo>
      </div>
          ...
          {sistema.materiales.map((m, i) => (
            <div key={m.id} className="border border-zinc-800 bg-zinc-950 p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Campo label="Costo por unidad (RD$)"><Input type="number" value={m.costo_unidad} onChange={v => actMat(i, 'costo_unidad', v)} /></Campo>
              </div>
              <div className="grid grid-cols-2 gap-2">
              ...
              <Campo label="Keywords Odoo (comas)"><Input value={Array.isArray(m.keywords_odoo) ? m.keywords_odoo.join(', ') : m.keywords_odoo} onChange={v => actMat(i, 'keywords_odoo', v)} /></Campo>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-4 space-y-3">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <div className="text-[11px] tracking-widest uppercase text-zinc-400 font-bold flex items-center gap-1"><Utensils className="w-3 h-3" /> Proyecto en el interior (dieta + dormida)</div>
          </label>
          {form.dieta.habilitada && (
            <div className="space-y-3 pt-2 border-t border-zinc-800">
              <div className="grid grid-cols-2 gap-2"><Campo label="Tarifa día/persona"><Input type="number" value={form.dieta.tarifa_dia_persona} onChange={v => setForm({ ...form, dieta: { ...form.dieta, tarifa_dia_persona: v } })} /></Campo><Campo label="Días-hombre presup."><Input type="number" value={form.dieta.dias_hombre_presupuestados} onChange={v => setForm({ ...form, dieta: { ...form.dieta, dias_hombre_presupuestados: v } })} /></Campo></div>
              {form.dieta.tarifa_dia_persona && form.dieta.dias_hombre_presupuestados && <div className="text-xs bg-zinc-950 border border-zinc-800 p-2">Total: <span className="text-white font-bold">{formatRD(parseFloat(form.dieta.tarifa_dia_persona) * parseFloat(form.dieta.dias_hombre_presupuestados))}</span></div>}
...
// ============================================================
// MIS PROYECTOS, DASHBOARD, DETALLE, TABS (sin cambios)
// ============================================================
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 border-b-2 border-zinc-800 overflow-x-auto">
        ...
        {!esSupervisor && <TabBtn active={tab === 'equipo'} onClick={() => setTab('equipo')}><Users className="w-3 h-3 inline mr-1" />Equipo</TabBtn>}
      </div>
      ...
      {tab === 'equipo' && !esSupervisor && <TabEquipo proyecto={proyecto} personal={data.personal} />}
    </div>
  );
}
...
function TabCosto({ proyecto, sistema, reportes, envios, config }) {
  ...
  const FilaCosto = ({ label, teorico, real, destacado }) => (
    <div className={`grid grid-cols-3 gap-2 py-2 border-b border-zinc-800 ${destacado ? 'font-bold' : ''}`}>
      <div className={`text-xs ${destacado ? 'text-white' : 'text-zinc-400'}`}>{label}</div>
      <div className="text-right text-xs">{formatRD(teorico)}</div>
      <div className={`text-right text-xs ${real > teorico ? 'text-yellow-400' : real < teorico ? 'text-green-400' : ''}`}>{formatRD(real)}</div>
    </div>
  );
  return (
    <div className="space-y-4">
      <div className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-zinc-800 p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><div className="text-[10px] text-zinc-500 uppercase tracking-wider">Valor Contrato</div><div className="text-xl font-black">{formatRD(a.valorContrato)}</div><div className="text-[10px] text-zinc-600">{formatNum(a.m2Total)} m² × {formatRD(sistema.precio_m2)}</div></div>
          <div><div className="text-[10px] text-zinc-500 uppercase tracking-wider">Costo Total</div><div className="text-xl font-black">{formatRD(a.costoTotalTeorico)}</div><div className="text-[10px] text-zinc-600">Real: {formatRD(a.costoTotalReal)}</div></div>
          <div><div className="text-[10px] text-green-400 uppercase tracking-wider">Margen Est.</div><div className="text-xl font-black text-green-400">{formatRD(a.margenTeorico)}</div><div className="text-[10px] text-zinc-600">{a.margenPctTeorico.toFixed(1)}%</div></div>
          <div><div className="text-[10px] text-zinc-500 uppercase tracking-wider">Objetivo</div><div className={`text-xl font-black ${a.margenPctTeorico >= config.margen_objetivo_pct ? 'text-green-400' : 'text-yellow-400'}`}>{config.margen_objetivo_pct}%</div><div className="text-[10px] text-zinc-600">{a.margenPctTeorico >= config.margen_objetivo_pct ? '✓ OK' : '⚠ Bajo'}</div></div>
        </div>
      </div>
      <div className="bg-zinc-900 border border-zinc-800 p-4">
        ...
        <FilaCosto label="COSTO TOTAL" teorico={a.costoTotalTeorico} real={a.costoTotalReal} destacado />
      </div>
      <div>
        <div className="text-[11px] tracking-widest uppercase text-zinc-400 font-bold mb-2">Detalle materiales</div>
        <div className="space-y-1">{sistema.materiales.map(mat => { const r = a.m2Total / mat.rinde_m2; return <div key={mat.id} className="bg-zinc-900 border border-zinc-800 p-3 flex justify-between items-center"><div><div className="font-bold text-sm">{mat.nombre}</div><div className="text-[10px] text-zinc-500">{formatNum(r)} {mat.unidad_plural} × {formatRD(mat.costo_unidad || 0)}</div></div><div className="font-bold">{formatRD(r * (mat.costo_unidad || 0))}</div></div>; })}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gradient-to-br from-orange-900/40 to-zinc-950 border border-orange-900/50 p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div><div className="text-[10px] text-zinc-500 uppercase tracking-wider">Presupuestado</div><div className="text-xl font-black">{formatRD(dieta.montoPresupuestado)}</div><div className="text-[10px] text-zinc-600">{dieta.diasHombrePresupuestados} días-hombre × {formatRD(dieta.tarifa)}</div></div>
          <div><div className="text-[10px] text-zinc-500 uppercase tracking-wider">Consumido</div><div className={`text-xl font-black ${dieta.pctConsumido > 100 ? 'text-red-400' : dieta.pctConsumido > 80 ? 'text-yellow-400' : 'text-green-400'}`}>{formatRD(dieta.montoConsumido)}</div><div className="text-[10px] text-zinc-600">{dieta.diasHombreConsumidos} días ({dieta.pctConsumido.toFixed(1)}%)</div></div>
          ...
          <div><div className="text-[10px] text-zinc-500 uppercase tracking-wider">Tarifa</div><div className="text-xl font-black">{formatRD(dieta.tarifa)}</div><div className="text-[10px] text-zinc-600">{dieta.personasPorDia} p/día</div></div>
        </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 p-4 space-y-3">
        ...
        {editando ? (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-2"><Campo label="Tarifa día/persona"><Input type="number" value={dietaEdit.tarifa_dia_persona} onChange={v => setDietaEdit({ ...dietaEdit, tarifa_dia_persona: v })} /></Campo><Campo label="Días-hombre"><Input type="number" value={dietaEdit.dias_hombre_presupuestados} onChange={v => setDietaEdit({ ...dietaEdit, dias_hombre_presupuestados: v })} /></Campo></div>
          ...
          <div className="space-y-2">
            <div className="text-xs text-zinc-400">Días trabajados: <span className="text-white font-bold">{dieta.diasTrabajados}</span></div>
            <div className="text-xs text-zinc-400">Personas por día: <span className="text-white font-bold">{dieta.personasPorDia}</span></div>
          </div>
        )}
      </div>
    </div>
...
function TabEquipo({ proyecto, personal }) {
  const supervisor = getPersona(personal, proyecto.supervisorId);
  const maestro = getPersona(personal, proyecto.maestroId);
  const ayudantes = (proyecto.ayudantesIds || []).map(id => getPersona(personal, id)).filter(Boolean);
  return (
    <div className="space-y-4">
      <div><div className="text-[11px] tracking-widest uppercase text-zinc-400 font-bold mb-2">Supervisor</div>{supervisor ? <PersonaCard persona={supervisor} icono="👔" /> : <div className="text-xs text-zinc-500">-</div>}</div>
      <div><div className="text-[11px] tracking-widest uppercase text-zinc-400 font-bold mb-2">Maestro</div>{maestro ? <PersonaCard persona={maestro} icono="🔨" /> : <div className="text-xs text-zinc-500">-</div>}</div>
      <div><div className="text-[11px] tracking-widest uppercase text-zinc-400 font-bold mb-2">Ayudantes ({ayudantes.length})</div>{ayudantes.length === 0 ? <div className="text-xs text-zinc-500">Sin ayudantes.</div> : <div className="space-y-1">{ayudantes.map(a => <PersonaCard key={a.id} persona={a} icono="👷" />)}</div>}</div>
    </div>
  );
}

function PersonaCard({ persona, icono }) {
  return <div className="bg-zinc-900 border border-zinc-800 p-3 flex items-center gap-3"><div className="text-2xl">{icono}</div><div><div className="font-bold text-sm">{persona.nombre}</div></div></div>;
}
  ...
  return (
    <div className="space-y-4">
      ...
      {modo === 'pdf' && !pdfExtraido && (
        <div className="space-y-3">
          <div className="flex justify-between items-center"><div className="text-xs font-bold uppercase">Subir Salida</div><button onClick={() => setModo(null)} className="text-zinc-500"><X className="w-4 h-4" /></button></div>
