...
export async function eliminarProyecto(proyectoId) {
  ...
  // v8.9.26: notificación por email (fire-and-forget, no bloquea)
  try {
    notificarEventoInterno('proyecto_eliminado', {
      mensaje: `Se eliminó el proyecto "${proyectoAntes?.nombre || proyectoAntes?.cliente || proyectoId}" del ERP.`,
      detalles: {
        'Proyecto': proyectoAntes?.nombre || proyectoAntes?.cliente || '(sin nombre)',
        'Referencia': proyectoAntes?.referencia_odoo || '—',
        'Cliente': proyectoAntes?.cliente || '—',
        'Estado previo': proyectoAntes?.estado || '—',
        'Eliminado por': ctx.usuarioNombre || 'Usuario desconocido',
...
// ============================================================
// Helper interno para llamar al endpoint de email sin crear dependencia circular
async function notificarEventoInterno(evento_key, opciones = {}) {
  try {
    await fetch('/api/email/send', {
      ...
      body: JSON.stringify({
        ...
        titulo_custom: opciones.titulo_custom,
      }),
    });
  } catch (e) {
    console.warn('notificarEventoInterno falló:', e.message);
  }
}
// Lista todas las configuraciones de notificaciones
