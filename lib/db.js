...
// ============================================================
// v8.17.58: estados que requieren validación de location + al menos un contacto.
// Por ahora solo en_ejecucion. PR E agregará 'planificado' a este set.
const ESTADOS_REQUIEREN_LOCATION_Y_CONTACTO = new Set(['en_ejecucion']);
...
export async function validarRequisitosCambioEstado(proyectoId, estadoNuevo) {
  if (!ESTADOS_REQUIEREN_LOCATION_Y_CONTACTO.has(estadoNuevo)) return;
  const { data: p } = await supabase
    .from('proyectos')
    .select('ubicacion_lat, ubicacion_lng, contacto_principal_id')
    .eq('id', proyectoId).single();
  const faltantes = [];
  if (p?.ubicacion_lat == null || p?.ubicacion_lng == null) faltantes.push('ubicación (lat/lng)');
  // Hay contacto si: principal asignado O al menos 1 en proyecto_contactos.
  ...
  if (faltantes.length > 0) {
    throw new Error(`Para pasar a ${estadoNuevo}, falta: ${faltantes.join(' y ')}. Edita el proyecto y completa esos datos.`);
  }
}
