...
export const GRUPOS = {
  ...
  almacen: { label: 'Almacén y envíos', orden: 3 },
  levantamientos: { label: 'Levantamientos', orden: 4 },
  reclamaciones: { label: 'Reclamaciones', orden: 5 },
  gerencia: { label: 'Gerencia', orden: 6 },
  tareas: { label: 'Tareas asignadas', orden: 7 },
};
...
export function generarPendientes(ctx) {
  const { usuario, data, jornadas7d = [], surveys = [], reclamaciones = [], tareas = [], cortes = [], requisiciones = [], esAdmin = false, esAlmacen = false } = ctx;
  const hoy = hoyRD();
  const out = [];
