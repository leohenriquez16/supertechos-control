...
export async function loadAllData() {
  try {
    ...
    const personal = (personalRes.data || []).map(p => ({
      ...
      // v8.17.29: dieta + hospedaje (toggles a nivel persona)
    ...
    // v8.17.29: Config global de dieta + hospedaje (defaults si la migración aún no se aplicó)
    let configDieta = { desayunoRd: 200, comidaRd: 350, cenaRd: 350, hotelRd: 900, updatedAt: null, updatedPorId: null };
    try {
      configDieta = await obtenerConfigDieta();
    } catch (e) { console.warn('Config dieta no cargada:', e.message); }

    // v8.17.49: Propiedades de la empresa (apartamentos, casas) — solo activas para uso en cierre de jornada.
    let propiedadesEmpresa = [];
    try {
      propiedadesEmpresa = await listarPropiedadesEmpresa({ soloActivas: true });
    } catch (e) { console.warn('Propiedades empresa no cargadas:', e.message); }
...
const SELECT_CAJA = 'id, persona_id, proyecto_id, fecha, tipo, monto, signo_ajuste, foto_path, proveedor, rnc, concepto, datos_ia, status, motivo_rechazo, aprobado_por_id, aprobado_at, creado_por_id, created_at, rotacion_grados, empresa_receptora, aplica_a, sub_tipo';
...
export async function actualizarMovimientoCajaChica(id, campos) {
  ...
  if (campos.empresaReceptora !== undefined) updates.empresa_receptora = campos.empresaReceptora || null; // v8.17.25
  // v8.17.29: override manual de partida y sub-tipo (admin puede corregir).
  if (campos.aplicaA !== undefined) updates.aplica_a = campos.aplicaA || null;
  if (campos.subTipo !== undefined) updates.sub_tipo = campos.subTipo || null;
