...
// ============================================================
// v8.27.18: Actas de proyecto firmadas vía DocuSeal
// (cubicaciones, entregas por área, entrega final del proyecto/edificio)
// ============================================================

const SELECT_ACTA = 'id, proyecto_id, area_id, area_ids, cc_emails, docuseal_documento_url, tipo, garantia_id, empresa, snapshot_datos, cliente_nombre, cliente_email, cliente_telefono, cliente_via_envio, docuseal_submission_id, docuseal_url_firmante, status, enviada_at, vista_por_cliente_at, firmada_at, rechazada_at, pdf_firmado_path, status_facturacion, facturada_at, facturada_por_id, factura_referencia, garantia_inicio_at, garantia_fin_at, garantia_anios, generada_por_id, created_at, updated_at';

function actaRowToObj(r) {
  if (!r) return null;
  return {
    id: r.id,
    proyectoId: r.proyecto_id,
    ...
    docusealDocumentoUrl: r.docuseal_documento_url || null,
    tipo: r.tipo,
    garantiaId: r.garantia_id || null,
    empresa: r.empresa || null,
    snapshotDatos: r.snapshot_datos || {},
    clienteNombre: r.cliente_nombre || '',
    clienteEmail: r.cliente_email || '',
    clienteTelefono: r.cliente_telefono || '',
    clienteViaEnvio: r.cliente_via_envio || null,
    docusealSubmissionId: r.docuseal_submission_id || null,
    docusealUrlFirmante: r.docuseal_url_firmante || null,
    status: r.status,
    enviadaAt: r.enviada_at,
    vistaPorClienteAt: r.vista_por_cliente_at,
    firmadaAt: r.firmada_at,
    rechazadaAt: r.rechazada_at,
    pdfFirmadoPath: r.pdf_firmado_path,
    statusFacturacion: r.status_facturacion,
    facturadaAt: r.facturada_at,
    facturadaPorId: r.facturada_por_id,
    facturaReferencia: r.factura_referencia,
    garantiaInicioAt: r.garantia_inicio_at,
    garantiaFinAt: r.garantia_fin_at,
    garantiaAnios: r.garantia_anios,
    generadaPorId: r.generada_por_id,
    createdAt: r.created_at,
    updatedAt: r.updated_at,
  };
}

// Crea un acta + submission en DocuSeal (server-side).
// Llama al endpoint /api/actas/crear que es quien tiene la API key.
// Devuelve el acta creada con su URL de firma para compartir al cliente.
export async function crearActaProyecto({ tipo, proyectoId, areaId = null, areaIds = [], ccEmails = [], empresa = null, garantiaId = null, garantiaInicio = null, garantiaFin = null, garantiaAnios = null, cliente, snapshotDatos, generadaPorId, docusealValues = {} }) {
  const res = await fetch('/api/actas/crear', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      tipo, proyectoId, areaId, areaIds, ccEmails, empresa, garantiaId, garantiaInicio, garantiaFin, garantiaAnios,
      cliente, snapshotDatos, generadaPorId, docusealValues,
    }),
  });
  const json = await res.json();
  if (!res.ok) {
  ...
  return { acta: actaRowToObj(json.acta), urlFirmante: json.urlFirmante };
}

// Lista actas con filtros.
export async function listarActas({ proyectoId = null, garantiaId = null, status = null, tipo = null, statusFacturacion = null } = {}) {
  let q = supabase.from('actas_proyecto').select(SELECT_ACTA).order('created_at', { ascending: false });
  if (proyectoId) q = q.eq('proyecto_id', proyectoId);
  if (garantiaId) q = q.eq('garantia_id', garantiaId);
  if (status) q = q.eq('status', status);
  if (tipo) q = q.eq('tipo', tipo);
  if (statusFacturacion) q = q.eq('status_facturacion', statusFacturacion);
  const { data, error } = await q;
  if (error) throw error;
  return (data || []).map(actaRowToObj);
}

// Trae una sola acta.
export async function obtenerActa(actaId) {
  const { data, error } = await supabase.from('actas_proyecto').select(SELECT_ACTA).eq('id', actaId).maybeSingle();
  if (error) throw error;
  return actaRowToObj(data);
}

// Marca una cubicación como facturada (admin/contabilidad).
export async function marcarActaFacturada(actaId, { facturadaPorId, facturaReferencia = null }) {
  const { error } = await supabase.from('actas_proyecto').update({
    status_facturacion: 'facturada',
    facturada_at: new Date().toISOString(),
    facturada_por_id: facturadaPorId || null,
    factura_referencia: facturaReferencia || null,
  }).eq('id', actaId);
  if (error) throw error;
  registrarAudit({
    ...getAuditContext(), accion: 'acta.facturada',
    recursoTipo: 'acta', recursoId: actaId,
    metadata: { facturaReferencia },
    severidad: 'info',
  });
}

// Devuelve un signed URL del PDF firmado para descargar/visualizar.
export async function obtenerUrlPdfFirmado(actaId) {
  const acta = await obtenerActa(actaId);
  if (!acta?.pdfFirmadoPath) return null;
  const { data, error } = await supabase.storage.from('actas-firmadas')
    .createSignedUrl(acta.pdfFirmadoPath, 3600);
  if (error) throw error;
  return data?.signedUrl || null;
}

// Devuelve la URL de firma para reenviar/compartir manualmente (WhatsApp/email).
export async function obtenerUrlFirma(actaId) {
  const acta = await obtenerActa(actaId);
  return acta?.docusealUrlFirmante || null;
}
