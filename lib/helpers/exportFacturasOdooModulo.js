...
// v8.27.54: normaliza para similitud (minúsculas, sin acentos) y tokeniza descartando
// palabras vacías y tokens muy cortos.
const _STOP = new Set(['de', 'y', 'con', 'o', 'la', 'el', 'los', 'las', 'del', 'e', 'a', 'un', 'una', 'para', 'por']);
function _tokens(s) {
  return String(s || '').normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase()
    .split(/[^a-z0-9]+/).filter((t) => t.length >= 3 && !_STOP.has(t));
}

// v8.27.54: resolver por SIMILITUD de nombre → id de producto. Para cuando ni el concepto
// ni la convención "GENERICO - <categoría>" calzan exacto: elige el producto de Odoo con
// más tokens en común (ej. "Materiales de Obra" → "GENERICO - Materiales Varios"). Exige
// al menos 1 token compartido y un ganador ÚNICO (si empatan, no arriesga → null).
export function construirResolverProductoSimilar(productos = []) {
  const idx = productos
    .map((p) => ({ id: p.id, tk: new Set(_tokens(p.name)) }))
    .filter((p) => p.tk.size > 0);
  return (nombre) => {
    const ct = _tokens(nombre);
    if (ct.length === 0) return null;
    let best = 0;
    const idsPorScore = new Map(); // score -> Set(ids)
    for (const p of idx) {
      const sh = ct.filter((t) => p.tk.has(t)).length;
      if (sh === 0) continue;
      if (!idsPorScore.has(sh)) idsPorScore.set(sh, new Set());
      idsPorScore.get(sh).add(p.id);
      if (sh > best) best = sh;
    }
    if (best === 0) return null;
    const ganadores = idsPorScore.get(best);
    return ganadores.size === 1 ? [...ganadores][0] : null;
  };
}
...
// v8.27.56: diario de GASTO por compañía (usa_docs=True) — para que el NCF pegue y separe
// los gastos de las compras normales. ids fijos de Odoo.
const DIARIO_GASTO_ODOO = { super_techos: 73, prouco: 124 };
// v8.27.56: tipo de documento fiscal (l10n_latam.document.type) por prefijo del NCF. En
// import por CSV Odoo NO infiere el tipo del número (el onchange no corre), así que sin
// tipo el NCF se descarta. Mapa fijo de la localización DO (ids estables).
const TIPO_DOC_NCF_ODOO = {
  B01: 1, B02: 2, B03: 3, B04: 4, B11: 5, B12: 6, B13: 7, B14: 8, B15: 9, B16: 10, B17: 11,
  E31: 13, E32: 14, E33: 15, E34: 16, E41: 17, E43: 18, E44: 19, E45: 20, E46: 21, E47: 22,
};
function tipoDocNcf(ncf) {
  const p = String(ncf || '').trim().toUpperCase().slice(0, 3);
  return TIPO_DOC_NCF_ODOO[p] || null;
}
...
function toCSV(facturas, catPorId = {}, maps = {}) {
  const { resolverAnalitica, resolverProducto, resolverProductoSimilar, resolverProveedor, cuentasProducto = {}, cuentaDefault = {}, impuestos = {}, sin } = maps;
  const headers = [
    'move_type', 'company_id', 'journal_id/.id', 'invoice_date', 'partner_id',
    'l10n_latam_document_number', 'l10n_latam_document_type_id/.id', 'ref', 'currency_id',
    ...
    'invoice_line_ids/account_id/.id', 'invoice_line_ids/analytic_distribution',
  ...
  const rows = facturas.map((f) => {
    ...
    // v8.27.58: ITBIS por el dato capturado (no por el producto: PG-Materiales no trae impuesto).
    // Con ITBIS>0 → 18% "precio incluido"; exento o sin ITBIS → Exento. price_unit = total,
    // Odoo extrae el ITBIS por dentro y el total cuadra exacto. Va como tax_ids/.id.
    const impSet = impuestos[f.empresa] || {};
    const esExento = (f.itbisModo === 'exento') || !(Number(f.itbis) > 0);
    const taxId = esExento ? impSet.exento : impSet.itbis18;
    const impuesto = taxId != null ? String(taxId) : '';
    ...
    // Producto → ID de producto de Odoo (product_id/.id). Se deriva SIN mapeos en DB,
    // en capas (primera que resuelve gana):
    //  1) override explícito por categoría (odoo_producto) — opcional, casos raros.
    //  2) concepto de la IA (Gas, Gasoil, Gasolina, Materiales Varios…).
    //  3) convención "GENERICO - <categoría>" (ej. Peaje → "GENERICO - Peaje").
    //  4) nombre de la categoría directo.
    //  5) similitud de tokens (ej. "Materiales de Obra" → "GENERICO - Materiales Varios").
    // Vacío si nada resuelve (no bloquea; el contador asigna) y se reporta.
    const mapeadoProducto = (cat?.odooProducto || '').trim();
    const conceptoProducto = (f.concepto || '').trim();
    const nombreCat = (cat?.nombre || '').trim();
    let productoId = null;
    if (resolverProducto) {
      if (mapeadoProducto) productoId = resolverProducto(mapeadoProducto);
      if (productoId == null && conceptoProducto) productoId = resolverProducto(conceptoProducto);
      if (productoId == null && nombreCat) productoId = resolverProducto(`GENERICO - ${nombreCat}`);
      if (productoId == null && nombreCat) productoId = resolverProducto(nombreCat);
    }
    if (productoId == null && nombreCat && resolverProductoSimilar) productoId = resolverProductoSimilar(nombreCat);
    if (productoId == null && (conceptoProducto || nombreCat) && sin) sin.producto.add(conceptoProducto || nombreCat);
    const producto = productoId != null ? String(productoId) : '';
    // Cuenta contable (account_id/.id): OBLIGATORIA — Odoo NO la autocompleta al importar,
    // y sin ella la línea viola el check de account.move.line. Se toma la cuenta de gasto
    // efectiva del producto según la compañía; si no hay producto o cuenta, el default de la
    // compañía (categoría "All / Saleable"). Vacío solo si Odoo no devolvió refs → se reporta.
    const cuentaId = (productoId != null && cuentasProducto[productoId]?.[f.empresa]) || cuentaDefault[f.empresa] || null;
    const cuenta = cuentaId != null ? String(cuentaId) : '';
    if (!cuenta && sin) sin.cuenta.add(conceptoProducto || nombreCat || (f.ncf || String(f.id)));
    ...
    return [
      ...
      DIARIO_GASTO_ODOO[f.empresa] || '', // journal_id/.id (diario de gasto, usa_docs)
      f.fecha || '',
      proveedor,          // partner_id (nombre EXACTO de Odoo, resuelto por RNC)
      f.ncf || '',
      tipoDocNcf(f.ncf) || '', // l10n_latam_document_type_id/.id (por prefijo del NCF)
      f.ncf || '', // ref (Referencia del proveedor) = NCF → llave para casar la foto luego
      (f.moneda || 'DOP'),
...
export async function generarZipFacturasOdoo({ facturas, categorias = [], cuentasAnaliticas = [], productos = [], proveedores = [], cuentasProducto = {}, cuentaDefault = {}, impuestos = {}, onProgreso }) {
  ...
  const maps = {
    ...
    resolverProductoSimilar: construirResolverProductoSimilar(productos || []),
    resolverProveedor: construirResolverProveedor(proveedores || []),
    cuentasProducto, cuentaDefault, // v8.27.55: cuenta de gasto por producto+compañía
    impuestos, // v8.27.58: ITBIS de compra por compañía
    sin: { analitica: new Set(), producto: new Set(), proveedor: new Set(), cuenta: new Set() },
  };
  ...
  return {
    ...
    counts: {
      ...
      sinCuenta: Array.from(maps.sin.cuenta),
    },
  };
}
